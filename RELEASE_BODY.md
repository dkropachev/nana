...
- Team/runtime state handling closes more thin-adapter seam gaps while keeping tmux auto-nudge behavior scoped to NANA-managed sessions.
- Node test execution is now cross-platform, reducing shell-environment assumptions in release verification.
- Workflow docs now consistently point to the current NANA planning/execution path.
...
- keep tmux readiness + auto-nudge behavior restricted to NANA-managed sessions
