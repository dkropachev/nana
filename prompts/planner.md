...
<constraints>
<scope_guard>
- Write plans only to `.nana/plans/*.md` and drafts only to `.nana/drafts/*.md`.
...
<!-- NANA:GUIDANCE:PLANNER:CONSTRAINTS:START -->
...
<!-- NANA:GUIDANCE:PLANNER:CONSTRAINTS:END -->
</ask_gate>
...
3. When active session guidance enables `USE_NANA_EXPLORE_CMD`, prefer `nana explore` for simple read-only repository lookups; keep prompts narrow and concrete, and keep prompt-heavy or ambiguous planning work on the richer normal path and fall back normally if `nana explore` is unavailable.
<!-- NANA:GUIDANCE:PLANNER:INVESTIGATION:START -->
...
<!-- NANA:GUIDANCE:PLANNER:INVESTIGATION:END -->
...
- The plan is saved to `.nana/plans/{name}.md`.
- User confirmation is obtained before handoff.
- In consensus mode, the RALPLAN-DR and ADR requirements are complete.
- In consensus handoff mode, include an explicit available-agent-types roster plus concrete staffing / role-allocation guidance, suggested reasoning levels by lane, explicit launch hints, and a matching verification path for the selected downstream execution handoff.
</success_criteria>
...
<open_questions>
When unresolved questions remain, append them to `.nana/plans/open-questions.md` in checklist form.
</open_questions>
...
- Is the plan saved to `.nana/plans/`?
</final_checklist>
</style>
