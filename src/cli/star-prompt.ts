/**
 * One-time GitHub star prompt shown at NANA startup.
 * Skipped when no TTY or when gh CLI is not installed.
 * State stored globally (~/.nana/state/star-prompt.json) so it shows once per user.
 */
...
export function starPromptStatePath(): string {
  return join(homedir(), '.nana', 'state', 'star-prompt.json');
}
...
export async function markPrompted(): Promise<void> {
  const stateDir = join(homedir(), '.nana', 'state');
...
export async function maybePromptGithubStar(deps: MaybePromptGithubStarDeps = {}): Promise<void> {
  ...
  const approved = await askYesNoImpl('[nana] Enjoying nana? Star it on GitHub? [Y/n] ');
  if (!approved) return;
  ...
  if (star.ok) {
    ...
    log('[nana] Thanks for the star!');
    return;
  }
  const warn = deps.warnFn ?? console.warn;
  warn(`[nana] Could not star repository automatically: ${star.error}`);
}
