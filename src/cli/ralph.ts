...
export const RALPH_HELP = `nana ralph - Launch Codex with ralph persistence mode active

Usage:
  nana ralph [task text...]
  nana ralph --prd "<task text>"
  nana ralph [ralph-options] [codex-args...] [task text...]
...
PRD mode:
  Ralph initializes persistence artifacts in .nana/ so PRD and progress
  state can survive across Codex sessions. Provide task text either as
  positional words or with --prd.

Common patterns:
  nana ralph "Fix flaky notify-hook tests"
  nana ralph --prd "Ship release checklist automation"
  nana ralph --model gpt-5 "Refactor state hydration"
  nana ralph -- --task-with-leading-dash
`;

const VALUE_TAKING_FLAGS = new Set(['--model', '--provider', '--config', '-c', '-i', '--images-dir']);
const RALPH_NANA_FLAGS = new Set(['--prd', '--no-deslop']);
const RALPH_APPEND_ENV = 'NANA_RALPH_APPEND_INSTRUCTIONS_FILE';
...
export function filterRalphCodexArgs(args: readonly string[]): string[] {
  ...
  for (const token of args) {
    if (RALPH_NANA_FLAGS.has(token.toLowerCase())) continue;
    filtered.push(token);
  }
  return filtered;
}
...
export function buildRalphAppendInstructions(
  ...
  return [
    ...
    'You are in NANA Ralph persistence mode.',
    `Primary task: ${task}`,
    'Parallelism guidance:',
    '- Prefer Codex native subagents for independent parallel subtasks.',
    '- Treat `.nana/state/subagent-tracking.json` as the native subagent activity ledger for this session.',
    ...
    options.noDeslop
      ...
      : `- Step 7.5 must run nana:ai-slop-cleaner in standard mode on changed files only, using the repo-relative paths listed in \`${options.changedFilesPath}\`.`,
    options.noDeslop
...
async function writeRalphSessionFiles(
  ...
  await writeFile(
    instructionsPath,
    `${buildRalphAppendInstructions(task, { changedFilesPath: '.nana/ralph/changed-files.txt', noDeslop: options.noDeslop, approvedHint: options.approvedHint ?? null })}\n`,
  );
  return { instructionsPath, changedFilesPath: '.nana/ralph/changed-files.txt' };
}
...
export async function ralphCommand(args: string[]): Promise<void> {
  ...
  await updateModeState('ralph', {
    ...
    native_subagent_tracking_path: '.nana/state/subagent-tracking.json',
