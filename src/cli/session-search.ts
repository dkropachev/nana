...
const HELP = `nana session - Search prior local session history
...
Examples:
  nana session search "worker inbox path"
  nana session search all_workers_idle --since 7d --limit 5
  nana session search "team api" --project current --json
`;
