...
function runNana(cwd: string, ...args: string[]) {
  return spawnSync(process.execPath, [nanaBin, ...args], {
    cwd,
    encoding: 'utf-8',
  });
}
...
describe('CLI session-scoped state parity', () => {
  it('status and cancel include session-scoped states', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-cli-session-scope-'));
    try {
      await mkdir(join(wd, '.nana', 'state'), { recursive: true });
      await writeFile(join(wd, '.nana', 'state', 'session.json'), JSON.stringify({ session_id: 'sess1' }));
      const scopedDir = join(wd, '.nana', 'state', 'sessions', 'sess1');
      ...
      const statusResult = runNana(wd, 'status');
      ...
      const cancelResult = runNana(wd, 'cancel');
  ...
  it('cancels linked ultrawork when Ralph is active', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-cli-ralph-link-'));
    try {
  ...
  it('does not mutate unrelated sessions when cancelling current session mode', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-cli-cross-session-'));
    try {
      const stateDir = join(wd, '.nana', 'state');
