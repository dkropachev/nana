...
describe('nana resume', () => {
  it('forwards --last to codex resume through the normal launch wrapper', async () => {
    ...
    try {
      ...
      const result = runNana(wd, ['resume', '--last'], {
        HOME: home,
  ...
  it('passes resume --help through to codex instead of printing top-level nana help', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-resume-cli-'));
    try {
      const home = join(wd, 'home');
      const fakeBin = join(wd, 'bin');
      ...
      const result = runNana(wd, ['resume', '--help'], {
        HOME: home,
        PATH: `${fakeBin}:/usr/bin:/bin`,
        NANA_AUTO_UPDATE: '0',
        NANA_NOTIFY_FALLBACK: '0',
