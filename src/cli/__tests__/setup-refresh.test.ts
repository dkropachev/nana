...
describe("nana setup refresh summary and dry-run behavior", () => {
  ...
  it("prints per-category summary and verbose changed-file detail", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-setup-refresh-"));
    try {
  ...
  it("does not overwrite or create backups during dry-run", async () => {
    ...
    try {
      ...
      assert.equal(existsSync(join(wd, ".nana", "backups", "setup")), false);
      assert.match(output, /skills: updated=/);
  ...
  it("creates .gitignore with a .nana/ entry during project-scoped setup", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-setup-refresh-"));
    try {
      await runSetupInTempDir(wd, { scope: "project" });

      assert.equal(existsSync(join(wd, ".nana", "state")), true);
      assert.equal(await readFile(join(wd, ".gitignore"), "utf-8"), ".nana/\n");
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

  it("appends .nana/ to an existing project .gitignore without duplicating it", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-setup-refresh-"));
    try {
      ...
      assert.equal(gitignore.match(/^\.nana\/$/gm)?.length ?? 0, 1);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it("creates backup files under the scope-specific setup backup root when refreshing modified managed files", async () => {
    ...
    try {
      ...
      const backupsRoot = join(wd, ".nana", "backups", "setup");
  ...
  it("skips NANA-managed [tui] writes for Codex CLI >= 0.107.0 and preserves an existing [tui] table", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-setup-refresh-"));
    try {
      await mkdir(join(wd, ".nana", "state"), { recursive: true });
      await mkdir(join(wd, ".codex"), { recursive: true });
      ...
      assert.match(
        output,
        /Codex CLI >= 0\.107\.0 manages \[tui\]; NANA left that section untouched\./,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it("keeps NANA-managed [tui] writes for older Codex CLI versions", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-setup-refresh-"));
    try {
      await mkdir(join(wd, ".nana", "state"), { recursive: true });
  ...
  it("syncs shared MCP registry entries into config.toml during setup", async () => {
    ...
    try {
      ...
      const config = await readFile(join(wd, ".codex", "config.toml"), "utf-8");
      assert.match(config, /nana \(NANA\) Shared MCP Registry Sync/);
  ...
  it("ignores legacy ~/.omc/mcp-registry.json during setup unless candidates are passed explicitly", async () => {
    ...
    try {
      ...
      assert.match(output, /move it to .*\.nana\/mcp-registry\.json/i);
    } finally {
      if (typeof previousHome === "string") process.env.HOME = previousHome;
      else delete process.env.HOME;
      if (typeof previousCodexHome === "string") process.env.CODEX_HOME = previousCodexHome;
