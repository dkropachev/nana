...
import { resolveGithubRunIdForTargetUrl } from "../github.js";
...
describe("resolveNotifyTempContract", () => {
  ...
  it("activates from NANA_NOTIFY_TEMP=1 env parity", () => {
...
describe("buildNotifyFallbackWatcherEnv", () => {
  it("enables watcher authority and propagates CODEX_HOME override when requested", () => {
    const env = buildNotifyFallbackWatcherEnv(
      { HOME: "/tmp/home", NANA_HUD_AUTHORITY: "0", TMUX: "sock,1,0", TMUX_PANE: "%2" },
      { codexHomeOverride: "/tmp/codex-home", enableAuthority: true },
    );
    assert.equal(env.NANA_HUD_AUTHORITY, "1");
  ...
  it("disables watcher authority explicitly when not requested", () => {
    const env = buildNotifyFallbackWatcherEnv(
      { HOME: "/tmp/home", NANA_HUD_AUTHORITY: "1", TMUX: "sock,1,0", TMUX_PANE: "%3" },
      { enableAuthority: false },
    );
    assert.equal(env.NANA_HUD_AUTHORITY, "0");
    assert.equal(env.HOME, "/tmp/home");
    assert.equal(env.TMUX, undefined);
    assert.equal(env.TMUX_PANE, undefined);
  });
...
describe("resolveWorkerSparkModel", () => {
  ...
  it("reads low-complexity team model from config when codexHomeOverride is provided", async () => {
    const codexHome = await mkdtemp(join(tmpdir(), "nana-codex-home-"));
    try {
      await writeFile(
        join(codexHome, ".nana-config.json"),
...
describe("resolveCliInvocation", () => {
  it("resolves reflect to reflect command", () => {
    assert.deepEqual(
      resolveCliInvocation(["reflect", "--prompt", "find", "auth"]),
      {
        command: "reflect",
        launchArgs: [],
      },
  ...
  it("routes legacy explore invocations to reflect internally", () => {
    assert.deepEqual(
  ...
  it("resolves implement to implement command and forwards trailing args", () => {
    assert.deepEqual(
      resolveCliInvocation(["implement", "https://github.com/acme/widget/issues/42", "--create-pr"]),
      {
        command: "implement",
        launchArgs: ["https://github.com/acme/widget/issues/42", "--create-pr"],
      },
    );
  });

  it("resolves investigate to investigate command and forwards trailing args", () => {
    assert.deepEqual(
      resolveCliInvocation(["investigate", "https://github.com/acme/widget/issues/42"]),
      {
        command: "investigate",
        launchArgs: ["https://github.com/acme/widget/issues/42"],
      },
    );
  });

  it("resolves sync to sync command and forwards trailing args", () => {
    assert.deepEqual(resolveCliInvocation(["sync", "--last"]), {
      command: "sync",
      launchArgs: ["--last"],
    });
  });

  it("resolves issue to issue command", () => {
    assert.deepEqual(resolveCliInvocation(["issue", "implement", "https://github.com/acme/widget/issues/42"]), {
      command: "issue",
      launchArgs: [],
    });
  });
...
describe("project launch scope helpers", () => {
  it("reads persisted setup scope when valid", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-launch-scope-"));
    try {
      await mkdir(join(wd, ".nana"), { recursive: true });
      await writeFile(
        join(wd, ".nana", "setup-scope.json"),
  ...
  it("ignores malformed persisted setup scope", async () => {
    ...
    try {
      await mkdir(join(wd, ".nana"), { recursive: true });
      await writeFile(join(wd, ".nana", "setup-scope.json"), "{not-json");
  ...
  it("uses NANA-managed user CODEX_HOME by default when scope is not project", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-launch-scope-"));
    try {
      assert.equal(resolveCodexHomeForLaunch(wd, {}), defaultUserCodexHome());
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
...
describe("credential bootstrap helpers", () => {
  it("imports legacy ~/.codex/auth.json into the resolved NANA-managed home", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-auth-import-"));
    const home = join(wd, "home");
    const cwd = join(wd, "project");
    try {
      await mkdir(join(home, ".codex"), { recursive: true });
      await mkdir(cwd, { recursive: true });
      await writeFile(join(home, ".codex", "auth.json"), '{"token":"legacy"}\n');

      const result = await importLegacyCodexAuth(cwd, { HOME: home });

      assert.equal(result.imported, true);
      assert.equal(
        await readFile(join(defaultUserCodexHome(home), "auth.json"), "utf-8"),
        '{"token":"legacy"}\n',
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
  ...
  it("does not overwrite existing target auth during automatic bootstrap", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-auth-bootstrap-"));
    const home = join(wd, "home");
    const cwd = join(wd, "project");
    try {
      await mkdir(join(home, ".codex"), { recursive: true });
      await mkdir(defaultUserCodexHome(home), { recursive: true });
      await mkdir(cwd, { recursive: true });
      await writeFile(join(home, ".codex", "auth.json"), '{"token":"legacy"}\n');
      await writeFile(join(defaultUserCodexHome(home), "auth.json"), '{"token":"managed"}\n');

      const result = await bootstrapResolvedCodexAuth(cwd, { HOME: home });
...
describe("GitHub sync alias helpers", () => {
  it("resolves the latest managed run id for a specific target URL", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-sync-target-"));
    const home = join(wd, "home");
    const repoRoot = join(home, ".nana", "repos", "dkropachev", "alternator-client-java");
    const issueRunDir = join(repoRoot, "runs", "gh-issue-1");
    const prRunDir = join(repoRoot, "runs", "gh-pr-6");
    try {
      await mkdir(issueRunDir, { recursive: true });
      await mkdir(prRunDir, { recursive: true });
      await writeFile(join(issueRunDir, "manifest.json"), JSON.stringify({
        version: 3,
        run_id: "gh-issue-1",
        created_at: "2026-04-07T10:00:00.000Z",
        updated_at: "2026-04-07T10:10:00.000Z",
        repo_slug: "dkropachev/alternator-client-java",
        repo_owner: "dkropachev",
        repo_name: "alternator-client-java",
        managed_repo_root: repoRoot,
        source_path: join(repoRoot, "source"),
        sandbox_id: "issue-1",
        sandbox_path: join(repoRoot, "sandboxes", "issue-1"),
        sandbox_repo_path: join(repoRoot, "sandboxes", "issue-1", "repo"),
        ...
        target_kind: "issue",
        target_number: 1,
        target_title: "Issue one",
        target_url: "https://github.com/dkropachev/alternator-client-java/issues/1",
        target_state: "open",
      ...
      await writeFile(join(prRunDir, "manifest.json"), JSON.stringify({
        version: 3,
        run_id: "gh-pr-6",
        ...
        source_path: join(repoRoot, "source"),
        sandbox_id: "pr-6",
        sandbox_path: join(repoRoot, "sandboxes", "pr-6"),
        sandbox_repo_path: join(repoRoot, "sandboxes", "pr-6", "repo"),
        considerations_active: [],
        role_layout: "split",
        consideration_pipeline: [],
        lane_prompt_artifacts: [],
        ...
        target_number: 6,
        target_title: "PR six",
        target_url: "https://github.com/dkropachev/alternator-client-java/pull/6",
        target_state: "open",
        review_reviewer: "dkropachev",
        api_base_url: "https://api.github.com",
        default_branch: "main",
      ...
      const issueRun = await resolveGithubRunIdForTargetUrl(
        "https://github.com/dkropachev/alternator-client-java/issues/1",
        { env: { HOME: home } as NodeJS.ProcessEnv, homeDir: home },
      );
      const prRun = await resolveGithubRunIdForTargetUrl(
        "https://github.com/dkropachev/alternator-client-java/pull/6",
        { env: { HOME: home } as NodeJS.ProcessEnv, homeDir: home },
      );

      assert.equal(issueRun, "gh-issue-1");
      assert.equal(prRun, "gh-pr-6");
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

  it("falls back from a PR URL to the linked issue-managed sandbox run", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-sync-pr-symlink-"));
    const home = join(wd, "home");
    const repoRoot = join(home, ".nana", "repos", "dkropachev", "alternator-client-java");
    const runsDir = join(repoRoot, "runs");
    const issueRunDir = join(runsDir, "gh-issue-1");
    const sandboxesDir = join(repoRoot, "sandboxes");
    const issueSandboxDir = join(sandboxesDir, "issue-1-pr-abc123");
    const prSandboxLink = join(sandboxesDir, "pr-6");
    try {
      await mkdir(issueRunDir, { recursive: true });
      await mkdir(join(issueSandboxDir, ".nana"), { recursive: true });
      await writeFile(join(issueRunDir, "manifest.json"), JSON.stringify({
        version: 3,
        run_id: "gh-issue-1",
        created_at: "2026-04-07T10:00:00.000Z",
        ...
        managed_repo_root: repoRoot,
        source_path: join(repoRoot, "source"),
        sandbox_id: "issue-1-pr-abc123",
        sandbox_path: issueSandboxDir,
        sandbox_repo_path: join(issueSandboxDir, "repo"),
        considerations_active: [],
        role_layout: "split",
        consideration_pipeline: [],
        lane_prompt_artifacts: [],
      ...
      await writeFile(join(issueSandboxDir, ".nana", "sandbox.json"), JSON.stringify({
        version: 1,
        sandbox_id: "issue-1-pr-abc123",
        repo_slug: "dkropachev/alternator-client-java",
        repo_name: "alternator-client-java",
        sandbox_path: issueSandboxDir,
        repo_checkout_path: join(issueSandboxDir, "repo"),
        branch_name: "nana/issue-1/issue-1-pr-abc123",
      ...
      await symlink(issueSandboxDir, prSandboxLink);
...
describe("tmux HUD pane helpers", () => {
  it("findHudWatchPaneIds detects stale HUD watch panes and excludes current pane", () => {
    const panes = parseTmuxPaneSnapshot(
      [
        "%1\tzsh\tzsh",
        "%2\tnode\tnode /tmp/bin/nana.js hud --watch",
        "%3\tnode\tnode /tmp/bin/nana.js hud --watch",
...
describe("detached tmux new-session sequencing", () => {
  it("buildDetachedSessionBootstrapSteps uses shared HUD height and split-capture ordering", () => {
    ...
    assert.equal(steps[1]?.args[6], "nana-demo");
    ...
    assert.equal(
      steps[0]?.args.includes('NANA_NOTIFY_TEMP_CONTRACT={\"active\":true}'),
      true,
    );
  });
    ...
    assert.equal(
      newSession!.args.includes("-e") &&
        newSession!.args.some((arg) =>
          arg.startsWith("NANA_NOTIFY_TEMP_CONTRACT="),
        ),
      true,
    );
  });

  it("buildDetachedSessionBootstrapSteps forwards NANA_SESSION_ID to detached tmux session", () => {
    const steps = buildDetachedSessionBootstrapSteps(
      "nana-demo",
      "/tmp/project",
      "'codex' '--model' 'gpt-5'",
    ...
    assert.equal(
      newSession!.args.includes("-e") &&
        newSession!.args.some((arg) => arg === "NANA_SESSION_ID=sess-detached-managed"),
      true,
    );
  });
  ...
  it("buildDetachedSessionBootstrapSteps kills detached tmux session on exit and interrupt", () => {
    ...
    assert.match(leaderCmd!, /^'\/usr\/bin\/zsh' -c '/);
    ...
    assert.match(leaderCmd!, /"nana-demo"/);
  ...
  it("buildDetachedSessionRollbackSteps unregisters hooks before killing session", () => {
    const steps = buildDetachedSessionRollbackSteps(
      "nana-demo",
      "nana-demo:0",
      "nana_resize_launch_demo_0_12",
      "nana_attached_launch_demo_0_12",
    );
    assert.deepEqual(
      steps.map((step) => step.name),
      [
    ...
    assert.equal(steps[0]?.args[3], "nana-demo:0");
    assert.match(steps[0]?.args[4] ?? "", /^client-attached\[\d+\]$/);
    assert.match(steps[1]?.args[4] ?? "", /^client-resized\[\d+\]$/);
    assert.deepEqual(steps[2]?.args, ["kill-session", "-t", "nana-demo"]);
  });
...
describe("buildTmuxSessionName", () => {
  it("uses detached fallback quietly outside git repos", () => {
    const name = buildTmuxSessionName(
      "/tmp/My Repo",
      "nana-1770992424158-abc123",
    );
    assert.equal(name, "nana-my-repo-detached-1770992424158-abc123");
  });

  it("sanitizes invalid characters", () => {
    const name = buildTmuxSessionName("/tmp/@#$", "nana-+++");
    assert.match(
      name,
      /^nana-(unknown|[a-z0-9-]+)-[a-z0-9-]+-(unknown|[a-z0-9-]+)$/,
    );
    assert.equal(name.includes("_"), false);
    assert.equal(name.includes(" "), false);
  });

  it("includes repo name when cwd is inside .nana-worktrees", () => {
    const name = buildTmuxSessionName(
      "/home/user/my-repo.nana-worktrees/launch-feature-x",
      "nana-123-abc",
    );
    assert.match(name, /^nana-my-repo-launch-feature-x-/);
  });

  it("includes repo name for detached worktree paths", () => {
    const name = buildTmuxSessionName(
      "/projects/cool-project.nana-worktrees/launch-detached",
      "nana-456-def",
    );
    assert.match(name, /^nana-cool-project-launch-detached-/);
  });

  it("includes repo name when cwd is inside .nana/worktrees", () => {
    const name = buildTmuxSessionName(
      "/home/user/my-repo/.nana/worktrees/autoresearch-demo",
      "nana-789-ghi",
    );
    assert.match(name, /^nana-my-repo-autoresearch-demo-/);
  });
});
...
describe("injectModelInstructionsBypassArgs", () => {
  ...
  it("does not append when bypass is disabled via env", () => {
    const args = injectModelInstructionsBypassArgs(
      "/tmp/my-project",
      ["--model", "gpt-5"],
      { NANA_BYPASS_DEFAULT_SYSTEM_PROMPT: "0" },
    );
  ...
  it("respects NANA_MODEL_INSTRUCTIONS_FILE env override", () => {
    const args = injectModelInstructionsBypassArgs("/tmp/my-project", [], {
      NANA_MODEL_INSTRUCTIONS_FILE: "/tmp/alt instructions.md",
    });
    assert.deepEqual(args, [
      "-c",
  ...
  it("uses session-scoped default model_instructions_file when provided", () => {
    const args = injectModelInstructionsBypassArgs(
      ...
      ["--model", "gpt-5"],
      {},
      "/tmp/my-project/.nana/state/sessions/session-1/AGENTS.md",
    );
    assert.deepEqual(args, [
      "--model",
      "gpt-5",
      "-c",
      'model_instructions_file="/tmp/my-project/.nana/state/sessions/session-1/AGENTS.md"',
    ]);
  });
});
