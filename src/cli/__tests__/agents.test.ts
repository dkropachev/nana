...
describe('nana agents', () => {
  it('lists project and user native agents with name, description, and model', async () => {
    ...
    try {
      ...
      const result = runNana(wd, ['agents', 'list'], {
        HOME: home,
  ...
  it('adds a scaffolded agent TOML file with required fields and commented optional fields', async () => {
    ...
    try {
      ...
      const result = runNana(wd, ['agents', 'add', 'my-helper', '--scope', 'project'], {
        HOME: home,
        CODEX_HOME: join(home, '.codex'),
      });
      if (shouldSkipForSpawnPermissions(result.error)) return;
  ...
  it('edits an existing agent via $EDITOR and removes it with --force', async () => {
    ...
    try {
      ...
      const editResult = runNana(wd, ['agents', 'edit', 'editor-test', '--scope', 'project'], {
        HOME: home,
        CODEX_HOME: join(home, '.codex'),
        EDITOR: editorScript,
      });
      ...
      const removeResult = runNana(wd, ['agents', 'remove', 'editor-test', '--scope', 'project', '--force'], {
        HOME: home,
        CODEX_HOME: join(home, '.codex'),
      });
  ...
  it('fails with clear guidance when remove runs in non-interactive mode without --force', async () => {
    ...
    try {
      ...
      const result = runNana(wd, ['agents', 'remove', 'non-interactive', '--scope', 'project'], {
        HOME: home,
        CODEX_HOME: join(home, '.codex'),
      });
      if (shouldSkipForSpawnPermissions(result.error)) return;
