...
describe("nana setup scope behavior", () => {
  it("accepts --scope project form", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-setup-scope-"));
    try {
      const home = join(wd, "home");
      await mkdir(home, { recursive: true });
      const bySeparateArg = runNana(
        wd,
      ...
      const byEqualsArg = runNana(wd, ["setup", "--dry-run", "--scope=user"], {
        HOME: home,
      });
  ...
  it("uses persisted setup scope when --scope is omitted", async () => {
    ...
    try {
      ...
      await mkdir(home, { recursive: true });
      await writeFile(
        join(nanaDir, "setup-scope.json"),
        JSON.stringify({ scope: "project" }),
      );

      const res = runNana(wd, ["setup", "--dry-run"], { HOME: home });
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);
      assert.match(
        res.stdout,
        /Using setup scope: project \(from \.nana\/setup-scope\.json\)/,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it("doctor respects persisted project setup scope paths", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-doctor-scope-"));
    try {
      const home = join(wd, "home");
      await mkdir(home, { recursive: true });
      ...
      const res = runNana(wd, ["doctor"], { HOME: home });
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);
      assert.match(
        res.stdout,
        /Resolved setup scope: project \(from \.nana\/setup-scope\.json\)/,
      );
      assert.match(
        res.stdout,
        new RegExp(
  ...
  it("does not persist setup scope on --dry-run", async () => {
    ...
    try {
      const home = join(wd, "home");
      await mkdir(home, { recursive: true });
      const res = runNana(wd, ["setup", "--scope", "project", "--dry-run"], {
        HOME: home,
      });
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);
      assert.equal(existsSync(join(wd, ".nana", "setup-scope.json")), false);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it("project scope writes prompts/skills/config/native-agents under cwd", async () => {
    ...
    try {
      const home = join(wd, "home");
      await mkdir(home, { recursive: true });
      const res = runNana(wd, ["setup", "--scope", "project"], { HOME: home });
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);
      ...
      const scopeFile = join(wd, ".nana", "setup-scope.json");
      ...
      assert.equal(
        existsSync(join(localSkills, "nana-setup", "SKILL.md")),
        true,
      );
      assert.equal(
      ...
      assert.match(configToml, /^USE_NANA_EXPLORE_CMD = "1"$/m);
  ...
  it("defaults to user scope in non-interactive runs when no scope is persisted", async () => {
    ...
    try {
      ...
      const res = runNana(wd, ["setup"], { HOME: home });
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);
      ...
      assert.equal(existsSync(join(wd, ".nana", "setup-scope.json")), true);
      const persistedScope = JSON.parse(
        await readFile(join(wd, ".nana", "setup-scope.json"), "utf-8"),
      ) as { scope: string };
  ...
  it("doctor does not warn about missing project AGENTS.md for user scope", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-doctor-user-scope-"));
    try {
      const home = join(wd, "home");
      ...
      assert.match(
        res.stdout,
        /Resolved setup scope: user \(from \.nana\/setup-scope\.json\)/,
      );
      assert.match(
        res.stdout,
  ...
  it("still refreshes existing AGENTS.md with --force", async () => {
    ...
    try {
      ...
      const res = runNana(wd, ["setup", "--scope=project", "--force"], {
        HOME: home,
      });
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);
