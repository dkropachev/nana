...
function runNana(cwd: string, argv: string[], envOverrides: Record<string, string> = {}) {
...
describe('nana session search', () => {
  it('prints structured JSON results for matching transcripts', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-session-search-cli-'));
    ...
    try {
      ...
      const result = runNana(cwd, ['session', 'search', 'team api', '--project', 'current', '--json'], {
