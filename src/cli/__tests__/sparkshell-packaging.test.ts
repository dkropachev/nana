...
describe('sparkshell packaging scaffold', () => {
  it('registers native helper scripts but keeps staged native artifacts out of npm releases', () => {
    ...
    const stagedRoot = mkdtempSync(join(tmpdir(), 'nana-sparkshell-stage-'));
    ...
    assert.match(testScriptSource, /'crates', 'nana-sparkshell', 'Cargo\.toml'/);
    assert.doesNotMatch(testScriptSource, /'native', 'nana-sparkshell', 'Cargo\.toml'/);
    ...
    try {
      ...
      const buildResult = spawnSync(process.execPath, [buildScriptPath], {
        ...
        env: {
          ...process.env,
          NANA_SPARKSHELL_MANIFEST: join(process.cwd(), 'crates', 'nana-sparkshell', 'Cargo.toml'),
          NANA_SPARKSHELL_STAGE_DIR: stagedRoot,
        },
      });
