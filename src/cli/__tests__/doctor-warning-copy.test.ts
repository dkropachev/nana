...
describe('nana doctor onboarding warning copy', () => {
  it('explains first-setup expectation for config and MCP onboarding warnings', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-doctor-copy-'));
    try {
      ...
      await writeFile(
        ...
        `
[mcp_servers.non_nana]
command = "node"
`.trimStart(),
      );

      const res = runNana(wd, ['doctor'], {
        HOME: home,
      ...
      assert.match(
        res.stdout,
        /Config: config\.toml exists but no NANA entries yet \(expected before first setup; run "nana setup --force" once\)/,
      );
      assert.match(
        res.stdout,
        /MCP Servers: 1 servers but no NANA servers yet \(expected before first setup; run "nana setup --force" once\)/,
      );
    } finally {
  ...
  it('warns when explore harness sources are packaged but cargo is unavailable', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-doctor-explore-copy-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        ...
        assert.match(
          res.stdout,
          /Explore Harness: (Rust harness sources are packaged, but no compatible packaged prebuilt or cargo was found \(install Rust or set NANA_EXPLORE_BIN for nana explore\)|not ready \(no packaged binary, NANA_EXPLORE_BIN, or cargo toolchain\))/,
        );
      });
    } finally {
      await rm(wd, { recursive: true, force: true });
  ...
  it('passes explore harness check when a packaged native binary is present even without cargo', async () => {
    await withPackagedExploreHarnessLock(async () => {
      const wd = await mkdtemp(join(tmpdir(), 'nana-doctor-explore-binary-'));
      try {
        const home = join(wd, 'home');
        const codexDir = join(home, '.codex');
        const fakeBin = join(wd, 'bin');
        const packageBinDir = join(process.cwd(), 'bin');
        const packagedBinary = join(packageBinDir, process.platform === 'win32' ? 'nana-explore-harness.exe' : 'nana-explore-harness');
        const packagedMeta = join(packageBinDir, 'nana-explore-harness.meta.json');
        ...
        await writeFile(packagedMeta, JSON.stringify({ binaryName: process.platform === 'win32' ? 'nana-explore-harness.exe' : 'nana-explore-harness', platform: process.platform, arch: process.arch }));
  ...
  it('warns when explore routing is explicitly disabled in config.toml', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-doctor-explore-routing-'));
    try {
      const home = join(wd, 'home');
      const codexDir = join(home, '.codex');
      await mkdir(codexDir, { recursive: true });
...
`.trimStart(),
      ...
      assert.match(
        res.stdout,
        /Explore routing: disabled in config\.toml \[env\]; set USE_NANA_EXPLORE_CMD = "1" to restore default explore-first routing/,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('warns when canonical and legacy skill roots overlap', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-doctor-skill-overlap-'));
    try {
      const home = join(wd, 'home');
      const codexDir = join(home, '.codex');
  ...
  it('passes when legacy skill root is a link to the canonical skills directory', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-doctor-skill-link-'));
    try {
      const home = join(wd, 'home');
      const codexDir = join(home, '.codex');
