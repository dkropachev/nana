...
describe('maybeCheckAndPromptUpdate', () => {
  ...
  it('runs setup refresh after a successful auto-update', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-update-'));
  ...
  it('does not refresh setup when the global update fails', async () => {
    ...
    try {
      ...
      assert.equal(setupCalls, 0);
      assert.match(logs.join('\n'), /Update failed\. Run manually: npm install -g nana@latest/);
    } finally {
      console.log = originalLog;
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('skips update checks entirely when running from a local checkout', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-update-'));
    let fetchCalls = 0;
    let promptCalls = 0;
