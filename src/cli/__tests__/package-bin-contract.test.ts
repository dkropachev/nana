...
describe('package bin contract', () => {
  it('declares nana with an explicit relative bin path and avoids packaging platform-specific native binaries', () => {
    ...
    assert.deepEqual(pkg.bin, { nana: 'dist/cli/nana.js' });
    assert.equal(pkg.scripts?.['build:explore'], 'cargo build -p nana-explore-harness');
    ...
    assert.equal(pkg.scripts?.['test:explore'], 'cargo test -p nana-explore-harness && node --test dist/cli/__tests__/explore.test.js dist/hooks/__tests__/explore-routing.test.js dist/hooks/__tests__/explore-sparkshell-guidance-contract.test.js');
    ...
    const binPath = join(process.cwd(), 'dist', 'cli', 'nana.js');
    ...
    const binEntry = results[0]?.files?.find((file) => file.path === 'dist/cli/nana.js');
    assert.ok(binEntry, 'expected npm pack output to include dist/cli/nana.js');

    const packagedHarnessPath = process.platform === 'win32' ? 'bin/nana-explore-harness.exe' : 'bin/nana-explore-harness';
    const packagedHarnessEntry = results[0]?.files?.find((file) => file.path === packagedHarnessPath);
    const packagedHarnessMetaEntry = results[0]?.files?.find((file) => file.path === 'bin/nana-explore-harness.meta.json');
    ...
    const crateManifestEntry = results[0]?.files?.find((file) => file.path === 'crates/nana-explore/Cargo.toml');
    const crateMainEntry = results[0]?.files?.find((file) => file.path === 'crates/nana-explore/src/main.rs');
    ...
    assert.ok(crateManifestEntry, 'expected npm pack output to include crates/nana-explore/Cargo.toml');
    assert.ok(crateMainEntry, 'expected npm pack output to include crates/nana-explore/src/main.rs');
  });
});
