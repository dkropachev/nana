...
describe('nana ask', () => {
  it('script usage documents provider-specific long flags from CLI help', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-ask-usage-'));
    try {
  ...
  it('preserves child stdout/stderr and exact non-zero exit code', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-ask-contract-'));
    try {
      const res = runNana(wd, ['ask', 'claude', 'pass-through'], {
  ...
  it('resolves relative advisor override path from package root even on non-root cwd', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-ask-relative-'));
    try {
      const res = runNana(wd, ['ask', 'gemini', 'relative-check'], {
        NANA_ASK_ADVISOR_SCRIPT: 'dist/scripts/fixtures/ask-advisor-stub.js',
        NANA_ASK_STUB_STDOUT: 'relative-override-ok\n',
        NANA_ASK_STUB_EXIT_CODE: '0',
      });
      if (shouldSkipForSpawnPermissions(res.error)) return;
  ...
  it('uses package-root advisor script path from non-package cwd and still writes artifact', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-ask-nonroot-'));
    try {
      ...
      const res = runNana(wd, ['ask', 'claude', 'non-root-default'], {
      ...
      assert.ok(artifactPath.startsWith(join(wd, '.nana', 'artifacts', 'claude-')));
  ...
  it('supports claude --print and gemini --prompt end-to-end through nana ask', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-ask-provider-flags-'));
    try {
      const fakeBin = join(wd, 'bin');
      await mkdir(fakeBin, { recursive: true });
      ...
      const claudeRes = runNana(wd, ['ask', 'claude', '--print', 'claude-long-flag'], env);
      ...
      const geminiRes = runNana(wd, ['ask', 'gemini', '--prompt', 'gemini-long-flag'], env);
  ...
  it('injects --agent-prompt content into final prompt while keeping Original task raw', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-ask-agent-prompt-'));
    try {
      const fakeBin = join(wd, 'bin');
  ...
  it('fails clearly when --agent-prompt role is missing from prompts directory', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-ask-agent-prompt-missing-'));
    try {
      const fakeBin = join(wd, 'bin');
      const codexHome = join(wd, '.codex-home');
      const promptsDir = join(codexHome, 'prompts');
