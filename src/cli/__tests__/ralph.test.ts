...
const approvedHint: ApprovedExecutionLaunchHint = {
  mode: 'ralph',
  command: 'nana ralph "Execute approved issue 1072 plan"',
  task: 'Execute approved issue 1072 plan',
  sourcePath: '.nana/plans/prd-issue-1072.md',
  testSpecPaths: ['.nana/plans/test-spec-issue-1072.md'],
  deepInterviewSpecPaths: ['.nana/specs/deep-interview-issue-1072.md'],
};
...
describe('ralph deslop launch wiring', () => {
  ...
  it('documents changed-files-only deslop guidance by default', () => {
    const instructions = buildRalphAppendInstructions('fix issue 920', {
      changedFilesPath: '.nana/ralph/changed-files.txt',
      noDeslop: false,
      approvedHint: null,
    });
    assert.match(instructions, /ai-slop-cleaner/i);
    assert.match(instructions, /changed files only/i);
    assert.match(instructions, /\.nana\/ralph\/changed-files\.txt/);
  ...
  it('includes approved plan and deep-interview handoff context when available', () => {
    ...
    assert.match(instructions, /approved plan: \.nana\/plans\/prd-issue-1072\.md/i);
    assert.match(instructions, /test specs: \.nana\/plans\/test-spec-issue-1072\.md/i);
    assert.match(instructions, /deep-interview specs: \.nana\/specs\/deep-interview-issue-1072\.md/i);
