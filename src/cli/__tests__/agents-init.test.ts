...
describe('nana agents-init', () => {
  it('creates a managed root AGENTS.md plus direct-child AGENTS.md files while skipping ignored directories', async () => {
    ...
    try {
      ...
      assert.match(rootAgents, /NANA:AGENTS-INIT:MANAGED/);
      ...
      assert.match(rootAgents, /# nana - Intelligent Multi-Agent Orchestration/);
  ...
  it('skips unmanaged AGENTS.md files by default but can adopt them with --force and a backup', async () => {
    ...
    try {
      ...
      assert.match(adopted, /NANA:AGENTS-INIT:MANAGED/);
      const backupRoot = join(wd, '.nana', 'backups', 'agents-init');
  ...
  it('protects project-root AGENTS.md during an active NANA session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-agents-init-'));
    try {
      ...
      await writeFile(join(wd, 'src', 'index.ts'), 'export const x = 1;\n');
      await writeFile(
        join(wd, '.nana', 'state', 'session.json'),
        JSON.stringify({
