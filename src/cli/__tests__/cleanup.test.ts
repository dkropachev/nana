...
const CURRENT_SESSION_PROCESSES: ProcessEntry[] = [
  ...
  { pid: 701, ppid: 700, command: 'node /repo/bin/nana.js cleanup --dry-run' },
  {
    pid: 710,
    ppid: 700,
    command: 'node /repo/nana/dist/mcp/state-server.js',
  },
  {
    pid: 800,
    ppid: 1,
    command: 'node /tmp/nana/dist/mcp/memory-server.js',
  },
  {
    pid: 810,
    ppid: 42,
  ...
  },
  {
    pid: 900,
    ppid: 1,
    command: 'node /tmp/not-nana/other-server.js',
  },
];

describe('findCleanupCandidates', () => {
  it('selects orphaned NANA MCP processes while preserving the current session tree', () => {
    assert.deepEqual(
...
describe('cleanupNanaMcpProcesses', () => {
  it('supports dry-run without sending signals', async () => {
    const lines: string[] = [];
    let signalCount = 0;

    const result = await cleanupNanaMcpProcesses(['--dry-run'], {
      currentPid: 701,
    ...
    assert.match(lines.join('\n'), /Dry run: would terminate 3 orphaned NANA MCP server process/);
    assert.match(lines.join('\n'), /PID 800/);
    assert.match(lines.join('\n'), /PID 810/);
  });
  ...
  it('sends SIGTERM, waits, and escalates with SIGKILL when needed', async () => {
    ...
    const result = await cleanupNanaMcpProcesses([], {
      currentPid: 701,
      listProcesses: () => [
    ...
    assert.match(lines.join('\n'), /Killed 2 orphaned NANA MCP server process\(es\) \(1 required SIGKILL\)\./);
  });
});

describe('cleanupStaleTmpDirectories', () => {
  const tmpEntries = [
    { name: 'nana-stale-a', isDirectory: () => true },
    { name: 'omc-stale-b', isDirectory: () => true },
    { name: 'nana-fresh', isDirectory: () => true },
    { name: 'nana-file', isDirectory: () => false },
  ...
  it('supports dry-run and reports stale matching directories older than one hour', async () => {
    ...
    assert.match(
      lines.join('\n'),
      /Dry run: would remove 2 stale NANA \/tmp directories:/,
    );
    assert.match(lines.join('\n'), /\/tmp\/omc-stale-b/);
    assert.match(lines.join('\n'), /\/tmp\/nana-stale-a/);
  ...
  it('removes only stale matching directories and returns the removed count', async () => {
    ...
    assert.deepEqual(removedPaths, ['/tmp/nana-stale-a', '/tmp/omc-stale-b']);
    assert.match(lines.join('\n'), /Removed stale \/tmp directory: \/tmp\/nana-stale-a/);
    assert.match(lines.join('\n'), /Removed stale \/tmp directory: \/tmp\/omc-stale-b/);
    assert.match(lines.join('\n'), /Removed 2 stale NANA \/tmp directories\./);
  });
});
