...
async function initRepo(): Promise<string> {
  const raw = await mkdtemp(join(tmpdir(), 'nana-autoresearch-test-'));
...
describe('nana autoresearch', () => {
  it('documents autoresearch in top-level help', async () => {
    ...
    try {
      ...
      assert.match(result.stdout, /nana autoresearch\s+Launch thin-supervisor autoresearch with keep\/discard\/reset parity/i);
    } finally {
  ...
  it('routes autoresearch --help to command-local help', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-autoresearch-local-help-'));
    try {
      const result = runNana(cwd, ['autoresearch', '--help']);
      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.match(result.stdout, /Usage:[\s\S]*nana autoresearch run <mission-dir>/i);
      assert.match(result.stdout, /nana autoresearch init/i);
  ...
  it('documents --resume in command-local help', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-autoresearch-resume-help-'));
    try {
      const result = runNana(cwd, ['autoresearch', '--help']);
      assert.equal(result.status, 0, result.stderr || result.stdout);
  ...
  it('fails fast when mission dir is missing', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-autoresearch-missing-arg-'));
    try {
      const result = runNana(cwd, ['autoresearch']);
      assert.notEqual(result.status, 0, result.stderr || result.stdout);
      assert.match(`${result.stderr}\n${result.stdout}`, /mission-dir|Usage:\s*nana autoresearch <mission-dir>/i);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('resolves guided deep-interview artifacts by seeded slug even when file mtimes predate launch timestamp', async () => {
    ...
    const fakeBin = await mkdtemp(join(tmpdir(), 'nana-autoresearch-deep-interview-mtime-bin-'));
    try {
      ...
      await writeFile(
        ...
        `#!/bin/sh
if [ "$1" = "exec" ]; then
  candidate_file=$(find "$NANA_TEST_REPO_ROOT/.nana/logs/autoresearch" -name candidate.json | head -n 1)
...
mkdir -p "$NANA_TEST_REPO_ROOT/.nana/specs/autoresearch-test-launch"
cat >"$NANA_TEST_REPO_ROOT/.nana/specs/deep-interview-autoresearch-test-launch.md" <<'EOF'
...
- launch
EOF
cat >"$NANA_TEST_REPO_ROOT/.nana/specs/autoresearch-test-launch/mission.md" <<'EOF'
# Mission

Investigate flaky onboarding behavior
EOF
cat >"$NANA_TEST_REPO_ROOT/.nana/specs/autoresearch-test-launch/sandbox.md" <<'EOF'
---
evaluator:
...
---
EOF
cat >"$NANA_TEST_REPO_ROOT/.nana/specs/autoresearch-test-launch/result.json" <<'EOF'
{
  "kind": "nana.autoresearch.deep-interview/v1",
  "compileTarget": {
  ...
  "draftArtifactPath": "${repo}/.nana/specs/deep-interview-autoresearch-test-launch.md",
  "missionArtifactPath": "${repo}/.nana/specs/autoresearch-test-launch/mission.md",
  "sandboxArtifactPath": "${repo}/.nana/specs/autoresearch-test-launch/sandbox.md",
  "launchReady": true,
  "blockedReasons": []
}
EOF
touch -t 202603180000 "$NANA_TEST_REPO_ROOT/.nana/specs/deep-interview-autoresearch-test-launch.md"
touch -t 202603180000 "$NANA_TEST_REPO_ROOT/.nana/specs/autoresearch-test-launch/mission.md"
touch -t 202603180000 "$NANA_TEST_REPO_ROOT/.nana/specs/autoresearch-test-launch/sandbox.md"
touch -t 202603180000 "$NANA_TEST_REPO_ROOT/.nana/specs/autoresearch-test-launch/result.json"
`,
        'utf-8',
      );
      execFileSync('chmod', ['+x', fakeCodexPath], { stdio: 'ignore' });

      const result = runNana(repo, ['autoresearch', '--topic', 'Investigate flaky onboarding behavior', '--evaluator', 'node scripts/eval.js', '--slug', 'test-launch'], {
        PATH: `${fakeBin}:${process.env.PATH || ''}`,
        NANA_TEST_REPO_ROOT: repo,
      });
      assert.equal(result.status, 0, result.stderr || result.stdout);
  ...
  it('launches interactive deep-interview intake, materializes mission files, and then prefers split-pane handoff', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'nana-autoresearch-deep-interview-bin-'));
    try {
...
mkdir -p "$NANA_TEST_REPO_ROOT/.nana/specs/deep-int"
mkdir -p "$NANA_TEST_REPO_ROOT/.nana/specs/autoresearch-test-launch"
cat >"$NANA_TEST_REPO_ROOT/.nana/specs/deep-interview-autoresearch-test-launch.md" <<'EOF'
# Deep Interview Autoresearch Draft — test-launch
...
`,
  ...
  it('uses split-window launch for explicit run inside tmux while preserving the interview pane', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'nana-autoresearch-run-split-bin-'));
    try {
...
case "$1" in
  ...
  display-message)
    ...
    ;;
  list-panes)
    printf '%%9\tzsh\tnana autoresearch\n'
    exit 0
    ;;
  split-window)
    last=""
...
`,
      ...
      const result = runNana(repo, ['autoresearch', 'run', missionDir, '--model', 'gpt-5'], {
        PATH: `${fakeBin}:${process.env.PATH || ''}`,
        NANA_TEST_REPO_ROOT: repo,
        TMUX: '/tmp/fake-tmux,12345,0',
        TMUX_PANE: '%9',
  ...
  it('falls back to foreground execution when tmux split-window fails', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'nana-autoresearch-run-fallback-bin-'));
    try {
      const missionDir = join(repo, 'missions', 'demo');
      await mkdir(missionDir, { recursive: true });
      await mkdir(join(repo, 'scripts'), { recursive: true });
...
`,
      ...
      const result = runNana(repo, ['autoresearch', 'run', missionDir], {
        PATH: `${fakeBin}:${process.env.PATH || ''}`,
        NANA_TEST_REPO_ROOT: repo,
        TMUX: '/tmp/fake-tmux,12345,0',
        TMUX_PANE: '%9',
      ...
      const logsRoot = join(repo, '.nana', 'logs', 'autoresearch');
  ...
  it('rejects mission directories outside a git repo', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-autoresearch-outside-git-'));
    try {
      await writeFile(join(cwd, 'mission.md'), '# Mission\n', 'utf-8');
      await writeFile(join(cwd, 'sandbox.md'), '---\nevaluator:\n  command: node eval.js\n---\n', 'utf-8');

      const result = runNana(cwd, ['autoresearch', cwd]);
      assert.notEqual(result.status, 0, result.stderr || result.stdout);
  ...
  it('rejects missing mission.md inside an in-repo mission dir', async () => {
    ...
    try {
      ...
      const result = runNana(repo, ['autoresearch', missionDir]);
      assert.notEqual(result.status, 0, result.stderr || result.stdout);
  ...
  it('rejects autoresearch launch when root ralph mode is already active', async () => {
    ...
    try {
      ...
      await mkdir(join(repo, '.nana', 'state'), { recursive: true });
      await writeFile(
        join(repo, '.nana', 'state', 'ralph-state.json'),
        JSON.stringify({
          active: true,
          mode: 'ralph',
          iteration: 0,
      ...
      const worktreesRoot = join(repo, '.nana', 'worktrees');
  ...
  it('launches codex exec for autoresearch turns without shelling out to cat', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'nana-autoresearch-fake-bin-'));
    try {
      const missionDir = join(repo, 'missions', 'demo');
      await mkdir(missionDir, { recursive: true });
      await mkdir(join(repo, 'scripts'), { recursive: true });
...
`,
      ...
      const result = runNana(
        repo,
        ['autoresearch', missionDir, '--dangerously-bypass-approvals-and-sandbox'],
        { PATH: `${fakeBin}:${process.env.PATH || ''}`, NANA_TEST_REPO_ROOT: repo },
      );
  ...
  it('stops after repeated noop turns', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'nana-autoresearch-noop-bin-'));
    try {
      const missionDir = join(repo, 'missions', 'demo');
      await mkdir(missionDir, { recursive: true });
      await mkdir(join(repo, 'scripts'), { recursive: true });
...
`,
      ...
      const state = JSON.parse(await readFile(join(repo, '.nana', 'state', 'autoresearch-state.json'), 'utf-8')) as {
        active: boolean;
      };
      ...
      const resumeResult = runNana(repo, ['autoresearch', '--resume', runId]);
