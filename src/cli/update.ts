/**
 * Launch-time update checks for nana.
 * Non-fatal and throttled; can be disabled via NANA_AUTO_UPDATE=0.
 */
...
function updateStatePath(cwd: string): string {
  return join(cwd, '.nana', 'state', 'update-check.json');
}
...
function isLocalCheckout(): boolean {
...
interface UpdateDependencies {
  ...
  setup: typeof setup;
  isLocalCheckout: typeof isLocalCheckout;
}
...
export async function maybeCheckAndPromptUpdate(
  ...
  if (process.env.NANA_AUTO_UPDATE === '0') return;
  if (!process.stdin.isTTY || !process.stdout.isTTY) return;
  if (updateDependencies.isLocalCheckout()) return;
  ...
  const approved = await updateDependencies.askYesNo(
    `[nana] Update available: v${current} → v${latest}. Update now? [Y/n] `,
  );
  if (!approved) return;

  console.log(`[nana] Running: npm install -g ${PACKAGE_NAME}@latest`);
  const result = updateDependencies.runGlobalUpdate();

  if (result.ok) {
    await updateDependencies.setup({ force: true });
    console.log(`[nana] Updated to v${latest}. Restart to use new code.`);
  } else {
    console.log('[nana] Update failed. Run manually: npm install -g nana@latest');
  }
}
