...
function inferMutationScope(cwd = process.cwd()): AgentScope {
  const persistedScopePath = join(cwd, '.nana', 'setup-scope.json');
...
export async function agentsCommand(args: string[]): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'add': {
      const name = args[1];
      assert.ok(name, 'Usage: nana agents add <name>');
    ...
    case 'edit': {
      const name = args[1];
      assert.ok(name, 'Usage: nana agents edit <name>');
    ...
    case 'remove': {
      const name = args[1];
      assert.ok(name, 'Usage: nana agents remove <name>');
