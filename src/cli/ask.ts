...
export const ASK_USAGE = [
  'Usage: nana ask <claude|gemini> <question or task>',
...
const ASK_ADVISOR_SCRIPT_ENV = 'NANA_ASK_ADVISOR_SCRIPT';
const ASK_AGENT_PROMPT_FLAG = '--agent-prompt';
const ASK_ORIGINAL_TASK_ENV = 'NANA_ASK_ORIGINAL_TASK';
...
async function resolveAgentPromptContent(
  ...
  if (!existsSync(promptsDir)) {
    throw new Error(`[ask] prompts directory not found: ${promptsDir}. Run "nana setup" to install prompts.`);
  }
