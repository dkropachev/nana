...
export const AUTORESEARCH_HELP = `nana research - Launch NANA autoresearch with thin-supervisor parity semantics

Usage:
  nana autoresearch                                           (alias for nana research)
  nana autoresearch init [--topic T] [--evaluator CMD] [--keep-policy P] [--slug S]
  nana autoresearch run <mission-dir> [codex-args...]         (alias for nana research run)
  nana autoresearch --resume <run-id> [codex-args...]         (alias for nana research --resume)
  nana research                                                (human entrypoint: launch Codex CLI deep-interview intake, then execute)
  nana research [--topic T] [--evaluator CMD] [--keep-policy P] [--slug S]
  nana research init [--topic T] [--evaluator CMD] [--keep-policy P] [--slug S]
  nana research run <mission-dir> [codex-args...]              (agent/explicit execution entrypoint)
  nana research <mission-dir> [codex-args...]                  (compatibility alias for run)
  nana research --resume <run-id> [codex-args...]

Arguments:
  (no args)        Launch an interactive Codex session that activates deep-interview --autoresearch,
                   writes .nana/specs artifacts, then launches only after explicit confirmation.
  ...
  <run-id>         Existing autoresearch run id from .nana/logs/autoresearch/<run-id>/manifest.json

Behavior:
  - deep-interview intake writes canonical artifacts under .nana/specs before launch
  ...
  - supervisor records baseline, candidate, keep/discard/reset, and results artifacts under .nana/logs/autoresearch/
...
const AUTORESEARCH_APPEND_INSTRUCTIONS_ENV = 'NANA_AUTORESEARCH_APPEND_INSTRUCTIONS_FILE';
...
function buildAutoresearchDeepInterviewAppendix(): string {
  return [
    ...
    'You are in NANA autoresearch intake mode.',
    'Run the deep-interview skill in autoresearch mode and clarify the research mission before launch.',
    'Do not start tmux, do not launch `nana research`, and do not bypass the user confirmation boundary.',
    'When the user confirms launch and the evaluator is concrete, persist canonical artifacts under `.nana/specs/` using the contracts in `src/cli/autoresearch-intake.ts`.',
...
async function writeAutoresearchDeepInterviewAppendixFile(repoRoot: string): Promise<string> {
  ...
  const dir = join(repoRoot, '.nana', 'autoresearch');
...
async function runGuidedAutoresearchDeepInterview(
  ...
  if (!result) {
    throw new Error('autoresearch deep-interview did not produce .nana/specs launch artifacts.');
  }
...
function listHudWatchPaneIdsInCurrentWindow(currentPaneId?: string): string[] {
  ...
  return (result.stdout || '')
    ...
    .filter((pane) => /\bnana\b.*\bhud\b.*--watch/i.test(pane.startCommand || ''))
...
function launchAutoresearchInSplitPane(args: {
  ...
  const nanaPath = process.argv[1];
  if (!nanaPath) return false;
  ...
  const command = [process.execPath, nanaPath, ...launchArgs]
  ...
  if (sessionName && process.env.NANA_MOUSE !== '0') {
