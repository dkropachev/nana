...
const NANA_MCP_SERVER_PATTERN = /(?:^|[\\/])dist[\\/]mcp[\\/](?:state|memory|code-intel|trace|team)-server\.(?:[cm]?js|ts)\b/i;
const CODEX_PROCESS_PATTERN = /(?:^|[\\/\s])codex(?:\.js)?(?:\s|$)|@openai[\\/]codex/i;
const NANA_TMP_DIRECTORY_PATTERN = /^(omc|nana|nana)-/;
...
export function isNanaMcpProcess(command: string): boolean {
  const normalized = normalizeCommand(command);
  return normalized.includes('nana/dist/mcp') || NANA_MCP_SERVER_PATTERN.test(normalized);
}
...
export function listNanaProcesses(): ProcessEntry[] {
...
export function findCleanupCandidates(
  ...
  return processes
    ...
    .filter((processEntry) => isNanaMcpProcess(processEntry.command))
...
export async function cleanupNanaMcpProcesses(
  ...
  const listProcessesImpl = dependencies.listProcesses ?? listNanaProcesses;
  ...
  if (candidates.length === 0) {
    writeLine(dryRun
      ? 'Dry run: no orphaned NANA MCP server processes found.'
      : 'No orphaned NANA MCP server processes found.');
    return {
      dryRun,
      candidates,
      terminatedCount: 0,
  ...
  if (dryRun) {
    writeLine(`Dry run: would terminate ${candidates.length} orphaned NANA MCP server process(es):`);
  ...
  writeLine(`Found ${candidates.length} orphaned NANA MCP server process(es). Sending SIGTERM...`);
  ...
  writeLine(`Killed ${terminatedCount} orphaned NANA MCP server process(es)${forceKilledCount > 0 ? ` (${forceKilledCount} required SIGKILL)` : ''}.`);
...
export async function cleanupStaleTmpDirectories(
  ...
  for (const entry of await listTmpEntries(tmpRoot)) {
    if (!entry.isDirectory() || !NANA_TMP_DIRECTORY_PATTERN.test(entry.name)) continue;
  ...
  if (staleDirectories.length === 0) {
    writeLine(dryRun
      ? 'Dry run: no stale NANA /tmp directories found.'
      : 'No stale NANA /tmp directories found.');
    return 0;
  }

  const summaryTarget = formatPlural(
    staleDirectories.length,
    'stale NANA /tmp directory',
    'stale NANA /tmp directories',
  );
  if (dryRun) {
...
export async function cleanupCommand(
  ...
  const cleanupProcesses = dependencies.cleanupProcesses ?? cleanupNanaMcpProcesses;
