...
export const AGENTS_INIT_USAGE = [
  "Usage: nana agents-init [path] [--dry-run] [--force] [--verbose]",
...
const MANAGED_MARKER = "<!-- NANA:AGENTS-INIT:MANAGED -->";
const MANUAL_START = "<!-- NANA:AGENTS-INIT:MANUAL:START -->";
const MANUAL_END = "<!-- NANA:AGENTS-INIT:MANUAL:END -->";
...
export async function agentsInit(
  ...
  console.log("nana AGENTS bootstrap");
  console.log("===========================\n");
  ...
  for (let index = 0; index < plannedDirs.length; index += 1) {
    ...
    const decision = await syncManagedAgentsFile(
      ...
      backupRoot,
      rootOverlayRisk
        ? "active nana session detected for project root AGENTS.md"
        : undefined,
    );
  ...
  console.log(
    "- Preserves manual notes only for files already managed by nana agents-init.",
  );
  console.log(
    "- Never overwrites unmanaged AGENTS.md files unless you pass --force.",
  );
  console.log(
    "- Avoids rewriting project-root AGENTS.md while an active nana session is running.\n",
  );
