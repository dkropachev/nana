...
export const EXPLORE_USAGE = [
  'Usage: nana reflect --prompt "<prompt>"',
...
async function runExploreViaSparkShell(route: ExploreSparkShellRoute, env: NodeJS.ProcessEnv = process.env): Promise<void> {
  ...
  if (result.error) {
    ...
    throw new Error(`[reflect] failed to launch sparkshell backend: ${errno.message}`);
  }

  if (isSparkShellNativeCompatibilityFailure(result)) {
    throw new Error('[reflect] sparkshell backend is incompatible with this Linux runtime (missing GLIBC symbols)');
  }
...
export function resolvePackagedExploreHarnessCommand(
  ...
  const metadataPath = join(packageRoot, 'bin', 'nana-explore-harness.meta.json');
...
export async function resolveExploreHarnessCommandWithHydration(
  ...
  for (const cached of resolveCachedNativeBinaryCandidatePaths('nana-explore-harness', version, process.platform, process.arch, env)) {
  ...
  if (!isRepositoryCheckout(packageRoot)) {
    const hydrated = await hydrateNativeBinary('nana-explore-harness', { packageRoot, env });
    if (hydrated) return { command: hydrated, args: [] };
    throw new Error('[explore] no compatible native harness is available for this install. Reconnect to the network so NANA can fetch the release asset, or set NANA_EXPLORE_BIN to a prebuilt harness binary.');
  }
...
export async function exploreCommand(args: string[]): Promise<void> {
  ...
  if (sparkShellRoute) {
    try {
      ...
      process.stderr.write(`[nana explore] sparkshell backend unavailable (${message}). Falling back to the explore harness.\n`);
    }
  }
  ...
  if (result.error) {
    ...
    if (harness.command === 'cargo' && errno.code === 'ENOENT') {
      throw new Error('[reflect] cargo was not found. Install a Rust toolchain, use a compatible packaged nana-explore prebuilt, or set NANA_EXPLORE_BIN to a prebuilt harness binary.');
    }
    throw new Error(`[reflect] failed to launch harness: ${result.error.message}`);
  }
