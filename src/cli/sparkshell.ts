...
export const SPARKSHELL_USAGE = [
  ...
  'Runs the native nana-sparkshell sidecar with direct argv execution or explicit tmux pane summarization.',
...
export function nestedRepoLocalSparkShellBinaryPath(
  ...
  return join(packageRoot, 'native', 'nana-sparkshell', 'target', 'release', sparkshellBinaryName(platform));
}
...
export function resolveSparkShellBinaryPath(options: ResolveSparkShellBinaryPathOptions = {}): string {
  ...
  throw new Error(
    `[sparkshell] native binary not found. Checked ${packagedCandidates.join(', ')}, ${repoLocal}, and ${nestedRepoLocal}. `
      + `Set ${NANA_SPARKSHELL_BIN_ENV} to override the path.`
  );
}
...
export async function resolveSparkShellBinaryPathWithHydration(
  ...
  for (const cached of resolveCachedNativeBinaryCandidatePaths('nana-sparkshell', version, platform, arch, env, {
  ...
  const hydrated = await hydrateNativeBinary('nana-sparkshell', { packageRoot, env, platform, arch });
  if (hydrated) return hydrated;

  throw new Error(
    `[sparkshell] native binary not found. Checked cached/native candidates under ${packageRoot}, ${repoLocal}, and ${nestedRepoLocal}. `
      + `Reconnect to the network so NANA can fetch the release asset, or set ${NANA_SPARKSHELL_BIN_ENV} to override the path.`
  );
}
...
export async function sparkshellCommand(args: string[]): Promise<void> {
  ...
  const hasExplicitOverride = typeof process.env[NANA_SPARKSHELL_BIN_ENV] === 'string'
    && process.env[NANA_SPARKSHELL_BIN_ENV]!.trim().length > 0;
