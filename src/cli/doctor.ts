/**
 * nana doctor - Validate nana installation
 */
...
import {
  ...
  userSkillsDir, projectSkillsDir, nanaStateDir, detectLegacySkillRootOverlap,
} from '../utils/paths.js';
...
import { NANA_EXPLORE_CMD_ENV, isExploreCommandRoutingEnabled } from '../hooks/explore-routing.js';
...
export async function doctor(options: DoctorOptions = {}): Promise<void> {
  ...
  const scopeSourceMessage = scopeResolution.source === 'persisted'
    ? ' (from .nana/setup-scope.json)'
    : '';

  console.log('nana doctor');
  console.log('==================\n');
...
async function collectTeamDoctorIssues(cwd: string): Promise<TeamDoctorIssue[]> {
  ...
  for (const teamName of teamDirs) {
    ...
    let tmuxSession = `nana-team-${teamName}`;
  ...
  if ((existsSync(hudStatePath) || existsSync(leaderActivityPath)) && teamDirs.length > 0) {
    try {
      ...
      if (leaderIsStale && !tmuxUnavailable) {
        ...
        for (const teamName of teamDirs) {
          const session = knownTeamSessions.has(`nana-team-${teamName}`)
            ? `nana-team-${teamName}`
...
function listTeamTmuxSessions(): Set<string> | null {
  ...
  const sessions = (res.stdout || '')
    .split('\n')
    .map((s) => s.trim())
    .filter((s) => s.startsWith('nana-team-'));
...
function checkExploreHarness(): Check {
  ...
  const manifestPath = join(packageRoot, 'crates', 'nana-explore', 'Cargo.toml');
  if (!existsSync(manifestPath)) {
    return {
      name: 'Explore Harness',
      status: 'warn',
      message: 'Rust harness sources not found in this install (nana explore unavailable until packaged or NANA_EXPLORE_BIN is set)',
    };
  }
  ...
  if (override) {
    ...
    return {
      name: 'Explore Harness',
      status: 'warn',
      message: `NANA_EXPLORE_BIN is set but path was not found (${override})`,
    };
  }
  ...
  if (result.error) {
    ...
    if (kind === 'missing') {
      return {
        name: 'Explore Harness',
        status: 'warn',
        message: `Rust harness sources are packaged, but no compatible packaged prebuilt or cargo was found (install Rust or set ${EXPLORE_BIN_ENV} for nana explore)`,
      };
    }
    return {
      name: 'Explore Harness',
...
async function checkConfig(configPath: string): Promise<Check> {
  ...
  try {
    ...
    const hasNana =
      /\[mcp_servers\.nana_/.test(content)
      || /#\s*nana top-level settings/i.test(content)
      || /managed by nana setup/i.test(content)
      || /\[env\][\s\S]*USE_NANA_/.test(content);
    if (hasNana) {
      return { name: 'Config', status: 'pass', message: 'config.toml has NANA entries' };
    }

    return {
      name: 'Config',
      status: 'warn',
      message: 'config.toml exists but no NANA entries yet (expected before first setup; run "nana setup --force" once)',
    };
  } catch {
...
async function checkExploreRouting(configPath: string): Promise<Check> {
  const envValue = process.env[NANA_EXPLORE_CMD_ENV];
  if (typeof envValue === 'string' && !isExploreCommandRoutingEnabled(process.env)) {
    return {
      name: 'Explore routing',
      status: 'warn',
      message:
        'disabled by environment override; enable with USE_NANA_EXPLORE_CMD=1 (or remove the explicit opt-out)',
    };
  }
  ...
  try {
    ...
    const configuredValue = parsed?.env?.USE_NANA_EXPLORE_CMD;

    if (
      typeof configuredValue === 'string' &&
      !isExploreCommandRoutingEnabled({
        USE_NANA_EXPLORE_CMD: configuredValue,
      })
    ) {
      return {
        name: 'Explore routing',
        status: 'warn',
        message:
          'disabled in config.toml [env]; set USE_NANA_EXPLORE_CMD = "1" to restore default explore-first routing',
      };
    }
...
function checkAgentsMd(scope: DoctorSetupScope, codexHomeDir: string): Check {
  if (scope === 'user') {
    ...
    return {
      name: 'AGENTS.md',
      status: 'warn',
      message: `not found in ${userAgentsMd} (run nana setup --scope user)`,
    };
  }
...
async function checkMcpServers(configPath: string): Promise<Check> {
  ...
  try {
    ...
    if (mcpCount > 0) {
      const hasNana = content.includes('nana_state') || content.includes('nana_memory');
      if (hasNana) {
        return { name: 'MCP Servers', status: 'pass', message: `${mcpCount} servers configured (NANA present)` };
      }
      return {
        name: 'MCP Servers',
        status: 'warn',
        message: `${mcpCount} servers but no NANA servers yet (expected before first setup; run "nana setup --force" once)`,
      };
    }
