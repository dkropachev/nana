...
const DEFAULT_CONFIG: TmuxHookConfig = {
  ...
  prompt_template: 'Continue from current mode state. [NANA_TMUX_INJECT]',
  marker: '[NANA_TMUX_INJECT]',
  dry_run: false,
  log_level: 'info',
...
const HELP = `
Usage:
  nana tmux-hook init       Create .nana/tmux-hook.json
  nana tmux-hook status     Show config + runtime state summary
  nana tmux-hook validate   Validate config and tmux target reachability
  nana tmux-hook test       Run a synthetic notify-hook turn (end-to-end)
`;
...
function nanaDir(cwd = process.cwd()): string {
  return join(cwd, '.nana');
}

function tmuxHookConfigPath(cwd = process.cwd()): string {
  return join(nanaDir(cwd), 'tmux-hook.json');
}

function tmuxHookStatePath(cwd = process.cwd()): string {
  return join(nanaDir(cwd), 'state', 'tmux-hook-state.json');
}

function tmuxHookLogPath(cwd = process.cwd()): string {
  return join(nanaDir(cwd), 'logs', `tmux-hook-${new Date().toISOString().split('T')[0]}.jsonl`);
}
...
async function readValidatedConfig(cwd = process.cwd()): Promise<TmuxHookConfig> {
  ...
  if (!existsSync(configPath)) {
    throw new Error('tmux-hook config missing. Run: nana tmux-hook init');
  }
  ...
  if (!existsSync(configPath)) {
    ...
    if (initResult.created) {
      ...
      if (initResult.usedPlaceholderTarget) {
        console.log('Could not auto-detect a tmux target. Edit `.nana/tmux-hook.json` when ready.');
...
async function initTmuxHookConfig(opts?: { silent?: boolean; cwd?: string }): Promise<InitConfigResult> {
  ...
  const configPath = tmuxHookConfigPath(cwd);
  await mkdir(nanaDir(cwd), { recursive: true });
...
async function testTmuxHook(args: string[]): Promise<void> {
  ...
  const payload = {
    ...
    'thread-id': threadId,
    'turn-id': turnId,
    'input-messages': ['nana tmux-hook test'],
  ...
  console.log('Check: .nana/logs/tmux-hook-YYYY-MM-DD.jsonl for skip/reason codes.');
}
