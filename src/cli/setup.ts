/**
 * nana setup - Automated installation of nana
...
const PROJECT_NANA_GITIGNORE_ENTRY = ".nana/";
...
function getBackupContext(
  ...
  if (scope === "project") {
    return {
      backupRoot: join(projectRoot, ".nana", "backups", "setup", timestamp),
      baseRoot: projectRoot,
    };
  }
  return {
    backupRoot: join(homedir(), ".nana", "backups", "setup", timestamp),
    baseRoot: homedir(),
  };
}
...
function getScopeFilePath(projectRoot: string): string {
  return join(projectRoot, ".nana", "setup-scope.json");
}
...
async function readPersistedSetupPreferences(
  ...
  try {
    ...
    if (parsed && typeof parsed.scope === "string") {
      ...
      if (migrated) {
        console.warn(
          `[nana] Migrating persisted setup scope "${parsed.scope}" → "${migrated}" ` +
...
async function promptForSetupScope(
  ...
  try {
    ...
    console.log(
      `  1) user (default) — installs to ~/.nana/codex-home (skills default to ~/.nana/codex-home/skills)`,
    );
...
function shouldNanaManageTuiFromCodexVersion(versionOutput: string | null): boolean {
...
async function ensureProjectNanaGitignore(
  projectRoot: string,
  backupContext: SetupBackupContext,
  options: Pick<SetupOptions, "dryRun" | "verbose">,
  ...
  if (hasGitignoreEntry(existing, PROJECT_NANA_GITIGNORE_ENTRY)) {
    return "unchanged";
  }

  const nextContent = destinationExists
    ? `${existing}${existing.endsWith("\n") || existing.length === 0 ? "" : "\n"}${PROJECT_NANA_GITIGNORE_ENTRY}\n`
    : `${PROJECT_NANA_GITIGNORE_ENTRY}\n`;
  ...
  if (options.verbose) {
    console.log(
      `  ${options.dryRun ? "would update" : destinationExists ? "updated" : "created"} .gitignore (${PROJECT_NANA_GITIGNORE_ENTRY})`,
    );
  }
...
export async function setup(options: SetupOptions = {}): Promise<void> {
  ...
  const scopeSourceMessage =
    resolvedScope.source === "persisted" ? " (from .nana/setup-scope.json)" : "";
  const backupContext = getBackupContext(resolvedScope.scope, projectRoot);

  console.log("nana setup");
  console.log("=================\n");
  console.log(
  ...
  const dirs = [
    ...
    nanaStateDir(projectRoot),
    nanaPlansDir(projectRoot),
    nanaLogsDir(projectRoot),
  ];
  ...
  if (resolvedScope.scope === "project") {
    const gitignoreResult = await ensureProjectNanaGitignore(
      projectRoot,
      backupContext,
      { dryRun, verbose },
    );
    if (gitignoreResult === "created") {
      console.log(
        "  Created .gitignore with .nana/ so local NANA runtime state stays out of source control.\n",
      );
    } else if (gitignoreResult === "updated") {
      console.log(
        "  Added .nana/ to .gitignore so local NANA runtime state stays out of source control.\n",
      );
    }
  }
  ...
  if (teamToolsCheck.ok) {
    console.log("  nana team api command detected (CLI-first interop ready)");
  } else {
    console.log(`  WARNING: ${teamToolsCheck.message}`);
    console.log("  Run `npm run build` and then re-run `nana setup`.");
  }
  console.log();
  ...
  if (existsSync(agentsMdSrc)) {
    ...
    if (agentsMdExists) {
      ...
      if (isNanaGeneratedAgentsMd(existing)) {
    ...
    if (
      ...
      console.log(
        "  WARNING: Active nana session detected (pid " +
          activeSession?.pid +
          ").",
      );
      console.log(
  ...
  const hudConfigPath = join(projectRoot, ".nana", "hud-config.json");
  ...
  console.log('Setup complete! Run "nana doctor" to verify installation.');
  console.log("\nNext steps:");
  ...
  console.log(
    '  6. "nana explore" and "nana sparkshell" can hydrate native release binaries on first use; source installs still allow repo-local fallbacks and NANA_EXPLORE_BIN / NANA_SPARKSHELL_BIN overrides',
  );
  if (isGitHubCliConfigured()) {
    console.log("\nSupport the project: gh repo star Yeachan-Heo/nana");
  }
}
...
async function syncManagedAgentsContent(
  ...
  if (destinationExists && !options.force) {
    ...
    if (!shouldOverwrite) {
      summary.skipped += 1;
      if (options.verbose) {
        const managedLabel = isNanaGeneratedAgentsMd(existing)
          ? "managed"
          : "unmanaged";
...
async function updateManagedConfig(
  ...
  const nanaManagesTui = shouldNanaManageTuiFromCodexVersion(codexVersion);
  ...
  if (!changed) {
    summary.unchanged += 1;
    return { finalConfig, nanaManagesTui };
  }
