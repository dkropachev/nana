...
async function initHooks(): Promise<void> {
  ...
  console.log('Plugins are enabled by default. Disable with NANA_HOOK_PLUGINS=0.');
}
...
async function statusHooks(): Promise<void> {
  ...
  console.log(
    `Plugins enabled: ${isHookPluginsEnabled(process.env) ? 'yes' : 'no (disabled with NANA_HOOK_PLUGINS=0)'}`,
  );
...
async function validateHooks(): Promise<void> {
  ...
  const plugins = await discoverHookPlugins(cwd);
  if (plugins.length === 0) {
    console.log('No plugins found. Run: nana hooks init');
    return;
  }
...
async function testHooks(): Promise<void> {
  ...
  const logPath = join(cwd, '.nana', 'logs', `hooks-${new Date().toISOString().split('T')[0]}.jsonl`);
