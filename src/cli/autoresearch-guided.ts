...
export function buildAutoresearchDeepInterviewPrompt(
  ...
  return [
    ...
    'Run the deep-interview skill in autoresearch mode for `nana autoresearch`.',
    'Guide the user through research topic definition, evaluator readiness, keep policy, and slug/session naming.',
    'Do not launch tmux or run `nana autoresearch` yourself.',
    'When the user confirms launch and the evaluator is concrete, write/update these canonical artifacts under `.nana/specs/`:',
...
export async function runAutoresearchNoviceBridge(
  ...
  try {
    while (true) {
      ...
      const evaluatorIntent = await promptWithDefault(io, '\nHow should NANA judge success? Describe it in plain language', topic);
...
export function spawnAutoresearchTmux(missionDir: string, slug: string): void {
  ...
  const sessionName = `nana-autoresearch-${slug}`;
  ...
  const nanaPath = resolve(join(__dirname, '..', '..', 'bin', 'nana.js'));
  const cmd = `${shellQuote(process.execPath)} ${shellQuote(nanaPath)} autoresearch ${shellQuote(missionDir)}`;
