/**
 * nana uninstall - Remove nana configuration and installed artifacts
 */
...
import {
  stripExistingNanaBlocks,
  stripNanaEnvSettings,
  stripNanaTopLevelKeys,
  stripNanaFeatureFlags,
...
import { isNanaGeneratedAgentsMd } from "../utils/agents-md.js";
...
function detectNanaConfigArtifacts(config: string): {
  ...
  const hasMcpServers = NANA_MCP_SERVERS.filter((name) =>
...
> {
  ...
  const detected = detectNanaConfigArtifacts(original);
  ...
  // Strip NANA tables block (MCP servers, agents, tui)
  let config = original;
  const { cleaned } = stripExistingNanaBlocks(config);
  config = cleaned;

  // Strip top-level keys
  config = stripNanaTopLevelKeys(config);

  // Strip feature flags
  config = stripNanaFeatureFlags(config);

  // Strip NANA-managed env defaults
  config = stripNanaEnvSettings(config);
  ...
  if (config !== original) {
    ...
    if (options.verbose) console.log("  No NANA config entries found.");
  }
...
async function removeAgentsMd(
  ...
  try {
    ...
    if (!isNanaGeneratedAgentsMd(content)) {
      if (options.verbose)
        console.log("  AGENTS.md is not NANA-generated, skipping.");
      return false;
    }
  } catch {
    return false;
...
async function removeCacheDirectory(
  ...
  const nanaDir = join(projectRoot, ".nana");
  if (!existsSync(nanaDir)) return false;

  if (!options.dryRun) {
    await rm(nanaDir, { recursive: true, force: true });
  }
  if (options.verbose)
    console.log(`  ${options.dryRun ? "Would remove" : "Removed"} ${nanaDir}`);
  return true;
}
...
function printSummary(summary: UninstallSummary, dryRun: boolean): void {
  ...
  if (summary.configCleaned) {
    console.log(`  ${prefix} NANA configuration block from config.toml`);
    ...
    console.log("  config.toml: no NANA entries found (or --keep-config used)");
  }
  ...
  if (summary.cacheDirectoryRemoved) {
    console.log(`  ${prefix} .nana/ cache directory`);
  }
  ...
  if (totalActions === 0) {
    console.log(
      "  Nothing to remove. nana does not appear to be installed.",
    );
  }
}
...
export async function uninstall(options: UninstallOptions = {}): Promise<void> {
  ...
  // Step 5: Remove AGENTS.md and optionally .nana/ cache directory
  ...
  if (purge) {
    ...
    const scopeFile = join(projectRoot, ".nana", "setup-scope.json");
    const hudConfig = join(projectRoot, ".nana", "hud-config.json");
