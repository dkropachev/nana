/**
 * TS Runtime Bridge — thin wrapper over nana-runtime binary.
 *
 * All semantic state mutations route through `execCommand()`.
 * All state queries read Rust-authored compatibility JSON files.
 * Set NANA_RUNTIME_BRIDGE=0 to disable bridge (fallback to TS-direct).
 */
...
export function resolveRuntimeBinaryPath(options: RuntimeBinaryDiscoveryOptions = {}): string {
  ...
  const workspaceRelease = options.releasePath ?? resolve(__bridge_dirname, '../../target/release/nana-runtime');
  if (exists(workspaceRelease)) return workspaceRelease;

  return options.fallbackBinary ?? 'nana-runtime';
}
...
export class RuntimeBridge {
  ...
  constructor(options: { stateDir?: string; binaryPath?: string } = {}) {
    this.enabled = process.env.NANA_RUNTIME_BRIDGE !== '0';
    this.stateDir = options.stateDir;
    this.binaryPath = options.binaryPath ?? resolveRuntimeBinaryPath();
  }

  /** Whether the bridge is enabled (NANA_RUNTIME_BRIDGE != '0'). */
  isEnabled(): boolean {
    return this.enabled;
  }
  ...
  private validateSchemaOnce(): void {
    ...
    try {
      ...
      if (missing.length > 0) {
        throw new Error(
          `nana-runtime schema missing commands: ${missing.join(', ')}. ` +
  ...
  private run(args: string[]): string {
    try {
      ...
      throw new Error(`nana-runtime ${args[0]} failed: ${stderr}`);
    }
  }
}
...
export function isBridgeEnabled(): boolean {
  return process.env.NANA_RUNTIME_BRIDGE !== '0';
}
