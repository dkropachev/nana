 ...
 * passed as a literal argument to tmux (no shell expansion).  `nanaBin` is
...
export function buildTmuxSplitArgs(
  ...
  const cmd = `node ${shellEscape(nanaBin)} hud --watch${presetArg}`;
...
async function launchTmuxPane(cwd: string, flags: HudFlags): Promise<void> {
  ...
  if (!process.env.TMUX) {
    console.error('Not inside a tmux session. Start tmux first, then run: nana hud --tmux');
    process.exit(1);
  }

  const nanaBin = process.argv[1]; // path to bin/nana.js
  const args = buildTmuxSplitArgs(cwd, nanaBin, flags.preset);

  try {
    // Split bottom pane, 4 lines tall, running nana hud --watch.
    // execFileSync bypasses the shell – cwd and nanaBin cannot inject commands.
