...
function buildLaunchHints(
  ...
  if (mode === 'team') {
    return {
      shellCommand: `nana team ${recommendedHeadcount}:${fallbackRole} ${toQuotedCliArg(task)}`,
  ...
  return {
    shellCommand: `nana ralph ${toQuotedCliArg(task)}`,
...
function buildVerificationPlan(
  ...
  if (mode === 'team') {
    ...
    return {
      ...
      checkpoints: [
        'Launch via `nana team ...` (or `$team ...`) so the team runtime owns both parallel delivery and coordinated verification.',
