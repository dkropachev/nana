...
const INJECTION_MARKER = '[NANA_TMUX_INJECT]';
...
const NANA_TEAM_WORKER_CLI_ENV = 'NANA_TEAM_WORKER_CLI';
const NANA_TEAM_WORKER_CLI_MAP_ENV = 'NANA_TEAM_WORKER_CLI_MAP';
const NANA_TEAM_WORKER_LAUNCH_MODE_ENV = 'NANA_TEAM_WORKER_LAUNCH_MODE';
const NANA_TEAM_AUTO_INTERRUPT_RETRY_ENV = 'NANA_TEAM_AUTO_INTERRUPT_RETRY';
...
const NANA_LEADER_NODE_PATH_ENV = 'NANA_LEADER_NODE_PATH';
const NANA_LEADER_CLI_PATH_ENV = 'NANA_LEADER_CLI_PATH';
...
function isHudWatchPane(pane: TmuxPaneInfo): boolean {
  ...
  return /\b(?:nana|nana)\b.*\bhud\b.*--watch/i.test(start);
}
...
function normalizeTeamWorkerCliMode(raw: string | undefined, sourceEnv: string = NANA_TEAM_WORKER_CLI_ENV): TeamWorkerCliMode {
...
export function resolveTeamWorkerLaunchMode(
  ...
  const raw = String(env[NANA_TEAM_WORKER_LAUNCH_MODE_ENV] ?? 'interactive').trim().toLowerCase();
  if (raw === '' || raw === 'interactive') return 'interactive';
  if (raw === 'prompt') return 'prompt';
  throw new Error(`Invalid ${NANA_TEAM_WORKER_LAUNCH_MODE_ENV} value "${env[NANA_TEAM_WORKER_LAUNCH_MODE_ENV]}". Expected: interactive, prompt`);
}
...
export function resolveTeamWorkerCli(launchArgs: string[] = [], env: NodeJS.ProcessEnv = process.env): TeamWorkerCli {
  const mode = normalizeTeamWorkerCliMode(env[NANA_TEAM_WORKER_CLI_ENV]);
...
export function resolveTeamWorkerCliPlan(
  ...
  const rawMap = String(env[NANA_TEAM_WORKER_CLI_MAP_ENV] ?? '').trim();
  ...
  if (entries.length === 0 || entries.every((part) => part.length === 0)) {
    throw new Error(
      `Invalid ${NANA_TEAM_WORKER_CLI_MAP_ENV} value "${env[NANA_TEAM_WORKER_CLI_MAP_ENV]}". `
  ...
  if (entries.length !== 1 && entries.length !== workerCount) {
    throw new Error(
      `Invalid ${NANA_TEAM_WORKER_CLI_MAP_ENV} length ${entries.length}; `
  ...
  return expanded.map((entry) => {
    const mode = normalizeTeamWorkerCliMode(entry, NANA_TEAM_WORKER_CLI_MAP_ENV);
...
export function assertTeamWorkerCliBinaryAvailable(
  ...
  throw new Error(
    `Selected team worker CLI "${workerCli}" is not available on PATH. `
      + `Install "${workerCli}" or set ${NANA_TEAM_WORKER_CLI_ENV}=codex|claude|gemini.`,
  );
}
...
function buildModelInstructionsOverride(cwd: string, env: NodeJS.ProcessEnv): string {
  const filePath = translatePathForMsys(env[NANA_MODEL_INSTRUCTIONS_FILE_ENV] || join(cwd, 'AGENTS.md'));
...
export function buildWorkerProcessLaunchSpec(
  ...
  const workerEnv: Record<string, string> = {
    NANA_TEAM_WORKER: `${teamName}/worker-${workerIndex}`,
    [NANA_LEADER_NODE_PATH_ENV]: resolveLeaderNodePath(),
    [NANA_LEADER_CLI_PATH_ENV]: resolvedCliPath,
  };
 ...
 * NANA requires tmux, which is unavailable on native Windows.
 */
...
export function createTeamSession(
  ...
  try {
    ...
    const nanaEntry = process.argv[1];
    if (nanaEntry && nanaEntry.trim() !== '') {
      const hudCmd = `node ${shellQuoteSingle(translatePathForMsys(nanaEntry))} hud --watch`;
    ...
    // Opt-out: set NANA_TEAM_MOUSE=0 in the environment.
    if (process.env.NANA_TEAM_MOUSE !== '0') {
...
export function restoreStandaloneHudPane(
  ...
  const nanaEntry = process.argv[1];
  if (!nanaEntry || nanaEntry.trim() === '') return null;
 ...
 *
 * This helper is intentionally limited to session-scoped options so NANA
...
function dismissClaudeBypassPermissionsPromptIfPresent(target: string, captured: string): boolean {
  if (process.env.NANA_TEAM_AUTO_ACCEPT_BYPASS === '0') return false;
...
function resolveSendStrategyFromEnv(): 'auto' | 'queue' | 'interrupt' {
  const raw = String(process.env.NANA_TEAM_SEND_STRATEGY || '')
    .trim()
    .toLowerCase();
...
function resolveWorkerCliFromMapForSend(
  ...
  try {
    const mode = normalizeTeamWorkerCliMode(selectedRaw, NANA_TEAM_WORKER_CLI_MAP_ENV);
 ...
 * 2) per-worker NANA_TEAM_WORKER_CLI_MAP entry (worker index aware)
 * 3) global/default NANA_TEAM_WORKER_CLI behavior
 */
...
export function waitForWorkerReady(
  ...
  const check = (): boolean => {
    ...
    if (paneHasTrustPrompt(result.stdout)) {
      ...
      // Opt-out by setting NANA_TEAM_AUTO_TRUST=0.
      if (process.env.NANA_TEAM_AUTO_TRUST !== '0') {
        sendRobustEnter();
        promptDismissed = true;
        return false;
      }
 ...
 * Opt-out: set NANA_TEAM_AUTO_TRUST=0 to disable auto-dismissal.
 */
export function dismissTrustPromptIfPresent(
  ...
  if (process.env.NANA_TEAM_AUTO_TRUST === '0') return false;
...
export async function sendToWorker(
  ...
  const allowAutoInterruptRetry = process.env[NANA_TEAM_AUTO_INTERRUPT_RETRY_ENV] !== '0';
  ...
  // If you need strictness for debugging, set NANA_TEAM_STRICT_SUBMIT=1.
  const strict = process.env.NANA_TEAM_STRICT_SUBMIT === '1';
  if (strict) {
...
// List all tmux sessions matching nana-team-* pattern
 ...
 * workers and runtime paths may message `leader-fixed` via `nana team api`
