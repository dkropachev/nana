...
export async function withTempTmuxSession<T>(
  ...
  const fixtureCwd = await mkdtemp(join(tmpdir(), 'nana-tmux-fixture-'));
  const sessionName = uniqueTmuxIdentifier('nana-test');
  const serverName = options.useAmbientServer ? '' : uniqueTmuxIdentifier('nana-fixture');
