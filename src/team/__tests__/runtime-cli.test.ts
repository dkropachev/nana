...
async function loadRuntimeCliModule() {
  process.env.NANA_RUNTIME_CLI_DISABLE_AUTO_START = '1';
...
describe('runtime-cli helpers', () => {
  ...
  it('refreshes pane targets from live team config after scale changes', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-runtime-cli-live-'));
    try {
  ...
  it('gracefully shuts down only when the leader explicitly requests shutdown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-runtime-cli-shutdown-'));
    ...
    try {
      ...
      const teamRoot = join(cwd, '.nana', 'state', 'team', 'shutdown-fallback');
  ...
  it('does not auto-shutdown merely because monitorTeam reaches complete', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-runtime-cli-complete-'));
    const previousTeamStateRoot = process.env.NANA_TEAM_STATE_ROOT;
    delete process.env.NANA_TEAM_STATE_ROOT;
    try {
      ...
      const teamRoot = join(cwd, '.nana', 'state', 'team', 'runtime-cli-complete');
      assert.equal(existsSync(teamRoot), true);
