...
export interface TeamConfig {
  ...
  workers: WorkerInfo[];
  created_at: string;
  tmux_session: string; // "nana-team-{name}"
  next_task_id: number;
  leader_cwd?: string;
...
function resolveDisplayModeFromEnv(env: NodeJS.ProcessEnv): TeamPolicy['display_mode'] {
  const raw = readEnvValue(env, ['NANA_TEAM_DISPLAY_MODE', 'NANA_TEAM_MODE']);
  if (!raw) return 'auto';
...
function resolveWorkerLaunchModeFromEnv(env: NodeJS.ProcessEnv): TeamPolicy['worker_launch_mode'] {
  const raw = readEnvValue(env, ['NANA_TEAM_WORKER_LAUNCH_MODE']);
  if (!raw || raw === 'interactive') return 'interactive';
  if (raw === 'prompt') return 'prompt';
  throw new Error(`Invalid NANA_TEAM_WORKER_LAUNCH_MODE value "${raw}". Expected: interactive, prompt`);
}
...
function resolvePermissionsSnapshot(env: NodeJS.ProcessEnv): PermissionsSnapshot {
  ...
  const sandboxMode = readEnvValue(env, ['NANA_SANDBOX_MODE', 'CODEX_SANDBOX_MODE', 'SANDBOX_MODE']);
  if (sandboxMode) snapshot.sandbox_mode = sandboxMode;

  const network = parseOptionalBoolean(readEnvValue(env, ['NANA_NETWORK_ACCESS', 'CODEX_NETWORK_ACCESS', 'NETWORK_ACCESS']));
...
async function resolveLeaderSessionId(cwd: string, env: NodeJS.ProcessEnv): Promise<string> {
  const fromEnv = readEnvValue(env, ['NANA_SESSION_ID', 'CODEX_SESSION_ID', 'SESSION_ID']);
  if (fromEnv) return fromEnv;

  const sessionPath = join(nanaStateDir(cwd), 'session.json');
  try {
...
// Creates: .nana/state/team/{name}/, workers/{worker-1}..{worker-N}/, tasks/
...
export async function initTeamState(
  ...
  const leaderWorkerId = readEnvValue(env, ['NANA_TEAM_WORKER']) ?? 'leader-fixed';
  ...
  const config: TeamConfig = {
    ...
    tmux_session: `nana-team-${teamName}`,
    next_task_id: 1,
...
async function readMailbox(teamName: string, workerName: string, cwd: string): Promise<TeamMailbox> {
  ...
  let legacyMessages: TeamMailboxMessage[] | null = null;
  try {
    if (existsSync(p)) {
      const raw = await readFile(p, 'utf8');
      const parsed = JSON.parse(raw) as unknown;
      if (parsed && typeof parsed === 'object') {
        ...
        if (v.worker === workerName && Array.isArray(v.messages)) {
          legacyMessages = v.messages as TeamMailboxMessage[];
        }
      }
    }
  } catch {
  ...
  if (isBridgeEnabled()) {
    try {
      ...
      if (compat) {
        ...
        if (!legacyMessages) return { worker: workerName, messages: bridgeMessages };

        const merged = new Map<string, TeamMailboxMessage>();
        for (const message of bridgeMessages) merged.set(message.message_id, message);
        for (const message of legacyMessages) {
          const bridgeMessage = merged.get(message.message_id);
          if (!bridgeMessage) continue;
          const bridgeTs = Date.parse(bridgeMessage.delivered_at ?? bridgeMessage.notified_at ?? bridgeMessage.created_at ?? '');
          const legacyTs = Date.parse(message.delivered_at ?? message.notified_at ?? message.created_at ?? '');
          if (!Number.isFinite(bridgeTs) || (Number.isFinite(legacyTs) && legacyTs >= bridgeTs)) {
            merged.set(message.message_id, message);
          }
        }
        return { worker: workerName, messages: [...merged.values()] };
      }
    } catch {
  ...
  return { worker: workerName, messages: legacyMessages ?? [] };
}
...
async function readDispatchRequests(teamName: string, cwd: string): Promise<TeamDispatchRequest[]> {
  ...
  let legacyRequests: TeamDispatchRequest[] | null = null;
  try {
    if (existsSync(path)) {
  ...
  if (isBridgeEnabled()) {
    try {
      ...
      if (compat) {
        ...
        if (!legacyRequests) return bridgeRequests;

        const merged = new Map<string, TeamDispatchRequest>();
        for (const request of bridgeRequests) merged.set(request.request_id, request);
        for (const request of legacyRequests) {
          const bridgeRequest = merged.get(request.request_id);
          if (!bridgeRequest) continue;
          const bridgeTs = Date.parse(bridgeRequest.updated_at ?? bridgeRequest.created_at ?? '');
          const legacyTs = Date.parse(request.updated_at ?? request.created_at ?? '');
          if (!Number.isFinite(bridgeTs) || (Number.isFinite(legacyTs) && legacyTs >= bridgeTs)) {
