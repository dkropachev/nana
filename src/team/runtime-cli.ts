 ...
 *
 * Spawned by nana_run_team_start in state-server.ts.
 */
...
async function writePanesFile(
  ...
  const nanaJobsDir = process.env.NANA_JOBS_DIR;
  if (!jobId || !nanaJobsDir) return;

  const panesPath = join(nanaJobsDir, `${jobId}-panes.json`);
  await writeFile(
    panesPath + '.tmp',
...
async function main(): Promise<void> {
  ...
  const stateRoot = join(cwd, '.nana', 'state');
  ...
  // Start the team — NANA's startTeam takes individual parameters
  const agentType = 'executor';
  try {
    const providers = normalizeAgentTypes(agentTypes, workerCount);
    const previousCliMap = process.env.NANA_TEAM_WORKER_CLI_MAP;
    try {
      process.env.NANA_TEAM_WORKER_CLI_MAP = providers.join(',');
      ...
      if (typeof previousCliMap === 'string') process.env.NANA_TEAM_WORKER_CLI_MAP = previousCliMap;
      else delete process.env.NANA_TEAM_WORKER_CLI_MAP;
    }
  } catch (err) {
  ...
  // Persist pane IDs so MCP server can clean up explicitly via nana_run_team_cleanup.
  const jobId = process.env.NANA_JOB_ID;
  try {
...
const shouldAutoStart = process.env.NANA_RUNTIME_CLI_DISABLE_AUTO_START !== '1';
