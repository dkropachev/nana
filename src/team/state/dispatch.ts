...
function hasExplicitRuntimeBinaryOverride(env: NodeJS.ProcessEnv = process.env): boolean {
  return typeof env.NANA_RUNTIME_BINARY === 'string' && env.NANA_RUNTIME_BINARY.trim() !== '';
}
...
export async function transitionDispatchRequest(
  ...
  return await deps.withDispatchLock(deps.teamName, deps.cwd, async () => {
    ...
    const existing = index >= 0 ? requests[index]! : (await readDispatchRequest(requestId, deps));
    ...
    if (!hasExplicitRuntimeBinaryOverride()) {
      if (index >= 0) requests[index] = next;
      else requests.push(next);
      await deps.writeDispatchRequests(deps.teamName, requests, deps.cwd);
    }

    if (to === 'notified' && existing.status !== 'failed') {
      executeBridgeCommand(deps.cwd, {
        command: 'MarkNotified',
        request_id: requestId,
        channel: typeof next.last_reason === 'string' && next.last_reason.trim() !== '' ? next.last_reason : 'tmux',
      });
    } else if (to === 'delivered') {
      executeBridgeCommand(deps.cwd, { command: 'MarkDelivered', request_id: requestId });
    } else if (to === 'failed') {
      executeBridgeCommand(deps.cwd, {
        command: 'MarkFailed',
        request_id: requestId,
        reason: typeof next.last_reason === 'string' && next.last_reason.trim() !== '' ? next.last_reason : 'failed',
      });
    }
    return next;
  });
...
export async function markDispatchRequestFailed(
  ...
  await transitionDispatchRequest(requestId, current.status, 'failed', { last_reason: reason }, deps);
}
