...
function summarizeWorktreeDiffWithSparkShell(worktreePath: string): string | null {
  ...
  const result = runCommand('nana', ['sparkshell', 'sh', '-lc', shellCommand], worktreePath);
...
function autoCommitDirtyWorktree(
  ...
  const msg = `nana(team): auto-checkpoint ${worker.name} [${taskId}]`;
...
async function integrateWorkerCommitsIntoLeader(params: {
  ...
  for (const worker of config.workers) {
    ...
    if (workerIsAheadOfLeader) {
      ...
      const merge = runGitCommand(repoRoot, ['merge', '--no-ff', '-X', 'theirs', '-m', `nana(team): merge ${worker.name}`, branchRef], cwd);
...
async function prepareShutdownMergeReport(
  ...
  if (hadChanges) {
    ...
    if (!addResult.ok) {
      return {
        workerName: worker.name,
        worktreePath,
        reportPath: join(worktreePath, '.nana', 'diff.md'),
        sourceRef: null,
    ...
    const commitResult = runGitCommand(
      repoRoot,
      ['commit', '--no-verify', '-m', `nana(team): checkpoint ${worker.name} shutdown changes`],
      worktreePath,
    );
    if (commitResult.ok) {
  ...
  const reportPath = join(worktreePath, '.nana', 'diff.md');
  ...
  await mkdir(join(worktreePath, '.nana'), { recursive: true });
...
async function prepareWorkerWorktreeShutdownReports(config: TeamConfig, leaderCwd: string): Promise<void> {
  for (const worker of config.workers) {
    ...
    try {
      ...
      await mkdir(join(worktreePath, '.nana'), { recursive: true }).catch(() => {});
...
const MODEL_INSTRUCTIONS_FILE_ENV = 'NANA_MODEL_INSTRUCTIONS_FILE';
const TEAM_STATE_ROOT_ENV = 'NANA_TEAM_STATE_ROOT';
const TEAM_LEADER_CWD_ENV = 'NANA_TEAM_LEADER_CWD';
const WORKTREE_TRIGGER_STATE_ROOT = '$NANA_TEAM_STATE_ROOT';
...
function resolveWorkerReadyTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw = env.NANA_TEAM_READY_TIMEOUT_MS;
...
async function assertNestedTeamAllowed(cwd: string): Promise<void> {
  const workerContext = parseTeamWorkerContext(process.env.NANA_TEAM_WORKER);
...
function shouldSkipWorkerReadyWait(env: NodeJS.ProcessEnv): boolean {
  return env.NANA_TEAM_SKIP_READY_WAIT === '1';
}
...
export function resolveWorkerLaunchArgsFromEnv(
  ...
  const preEnvArgs = splitWorkerLaunchArgs(env.NANA_TEAM_WORKER_LAUNCH_ARGS);
  ...
  const resolved = resolveTeamWorkerLaunchArgs({
    existingRaw: env.NANA_TEAM_WORKER_LAUNCH_ARGS,
    inheritedArgs,
    fallbackModel,
    preferredReasoning,
  });
  ...
  if (effectiveWorkerCli === 'claude') {
    console.log('[nana:team] worker startup resolution: model=claude source=local-settings');
  } else if (effectiveWorkerCli === 'gemini') {
    console.log('[nana:team] worker startup resolution: model=gemini source=local-settings');
  } else {
    console.log(`[nana:team] worker startup resolution: model=${resolvedModel} thinking_level=${thinkingLevel} source=${source}`);
  }
...
function resolveEffectiveWorkerCliForStartupLog(
  ...
  const rawCliMap = String(env.NANA_TEAM_WORKER_CLI_MAP ?? '').trim();
  if (rawCliMap !== '') {
...
export async function startTeam(
  ...
  let sessionName = `nana-team-${sanitized}`;
  ...
  const sharedWorkerLaunchArgs = resolveTeamWorkerLaunchArgs({
    existingRaw: process.env.NANA_TEAM_WORKER_LAUNCH_ARGS,
  ...
  try {
    ...
    config = await initTeamState(
      ...
      leaderCwd,
      DEFAULT_MAX_WORKERS,
      { ...process.env, NANA_TEAM_DISPLAY_MODE: displayMode, NANA_TEAM_WORKER_LAUNCH_MODE: workerLaunchMode },
      {
    ...
    const workerStartups = workerBootstrapPlans.map((plan) => {
      ...
      if (plan.workerWorkspace.worktreePath) {
        env.NANA_TEAM_WORKTREE_PATH = plan.workerWorkspace.worktreePath;
      }
      if (plan.workerWorkspace.worktreeBranch) {
        env.NANA_TEAM_WORKTREE_BRANCH = plan.workerWorkspace.worktreeBranch;
      }
      if (typeof plan.workerWorkspace.worktreeDetached === 'boolean') {
        env.NANA_TEAM_WORKTREE_DETACHED = plan.workerWorkspace.worktreeDetached ? '1' : '0';
      }
      return {
    ...
    for (let i = 1; i <= workerCount; i++) {
      ...
      if (workerTasks.map(t => t.role).filter(Boolean).length > 0 && new Set(workerTasks.map(t => t.role).filter(Boolean)).size > 1) {
        console.log(`[nana:team] ${workerName}: mixed task roles [${[...new Set(workerTasks.map(t => t.role).filter(Boolean))].join(', ')}], falling back to ${agentType}`);
      }
...
export async function shutdownTeam(teamName: string, cwd: string, options: ShutdownOptions = {}): Promise<void> {
  ...
  if (!config) {
    ...
    try {
      destroyTeamSession(`nana-team-${sanitized}`);
    } catch (err) {
      process.stderr.write(`[team/runtime] operation failed: ${err}\n`);
    }
...
async function findActiveTeams(cwd: string, leaderSessionId: string): Promise<string[]> {
  const root = join(cwd, '.nana', 'state', 'team');
  ...
  for (const e of entries) {
    ...
    const workerLaunchMode = cfg?.worker_launch_mode
      ...
      ?? 'interactive';
    const tmuxSession = (manifest?.tmux_session || cfg?.tmux_session || `nana-team-${teamName}`).split(':')[0];
    if (leaderSessionId) {
...
async function resolveLeaderSessionId(cwd: string): Promise<string> {
  const fromEnv = process.env.NANA_SESSION_ID || process.env.CODEX_SESSION_ID || process.env.SESSION_ID;
  if (fromEnv && fromEnv.trim() !== '') return fromEnv.trim();

  const p = join(cwd, '.nana', 'state', 'session.json');
