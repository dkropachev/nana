...
export function hookPluginRootDir(cwd: string, pluginName: string): string {
  return join(cwd, '.nana', 'state', 'hooks', 'plugins', sanitizeHookPluginName(pluginName));
}
...
export function nanaRootStateFilePath(cwd: string, fileName: string): string {
  return join(cwd, '.nana', 'state', fileName);
}
