...
describe('hooksDir', () => {
  it('returns .nana/hooks under cwd', () => {
    assert.equal(hooksDir('/project'), join('/project', '.nana', 'hooks'));
  });
});
...
describe('ensureHooksDir', () => {
  it('creates .nana/hooks directory and returns path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-ensure-'));
    try {
      const dir = await ensureHooksDir(cwd);
      assert.equal(dir, join(cwd, '.nana', 'hooks'));
...
describe('discoverHookPlugins', () => {
  it('returns empty array when hooks directory does not exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-discover-'));
    try {
...
describe('validateHookPluginExport', () => {
  it('returns valid for plugin with onHookEvent export', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-validate-'));
    try {
...
describe('loadHookPluginDescriptors', () => {
  it('returns empty array when no hooks directory', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-load-'));
    try {
