/**
 * Session Lifecycle Manager for nana
 *
...
import { nanaStateDir, nanaLogsDir } from '../utils/paths.js';
...
function sessionPath(cwd: string): string {
  return join(nanaStateDir(cwd), SESSION_FILE);
}

function historyPath(cwd: string): string {
  return join(nanaLogsDir(cwd), HISTORY_FILE);
}
...
export async function resetSessionMetrics(cwd: string): Promise<void> {
  ...
  await writeFile(join(nanaDir, 'metrics.json'), JSON.stringify({
    total_turns: 0,
    session_turns: 0,
    last_activity: now,
...
export async function appendToLog(cwd: string, entry: Record<string, unknown>): Promise<void> {
  ...
  const logFile = join(logsDir, `nana-${date}.jsonl`);
