...
describe('session lifecycle manager', () => {
  it('resets session metrics files with zeroed counters', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-session-metrics-'));
    try {
      await resetSessionMetrics(cwd);

      const metricsPath = join(cwd, '.nana', 'metrics.json');
      const hudPath = join(cwd, '.nana', 'state', 'hud-state.json');
  ...
  it('writes session start/end lifecycle artifacts and archives session history', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-session-lifecycle-'));
    ...
    try {
      ...
      const historyPath = join(cwd, '.nana', 'logs', 'session-history.jsonl');
      ...
      const dailyLogPath = join(cwd, '.nana', 'logs', `nana-${todayIsoDate()}.jsonl`);
  ...
  it('treats invalid session JSON as absent state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-session-invalid-'));
    try {
      const statePath = join(cwd, '.nana', 'state', 'session.json');
      await resetSessionMetrics(cwd);
...
describe('isSessionStale', () => {
  it('returns false for a live Linux process when identity matches', () => {
    const state = makeState({
      pid_start_ticks: 111,
      pid_cmdline: 'node nana',
    });

    const stale = isSessionStale(state, {
      platform: 'linux',
      isPidAlive: () => true,
      readLinuxIdentity: () => ({ startTicks: 111, cmdline: 'node nana' }),
    });
  ...
  it('returns true for PID reuse on Linux when start ticks mismatch', () => {
    ...
    const stale = isSessionStale(state, {
      platform: 'linux',
      isPidAlive: () => true,
      readLinuxIdentity: () => ({ startTicks: 222, cmdline: 'node nana' }),
    });
