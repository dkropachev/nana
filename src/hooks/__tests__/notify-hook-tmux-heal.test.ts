...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'nana-notify-tmux-heal-'));
  try {
    await run(cwd);
  } finally {
...
describe('notify-hook tmux target healing', () => {
  it('falls back to global mode state when scoped session has no allowed active mode', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const sessionId = 'nana-abc123';
      const sessionStateDir = join(stateDir, 'sessions', sessionId);
      const fakeBinDir = join(cwd, 'fake-bin');
      const fakeTmuxPath = join(fakeBinDir, 'tmux');
      const configPath = join(nanaDir, 'tmux-hook.json');
      ...
      await writeJson(configPath, {
        ...
        prompt_template: 'Continue [NANA_TMUX_INJECT]',
...
`;
  ...
  it('heals a stale HUD pane target back to the canonical codex pane', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const stateDir = join(nanaDir, 'state');
      const logsDir = join(nanaDir, 'logs');
      const sessionId = 'nana-hud-stale';
      const sessionStateDir = join(stateDir, 'sessions', sessionId);
      const fakeBinDir = join(cwd, 'fake-bin');
      const fakeTmuxPath = join(fakeBinDir, 'tmux');
      const configPath = join(nanaDir, 'tmux-hook.json');
...
`;
  ...
  it('skips injection when the resolved pane is still busy', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const stateDir = join(nanaDir, 'state');
      const logsDir = join(nanaDir, 'logs');
      const sessionId = 'nana-busy-pane';
      const sessionStateDir = join(stateDir, 'sessions', sessionId);
      const fakeBinDir = join(cwd, 'fake-bin');
      const fakeTmuxPath = join(fakeBinDir, 'tmux');
      const configPath = join(nanaDir, 'tmux-hook.json');
