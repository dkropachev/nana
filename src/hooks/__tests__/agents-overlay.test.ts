...
const RUNTIME_START = "<!-- NANA:RUNTIME:START -->";
const RUNTIME_END = "<!-- NANA:RUNTIME:END -->";
const WORKER_START = "<!-- NANA:TEAM:WORKER:START -->";
const WORKER_END = "<!-- NANA:TEAM:WORKER:END -->";

async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "nana-overlay-test-"));
  await mkdir(join(dir, ".nana", "state"), { recursive: true });
  return dir;
}
...
describe("generateOverlay", () => {
  ...
  it("generates overlay with no state files (empty but valid)", async () => {
    ...
    assert.ok(overlay.includes("<!-- NANA:RUNTIME:START -->"));
    assert.ok(overlay.includes("<!-- NANA:RUNTIME:END -->"));
  ...
  it("adds advisory explore routing guidance by default and hides it only on explicit opt-out", async () => {
    const previous = process.env.USE_NANA_EXPLORE_CMD;
    try {
      delete process.env.USE_NANA_EXPLORE_CMD;
      ...
      assert.match(defaultOverlay, /nana explore` FIRST before attempting full code analysis/i);

      process.env.USE_NANA_EXPLORE_CMD = "off";
      ...
      if (typeof previous === "string")
        process.env.USE_NANA_EXPLORE_CMD = previous;
      else delete process.env.USE_NANA_EXPLORE_CMD;
    }
  });

  it("generates overlay with active modes", async () => {
    const sessionId = "test-session-2";
    const sessionDir = join(tempDir, ".nana", "state", "sessions", sessionId);
  ...
  it("generates overlay with session-scoped active modes for current session", async () => {
    await mkdir(join(tempDir, ".nana", "state", "sessions", "sess1"), {
      recursive: true,
    });
    await writeFile(
      join(tempDir, ".nana", "state", "sessions", "sess1", "team-state.json"),
      JSON.stringify({
        active: true,
        iteration: 1,
        max_iterations: 5,
  ...
  it("generates overlay with project memory summary", async () => {
    await writeFile(
      join(tempDir, ".nana", "project-memory.json"),
      JSON.stringify({
  ...
  it("skips inactive modes", async () => {
    await writeFile(
      join(tempDir, ".nana", "state", "autopilot-state.json"),
  ...
  it("adds blocked ralph planning gate when PRD/test spec are missing", async () => {
    ...
    await mkdir(join(tempDir, ".nana", "plans"), { recursive: true });
  ...
  it("unlocks ralph planning gate when PRD and test spec exist", async () => {
    ...
    const plansDir = join(tempDir, ".nana", "plans");
...
describe("resolveSessionOrchestrationMode", () => {
  ...
  it("falls back to root team skill state only when no session-scoped skill state exists", async () => {
    ...
    await writeFile(
      join(tempDir, ".nana", "state", "skill-active-state.json"),
      JSON.stringify({ active: true, skill: "team" }),
    );
...
`;
  ...
  it("applyOverlay is idempotent (apply twice, no duplication)", async () => {
    ...
    const startCount = (secondApply.match(/<!-- NANA:RUNTIME:START -->/g) || [])
      .length;
  ...
  it("handles stale markers from previous session", async () => {
    ...
    const startCount = (result.match(/<!-- NANA:RUNTIME:START -->/g) || [])
      .length;
    assert.equal(startCount, 1);
  });
...
describe("session-scoped model instructions file", () => {
  ...
  it("writes user + project AGENTS.md + runtime overlay into session-scoped file", async () => {
    ...
    assert.match(sessionContent, /<!-- NANA:RUNTIME:START -->/);
  ...
  it("writes overlay-only session file when no base AGENTS.md files exist", async () => {
    ...
    assert.ok(sessionContent.includes("<!-- NANA:RUNTIME:START -->"));
    assert.ok(sessionContent.includes("<!-- NANA:RUNTIME:END -->"));
  });
...
describe("hasOverlay", () => {
  ...
  it("returns false when only start marker", () => {
    assert.ok(!hasOverlay("<!-- NANA:RUNTIME:START -->\nbroken"));
  });
});
