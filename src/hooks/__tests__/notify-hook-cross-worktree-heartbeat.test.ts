...
async function withTempDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'nana-notify-cross-worktree-'));
  try {
    await run(cwd);
  } finally {
...
function runWorkerNotify(
  ...
  const inheritedEnv: NodeJS.ProcessEnv = {
    ...process.env,
    NANA_TEAM_WORKER: teamWorker,
    TMUX: '',
    TMUX_PANE: '',
  };
  if (!Object.prototype.hasOwnProperty.call(extraEnv, 'NANA_TEAM_STATE_ROOT')) {
    delete inheritedEnv.NANA_TEAM_STATE_ROOT;
  }
  if (!Object.prototype.hasOwnProperty.call(extraEnv, 'NANA_TEAM_LEADER_CWD')) {
    delete inheritedEnv.NANA_TEAM_LEADER_CWD;
  }
...
describe('notify-hook cross-worktree heartbeat resolution', () => {
  it('writes heartbeat under NANA_TEAM_STATE_ROOT even when payload cwd is a different worktree', async () => {
    await withTempDir(async (root) => {
      ...
      const leaderWorkerDir = join(leaderCwd, '.nana', 'state', 'team', teamName, 'workers', workerName);
      await mkdir(leaderWorkerDir, { recursive: true });
      await mkdir(workerCwd, { recursive: true });

      const result = runWorkerNotify(workerCwd, `${teamName}/${workerName}`, {
        NANA_TEAM_STATE_ROOT: join(leaderCwd, '.nana', 'state'),
      });
      ...
      const wrongHeartbeatPath = join(workerCwd, '.nana', 'state', 'team', teamName, 'workers', workerName, 'heartbeat.json');
  ...
  it('keeps leader-owned heartbeat state when worker cwd uses the team worktree layout', async () => {
    await withTempDir(async (root) => {
      ...
      const workerName = 'worker-1';
      const workerCwd = join(leaderCwd, '.nana', 'team', teamName, 'worktrees', workerName);
  ...
  it('falls back to worker identity/config metadata when NANA_TEAM_STATE_ROOT is absent', async () => {
    await withTempDir(async (root) => {
      const leaderCwd = join(root, 'leader');
      const workerCwd = join(root, 'worker-worktree');
      const teamName = 'cross-meta';
      const workerName = 'worker-1';
      const teamStateRoot = join(leaderCwd, '.nana', 'state');
