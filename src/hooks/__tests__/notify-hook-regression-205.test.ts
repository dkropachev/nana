...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'nana-regression-205-'));
  try {
    await run(cwd);
  } finally {
...
function runNotifyHook(
  ...
  return spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
    ...
    env: {
      ...
      NANA_SESSION_ID: 'sess-managed-regression',
      TMUX_PANE: '%99',
      TMUX: '1',
      NANA_TEAM_WORKER: '',
...
describe('regression-205: detectStallPattern matches "if you want"', () => {
  ...
  it('ignores NANA injection-marker lines when matching patterns', async () => {
    const { detectStallPattern, DEFAULT_STALL_PATTERNS } = await loadModule('notify-hook/auto-nudge.js');
    assert.equal(
      detectStallPattern('keep going [NANA_TMUX_INJECT]', DEFAULT_STALL_PATTERNS),
      false,
    );
  });
...
describe('regression-205: notify-hook records pending stall state on "if you want" by default', () => {
  ...
  before(() => {
    originalTeamWorker = process.env.NANA_TEAM_WORKER;
    originalTeamStateRoot = process.env.NANA_TEAM_STATE_ROOT;
    delete process.env.NANA_TEAM_WORKER;
    delete process.env.NANA_TEAM_STATE_ROOT;
  });

  after(() => {
    if (originalTeamWorker === undefined) delete process.env.NANA_TEAM_WORKER;
    else process.env.NANA_TEAM_WORKER = originalTeamWorker;
    if (originalTeamStateRoot === undefined) delete process.env.NANA_TEAM_STATE_ROOT;
    else process.env.NANA_TEAM_STATE_ROOT = originalTeamStateRoot;
  });
  ...
  it('records pending stall state instead of injecting immediately', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.doesNotMatch(
        tmuxLog,
        /send-keys -t %99 -l yes, proceed \[NANA_TMUX_INJECT\]/,
...
describe('regression-205: resolveTeamStateDirForWorker is exported from team-worker.js', () => {
  ...
  it('uses NANA_TEAM_STATE_ROOT env var when set', async () => {
    const { resolveTeamStateDirForWorker } = await loadModule('notify-hook/team-worker.js');
    const saved = process.env.NANA_TEAM_STATE_ROOT;
    ...
    try {
      ...
      if (saved === undefined) {
        ...
        process.env.NANA_TEAM_STATE_ROOT = saved;
      }
    }
  });

  it('falls back to {cwd}/.nana/state when no env var and no team dir exists', async () => {
    const { resolveTeamStateDirForWorker } = await loadModule('notify-hook/team-worker.js');
    const savedRoot = process.env.NANA_TEAM_STATE_ROOT;
    const savedLeader = process.env.NANA_TEAM_LEADER_CWD;
    delete process.env.NANA_TEAM_STATE_ROOT;
    ...
    try {
      ...
      assert.equal(result, join(cwd, '.nana', 'state'));
    } finally {
      if (savedRoot === undefined) {
        delete process.env.NANA_TEAM_STATE_ROOT;
      } else {
        process.env.NANA_TEAM_STATE_ROOT = savedRoot;
      }
      if (savedLeader === undefined) {
        delete process.env.NANA_TEAM_LEADER_CWD;
      } else {
        process.env.NANA_TEAM_LEADER_CWD = savedLeader;
      }
    }
  });
});
