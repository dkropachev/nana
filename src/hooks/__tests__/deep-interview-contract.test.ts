...
describe("deep-interview Ouroboros contract", () => {
	...
	it("uses NANA-native output paths", () => {
		assert.match(deepInterviewSkill, /\.nana\/interviews\//);
		assert.match(deepInterviewSkill, /\.nana\/specs\//);
	});
	...
	it("requires preflight context intake before interview rounds", () => {
		...
		assert.match(
			deepInterviewSkill,
			/\.nana\/context\/\{slug\}-\{timestamp\}\.md/,
		);
	...
	it("documents the autoresearch specialization contract", () => {
		...
		assert.match(
			deepInterviewSkill,
			/\.nana\/specs\/deep-interview-autoresearch-\{slug\}\.md/i,
		);
