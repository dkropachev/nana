/**
 * Tests for issue #215: tmux scrollback preservation during NANA output injection.
 *
...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'nana-notify-scroll-'));
  try {
    await run(cwd);
  } finally {
...
async function setupFixture(cwd: string, paneInMode: '0' | '1', skipIfScrolling = true) {
  ...
  const sessionId = 'nana-scroll-test';
  ...
  await writeJson(join(nanaDir, 'tmux-hook.json'), {
    enabled: true,
