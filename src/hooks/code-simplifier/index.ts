 ...
 *
 * Opt-in via ~/.nana/config.json: { "codeSimplifier": { "enabled": true } }
...
/** Global NANA config shape (subset relevant to code-simplifier) */
interface NanaGlobalConfig {
...
/**
 * Read the global NANA config from ~/.nana/config.json.
 * Returns null if the file does not exist or cannot be parsed.
 *
 * @param configDir - Optional override for the home directory. When provided,
 *   the config is read from `<configDir>/.nana/config.json` instead of
 *   `~/.nana/config.json`. Useful for testing without relying on `os.homedir()`.
 */
export function readNanaConfig(configDir?: string): NanaGlobalConfig | null {
  const configPath = join(configDir ?? homedir(), '.nana', 'config.json');
  ...
  try {
    return JSON.parse(readFileSync(configPath, 'utf-8')) as NanaGlobalConfig;
  } catch {
    return null;
  }
}
...
export function isCodeSimplifierEnabled(configDir?: string): boolean {
  const config = readNanaConfig(configDir);
