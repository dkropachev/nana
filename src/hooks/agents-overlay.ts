/**
 * AGENTS.md Runtime Overlay for nana
 *
...
const START_MARKER = "<!-- NANA:RUNTIME:START -->";
const END_MARKER = "<!-- NANA:RUNTIME:END -->";
const WORKER_START_MARKER = "<!-- NANA:TEAM:WORKER:START -->";
const WORKER_END_MARKER = "<!-- NANA:TEAM:WORKER:END -->";
...
function lockPath(cwd: string): string {
  return join(cwd, ".nana", "state", "agents-md.lock");
}
...
async function readNotepadPriority(cwd: string): Promise<string> {
  const notePath = nanaNotepadPath(cwd);
...
async function readProjectMemorySummary(cwd: string): Promise<string> {
  const memPath = nanaProjectMemoryPath(cwd);
...
export async function writeSessionModelInstructionsFile(
  ...
  const projectAgentsRoot = process.env.NANA_PROJECT_AGENTS_ROOT?.trim() || cwd;
  const sourcePaths = [join(codexHome(), "AGENTS.md"), join(projectAgentsRoot, "AGENTS.md")];
