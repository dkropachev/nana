...
import { nanaPlansDir } from '../utils/paths.js';
...
export function readPlanningArtifacts(cwd: string): PlanningArtifacts {
  const plansDir = nanaPlansDir(cwd);
  const specsDir = join(cwd, '.nana', 'specs');
...
export function readApprovedExecutionLaunchHint(
  ...
  if (mode === 'team') {
    const teamPattern = /(?<command>(?:nana\s+team|\$team)\s+(?<ralph>ralph\s+)?(?<count>\d+)(?::(?<role>[a-z][a-z0-9-]*))?\s+(?<task>"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'))/gi;
  ...
  const ralphPattern = /(?<command>(?:nana\s+ralph|\$ralph)\s+(?<task>"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'))/gi;
