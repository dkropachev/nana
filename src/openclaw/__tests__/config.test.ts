...
describe("getOpenClawConfig", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = join(tmpdir(), `nana-openclaw-test-${Date.now()}`);
  ...
  it("returns null when NANA_OPENCLAW is not set", async () => {
    delete process.env.NANA_OPENCLAW;
  ...
  it("returns null when NANA_OPENCLAW !== '1'", async () => {
    process.env.NANA_OPENCLAW = "0";
    const { getOpenClawConfig, resetOpenClawConfigCache } = await import("../config.js");
    resetOpenClawConfigCache();
    const result = getOpenClawConfig();
    assert.equal(result, null);
  ...
  it("returns null when NANA_OPENCLAW_CONFIG file does not exist", async () => {
    process.env.NANA_OPENCLAW = "1";
    process.env.NANA_OPENCLAW_CONFIG = join(tmpDir, "nonexistent.json");
    const { getOpenClawConfig, resetOpenClawConfigCache } = await import("../config.js");
    resetOpenClawConfigCache();
    const result = getOpenClawConfig();
    assert.equal(result, null);
  ...
  it("reads config from NANA_OPENCLAW_CONFIG override file", async () => {
    process.env.NANA_OPENCLAW = "1";
    ...
    process.env.NANA_OPENCLAW_CONFIG = configPath;
    const { getOpenClawConfig, resetOpenClawConfigCache } = await import("../config.js");
    resetOpenClawConfigCache();
    const result = getOpenClawConfig();
...
describe("getOpenClawConfig generic alias normalization", () => {
  ...
  beforeEach(() => {
    originalEnv = { ...process.env };
    tmpDir = join(tmpdir(), `nana-openclaw-alias-test-${Date.now()}`);
    mkdirSync(tmpDir, { recursive: true });
    process.env.NANA_OPENCLAW = "1";
    process.env.HOME = tmpDir;
    process.env.CODEX_HOME = join(tmpDir, ".codex");
    delete process.env.NANA_OPENCLAW_CONFIG;
  });
  ...
  it("normalizes custom_webhook_command alias to openclaw runtime config", async () => {
    const nanaConfigPath = join(tmpDir, ".codex", ".nana-config.json");
    mkdirSync(join(tmpDir, ".codex"), { recursive: true });
    writeFileSync(nanaConfigPath, JSON.stringify({
      notifications: {
        enabled: true,
