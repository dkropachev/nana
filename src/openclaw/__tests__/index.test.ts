...
describe("wakeOpenClaw", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = join(tmpdir(), `nana-openclaw-index-test-${Date.now()}`);
  ...
  it("returns result with success:false for disabled command gateway", async () => {
    process.env.NANA_OPENCLAW = "1";
    delete process.env.NANA_OPENCLAW_COMMAND;
    const configPath = join(tmpDir, "openclaw.json");
    writeFileSync(configPath, JSON.stringify({
      enabled: true,
    ...
    assert.ok(result!.error?.includes("NANA_OPENCLAW_COMMAND"));
  });
  ...
  it("succeeds with command gateway when both env vars set", async () => {
    process.env.NANA_OPENCLAW = "1";
    process.env.NANA_OPENCLAW_COMMAND = "1";
    const configPath = join(tmpDir, "openclaw.json");
    writeFileSync(configPath, JSON.stringify({
      enabled: true,
