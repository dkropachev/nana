 ...
 *
 * Reads OpenClaw config from the notifications.openclaw key in ~/.codex/.nana-config.json.
 ...
 * Config file path can be overridden via NANA_OPENCLAW_CONFIG env var (points to a separate file).
 */
  ...
  if (cliEnabled && cliAlias) {
    ...
    applyHooks(
      ...
      parseInstruction(
        ...
        "NANA event {{event}} for {{projectPath}}",
      ),
      "custom_cli_command",
    );
  }
 ...
 *
 * Returns null when:
 * - NANA_OPENCLAW env var is not "1"
 * - Config file does not exist
 * - Config file is invalid JSON
 * - Config has enabled: false
 *
 * Config is read from:
 * 1. NANA_OPENCLAW_CONFIG env var path (separate file), if set
 * 2. notifications.openclaw key in ~/.codex/.nana-config.json
...
export function getOpenClawConfig(): OpenClawConfig | null {
  // Activation gate: only active when NANA_OPENCLAW=1
  if (process.env.NANA_OPENCLAW !== "1") {
    return null;
  }
  ...
  try {
    const envOverride = process.env.NANA_OPENCLAW_CONFIG;

    if (envOverride) {
      // NANA_OPENCLAW_CONFIG points to a separate config file
    ...
    // Primary: read from notifications block in .nana-config.json
    const nanaConfigPath = join(codexHome(), ".nana-config.json");
    if (!existsSync(nanaConfigPath)) {
      _cachedConfig = undefined;
      return null;
    }

    const fullConfig = JSON.parse(readFileSync(nanaConfigPath, "utf-8")) as Record<string, unknown>;
