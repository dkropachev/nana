 ...
 *
 * SECURITY: Command gateway requires NANA_OPENCLAW_COMMAND=1 opt-in.
 ...
 * gateway timeout > NANA_OPENCLAW_COMMAND_TIMEOUT_MS > default.
 */
export function resolveCommandTimeoutMs(
  gatewayTimeout?: number,
  envTimeoutRaw: string | undefined = process.env.NANA_OPENCLAW_COMMAND_TIMEOUT_MS,
): number {
 ...
 * - Requires NANA_OPENCLAW_COMMAND=1 opt-in (separate gate from NANA_OPENCLAW)
 * - Timeout is configurable via gateway.timeout or NANA_OPENCLAW_COMMAND_TIMEOUT_MS
...
export async function wakeCommandGateway(
  ...
  if (process.env.NANA_OPENCLAW_COMMAND !== "1") {
    return {
      gateway: gatewayName,
      success: false,
      error: "Command gateway disabled (set NANA_OPENCLAW_COMMAND=1 to enable)",
    };
  }
