...
describe('CI Rust gates', () => {
  ...
  it('uses the current crates/nana-sparkshell manifest for Rust coverage', () => {
    const workflowPath = join(process.cwd(), '.github', 'workflows', 'ci.yml');
    const workflow = readFileSync(workflowPath, 'utf-8');

    assert.match(workflow, /crates\/nana-sparkshell\/Cargo\.toml/);
    assert.doesNotMatch(workflow, /native\/nana-sparkshell\/Cargo\.toml/);
  });
  ...
  it('uses the current sparkshell crate manifest in the Rust coverage lane', () => {
    ...
    assert.match(workflow, /cargo llvm-cov --manifest-path crates\/nana-sparkshell\/Cargo\.toml --summary-only/);
    assert.match(workflow, /cargo llvm-cov --manifest-path crates\/nana-sparkshell\/Cargo\.toml --lcov --output-path coverage\/rust\/nana-sparkshell\.lcov/);
    assert.doesNotMatch(workflow, /native\/nana-sparkshell\/Cargo\.toml/);
  });
});
