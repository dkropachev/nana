...
describe('state paths', () => {
  ...
  it('enforces NANA_MCP_WORKDIR_ROOTS allowlist when configured', async () => {
    const allowedRoot = await mkdtemp(join(tmpdir(), 'nana-allowed-root-'));
    const disallowedRoot = await mkdtemp(join(tmpdir(), 'nana-disallowed-root-'));
    const prev = process.env.NANA_MCP_WORKDIR_ROOTS;
    process.env.NANA_MCP_WORKDIR_ROOTS = allowedRoot;
    try {
      assert.equal(
      ...
      assert.throws(
        ...
        /outside allowed roots \(NANA_MCP_WORKDIR_ROOTS\)/,
      );
    } finally {
      if (typeof prev === 'string') process.env.NANA_MCP_WORKDIR_ROOTS = prev;
      else delete process.env.NANA_MCP_WORKDIR_ROOTS;
  ...
  it('builds global state paths', () => {
    ...
    assert.equal(base, '/repo/.nana/state');
    assert.equal(getStateDir('/repo'), '/repo/.nana/state');
    assert.equal(getStatePath('team', '/repo'), '/repo/.nana/state/team-state.json');
  });

  it('builds session state paths', () => {
    assert.equal(getStateDir('/repo', 'sess1'), '/repo/.nana/state/sessions/sess1');
    assert.equal(
      getStatePath('ralph', '/repo', 'sess1'),
      '/repo/.nana/state/sessions/sess1/ralph-state.json'
    );
  });
  ...
  it('enumerates global-only path', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-state-paths-'));
    try {
