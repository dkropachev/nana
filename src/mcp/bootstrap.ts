...
const SERVER_DISABLE_ENV: Record<McpServerName, string> = {
  state: 'NANA_STATE_SERVER_DISABLE_AUTO_START',
  memory: 'NANA_MEMORY_SERVER_DISABLE_AUTO_START',
  code_intel: 'NANA_CODE_INTEL_SERVER_DISABLE_AUTO_START',
  trace: 'NANA_TRACE_SERVER_DISABLE_AUTO_START',
  team: 'NANA_TEAM_SERVER_DISABLE_AUTO_START',
};

const GLOBAL_DISABLE_ENV = 'NANA_MCP_SERVER_DISABLE_AUTO_START';
...
export function autoStartStdioMcpServer(
  ...
  const shutdown = async () => {
    ...
    try {
      ...
      console.error(`[nana-${serverName}-server] shutdown failed`, error);
    }
  };
