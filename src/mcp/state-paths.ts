...
const WORKDIR_ALLOWLIST_ENV = 'NANA_MCP_WORKDIR_ROOTS';
...
export function getBaseStateDir(workingDirectory?: string): string {
  if ((workingDirectory == null || workingDirectory === '') && typeof process.env.NANA_TEAM_STATE_ROOT === 'string' && process.env.NANA_TEAM_STATE_ROOT.trim() !== '') {
    try {
      return resolveWorkingDirectoryForState(process.env.NANA_TEAM_STATE_ROOT.trim());
    } catch {}
  }
  return join(resolveWorkingDirectoryForState(workingDirectory), '.nana', 'state');
}
