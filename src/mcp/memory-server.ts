/**
 * NANA Project Memory & Notepad MCP Server
 * Provides persistent project memory and session notepad tools
 * Storage: .nana/project-memory.json, .nana/notepad.md
 */
...
function getMemoryPath(wd: string): string {
  return join(wd, '.nana', 'project-memory.json');
}

function getNotepadPath(wd: string): string {
  return join(wd, '.nana', 'notepad.md');
}
...
const server = new Server(
  { name: 'nana-memory', version: '0.1.0' },
  { capabilities: { tools: {} } }
);
