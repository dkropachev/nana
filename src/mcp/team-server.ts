/**
 * NANA Team Runner MCP Server
 * Provides nana_run_team_* tools for spawning and managing tmux CLI worker teams.
 * Storage: ~/.nana/team-jobs/
 */
...
const jobIdSchema = z.string().regex(/^nana-[a-z0-9]{1,12}$/);
...
const nanaTeamJobs = new Map<string, NanaTeamJob>();

function getNanaJobsDir(): string {
  return join(homedir(), '.nana', 'team-jobs');
}

function persistJob(jobId: string, job: NanaTeamJob): void {
  try {
    const jobsDir = getNanaJobsDir();
...
function loadJobFromDisk(jobId: string): NanaTeamJob | undefined {
  try {
    return JSON.parse(readFileSync(join(getNanaJobsDir(), `${jobId}.json`), 'utf-8')) as NanaTeamJob;
  } catch (err) {
...
async function loadPaneIds(jobId: string): Promise<{ paneIds: string[]; leaderPaneId: string } | null> {
  try {
    const parsed = JSON.parse(await readFile(join(getNanaJobsDir(), `${jobId}-panes.json`), 'utf-8')) as {
      paneIds?: unknown;
...
async function resolveCleanupTargets(jobId: string, job: NanaTeamJob): Promise<CleanupSelection> {
...
const server = new Server(
  { name: 'nana-team-server', version: '0.1.0' },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    {
      name: 'nana_run_team_start',
      description: 'Spawn tmux CLI workers (codex/claude/gemini) in the background. Returns jobId immediately. Poll with nana_run_team_status.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'nana_run_team_status',
      description: 'Non-blocking status check for a background nana_run_team job. Returns status and result when done.',
      inputSchema: {
        type: 'object',
        properties: {
          job_id: { type: 'string', description: 'Job ID returned by nana_run_team_start' },
        },
        required: ['job_id'],
      },
    },
    {
      name: 'nana_run_team_wait',
      description: 'Block (poll internally) until a background nana_run_team job reaches a terminal state (completed or failed) or, in wake_on=event mode, until the next team event arrives. Uses exponential backoff (500ms to 2000ms). Auto-nudges idle teammate panes via tmux send-keys. If this wait call times out, workers are left running -- call nana_run_team_wait again to keep waiting, or nana_run_team_cleanup to stop them explicitly.',
      inputSchema: {
        type: 'object',
        properties: {
          job_id: { type: 'string', description: 'Job ID returned by nana_run_team_start' },
...
export async function handleTeamToolCall(request: {
  ...
  try {
    switch (name) {
      case 'nana_run_team_start': {
        const { teamName, agentTypes, tasks, cwd: inputCwd } = startSchema.parse(a);

        const jobId = `nana-${Date.now().toString(36)}`;
        const runtimeCliPath = join(dirname(fileURLToPath(import.meta.url)), '..', 'team', 'runtime-cli.js');

        const job: NanaTeamJob = { status: 'running', startedAt: Date.now(), teamName, cwd: inputCwd };
        nanaTeamJobs.set(jobId, job);

        const child = spawn('node', [runtimeCliPath], {
          env: { ...process.env, NANA_JOB_ID: jobId, NANA_JOBS_DIR: getNanaJobsDir() },
        ...
        return {
          content: [{ type: 'text' as const, text: JSON.stringify({ jobId, pid: job.pid, message: 'Team started. Poll with nana_run_team_status.' }) }],
        };
      }

      case 'nana_run_team_status': {
        const { job_id: jobId } = statusSchema.parse(a);
        const job = nanaTeamJobs.get(jobId) ?? loadJobFromDisk(jobId);
        if (!job) {
      ...
      case 'nana_run_team_wait': {
        ...
        const elapsed = ((Date.now() - (nanaTeamJobs.get(jobId)?.startedAt ?? Date.now())) / 1000).toFixed(1);
        const timeoutOut: Record<string, unknown> = {
          error: `Timed out waiting for job ${jobId} after ${(timeoutMs / 1000).toFixed(0)}s -- workers are still running; call nana_run_team_wait again to keep waiting or nana_run_team_cleanup to stop them`,
          jobId,
          status: 'running',
          wake_on: wakeOn,
          elapsedSeconds: elapsed,
