...
describe('ensureCanonicalRalphArtifacts', () => {
  it('keeps canonical files authoritative when they already exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-ralph-canonical-'));
    try {
      const canonicalPrd = join(cwd, '.nana', 'plans', 'prd-existing.md');
      const canonicalProgress = join(cwd, '.nana', 'state', 'ralph-progress.json');
      await mkdir(join(cwd, '.nana', 'plans'), { recursive: true });
      await mkdir(join(cwd, '.nana', 'state'), { recursive: true });
      ...
      await writeFile(join(cwd, '.nana', 'prd.json'), JSON.stringify({ project: 'legacy-project' }));
      await writeFile(join(cwd, '.nana', 'progress.txt'), 'legacy line\n');
  ...
  it('migrates legacy PRD/progress files one-way when canonical artifacts are absent', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-ralph-migrate-'));
    try {
      const legacyPrdPath = join(cwd, '.nana', 'prd.json');
      const legacyProgressPath = join(cwd, '.nana', 'progress.txt');
      await mkdir(join(cwd, '.nana'), { recursive: true });
      ...
      assert.match(canonicalPrd, /Migrated from legacy `.nana\/prd\.json`/);
      assert.equal(canonicalProgress.source, '.nana/progress.txt');
  ...
  it('records visual feedback with numeric and qualitative guidance for the next iteration', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nana-ralph-visual-feedback-'));
    try {
