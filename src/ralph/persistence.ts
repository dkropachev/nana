...
const LEGACY_PRD_PATH = '.nana/prd.json';
const LEGACY_PROGRESS_PATH = '.nana/progress.txt';
...
async function writeMigrationMarker(
  ...
  const markerPath = join(cwd, '.nana', 'plans', 'ralph-migration-marker.json');
...
async function migrateLegacyPrdIfNeeded(
  ...
  const markdown = [
    `# ${title}`,
    '',
    '> Migrated from legacy `.nana/prd.json` (read-only compatibility import).',
    '',
    '## Migration Marker',
