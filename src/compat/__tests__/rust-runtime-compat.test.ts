...
async function withTempTeamStateRoot<T>(
  ...
  const previousRoot = process.env.NANA_TEAM_STATE_ROOT;
  process.env.NANA_TEAM_STATE_ROOT = teamStateRoot;
  try {
    return await fn();
  } finally {
    if (previousRoot === undefined) delete process.env.NANA_TEAM_STATE_ROOT;
    else process.env.NANA_TEAM_STATE_ROOT = previousRoot;
  }
}
...
describe('rust runtime legacy-reader compatibility', () => {
  ...
  it('keeps doctor --team on the manifest-authored tmux session', async () => {
    ...
    try {
      ...
      await withTempTeamStateRoot(teamStateRoot, async () => {
        ...
        config.tmux_session = 'nana-team-legacy-rust-compat-doctor';
        manifest.tmux_session = 'nana-team-rust-compat-doctor';
        ...
        await writeFile(
          tmuxPath,
          '#!/bin/sh\nif [ "$1" = "list-sessions" ]; then echo "nana-team-rust-compat-doctor"; exit 0; fi\nexit 0\n',
        );
        ...
        const result = runNana(
          wd,
          ['doctor', '--team'],
          { PATH: `${fakeBin}:${process.env.PATH || ''}`, NANA_TEAM_STATE_ROOT: teamStateRoot },
        );
        if (shouldSkipForSpawnPermissions(result.error)) return;
  ...
  it('keeps HUD team state on the session-scoped compatibility file', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-rust-compat-hud-'));
    try {
      const stateRoot = join(wd, '.nana', 'state');
  ...
  it('prefers bridge-authored dispatch/mailbox compatibility views over stale legacy files', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-rust-compat-bridge-'));
    ...
    try {
      ...
      await withTempTeamStateRoot(teamStateRoot, async () => {
        ...
        const fakeBin = join(wd, 'bin');
        await mkdir(fakeBin, { recursive: true });
        const runtimePath = join(fakeBin, 'nana-runtime');
        await writeCompatRuntimeFixture(runtimePath);
        process.env.NANA_RUNTIME_BINARY = runtimePath;
