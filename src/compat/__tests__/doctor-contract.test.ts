...
const defaultTarget = join(repoRoot, 'dist', 'cli', 'nana.js');
...
function resolveCompatTarget(): { command: string; argsPrefix: string[] } {
  const override = process.env.NANA_COMPAT_TARGET?.trim();
...
function normalizeInstallDoctorOutput(text: string, home: string, cwd: string): string {
  const repoStateDir = join(cwd, '.nana', 'state').replace(/\\/g, '/');
  return text
    ...
    .map((line) => {
      ...
      if (line.startsWith('Run "nana setup')) {
...
describe('compat doctor contract', () => {
  it('matches onboarding warning copy for first setup expectations', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-compat-doctor-'));
    const home = join(wd, 'home');
    const codexHome = join(home, '.codex');
    await mkdir(codexHome, { recursive: true });
    await writeFile(join(codexHome, 'config.toml'), '[mcp_servers.non_nana]\ncommand = "node"\n');
  ...
  it('matches doctor --team resume_blocker behavior', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-compat-doctor-team-'));
    try {
      const teamRoot = join(wd, '.nana', 'state', 'team', 'alpha');
      await mkdir(join(teamRoot, 'workers', 'worker-1'), { recursive: true });
      await writeFile(join(teamRoot, 'config.json'), JSON.stringify({ name: 'alpha', tmux_session: 'nana-team-alpha' }));
