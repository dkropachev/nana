 ...
 *
 * Config key : notifications.dispatchCooldownSeconds in ~/.codex/.nana-config.json
 * Env var    : NANA_DISPATCH_COOLDOWN_SECONDS  (overrides config)
 * State file : .nana/state/dispatch-notif-cooldown.json
 ...
 *
 * Resolution order:
 *   1. NANA_DISPATCH_COOLDOWN_SECONDS env var
 *   2. notifications.dispatchCooldownSeconds in ~/.codex/.nana-config.json
 *   3. Default: 60 seconds
 */
export function getDispatchNotificationCooldownSeconds(): number {
  // 1. Environment variable override
  const envVal = process.env.NANA_DISPATCH_COOLDOWN_SECONDS;
  if (envVal !== undefined) {
  ...
  // 2. Config file
  try {
    const configPath = join(codexHome(), '.nana-config.json');
