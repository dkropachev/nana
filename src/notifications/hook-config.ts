 ...
 *
 * Reads hookTemplates from .nana-config.json for user-customizable message templates.
 * Config is stored under the notifications.hookTemplates key in codexHome()/.nana-config.json.
 * Env var NANA_HOOK_CONFIG overrides to a separate file path.
 */
 ...
 *
 * Primary source: notifications.hookTemplates key in codexHome()/.nana-config.json
 * Env var override: NANA_HOOK_CONFIG points to a separate file containing the
...
export function getHookConfig(): HookNotificationConfig | null {
  ...
  const envOverridePath = process.env.NANA_HOOK_CONFIG;
  ...
  // Primary: read from notifications.hookTemplates in .nana-config.json
  const NANA_CONFIG_PATH = join(codexHome(), ".nana-config.json");
  if (!existsSync(NANA_CONFIG_PATH)) {
    cachedConfig = null;
    return null;
  }

  try {
    const raw = JSON.parse(readFileSync(NANA_CONFIG_PATH, "utf-8"));
 ...
 * - hook event `enabled: false` overrides .nana-config.json event `enabled: true`
 * - Platform credentials are NOT affected (they stay in .nana-config.json)
 */
