...
describe('analyzePaneContent', () => {
  ...
  it('detects "nana" keyword', () => {
    const result = analyzePaneContent('nana session started');
    assert.equal(result.hasCodex, true);
  });

  it('detects "nana" keyword', () => {
    const result = analyzePaneContent('nana v1.0');
    assert.equal(result.hasCodex, true);
  });
  ...
  it('caps confidence at 1.0', () => {
    const result = analyzePaneContent('Codex $ > agent task running nana');
