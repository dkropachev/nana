...
describe('getTeamTmuxSessions - session matching', () => {
  ...
  function makeFakeTmux(sessions: string[]): void {
    const fakeBinDir = mkdtempSync(join(tmpdir(), 'nana-tmux-team-test-'));
  ...
  it('returns canonical session name (nana-team-alpha) for team "alpha"', () => {
    makeFakeTmux(['nana-team-alpha', 'other-session']);
    assert.deepEqual(getTeamTmuxSessions('alpha'), ['nana-team-alpha']);
  });

  it('returns prefixed worker sessions (nana-team-alpha-worker1)', () => {
    makeFakeTmux(['nana-team-alpha-worker1', 'nana-team-alpha-worker2']);
    assert.deepEqual(getTeamTmuxSessions('alpha'), ['nana-team-alpha-worker1', 'nana-team-alpha-worker2']);
  });

  it('does NOT return sessions for a different team', () => {
    makeFakeTmux(['nana-team-beta', 'nana-team-beta-worker1']);
...
describe('captureTmuxPane', () => {
  ...
  it('captures output for valid pane id and sanitizes/clamps lines', () => {
    const fakeBinDir = mkdtempSync(join(tmpdir(), 'nana-tmux-test-'));
    tmpDirs.push(fakeBinDir);
    const tmuxPath = join(fakeBinDir, 'tmux');
    writeFileSync(
      tmuxPath,
