...
describe('loadNotificationConfig', () => {
  it('returns null when config file does not exist', async () => {
    const fakePath = join(tmpdir(), `nana-test-${randomUUID()}`);
    const config = await loadNotificationConfig(fakePath);
    assert.equal(config, null);
  });

  it('returns parsed config when file exists', async () => {
    const tmpDir = join(tmpdir(), `nana-test-${randomUUID()}`);
    const nanaDir = join(tmpDir, '.nana');
    mkdirSync(nanaDir, { recursive: true });
    ...
    writeFileSync(join(nanaDir, 'notifications.json'), JSON.stringify(configData));
  ...
  it('returns null for invalid JSON', async () => {
    ...
    const nanaDir = join(tmpDir, '.nana');
    mkdirSync(nanaDir, { recursive: true });
    writeFileSync(join(nanaDir, 'notifications.json'), 'not-json');
