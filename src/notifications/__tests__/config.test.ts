...
const ENV_KEYS = [
  ...
  'NANA_DISCORD_MENTION',
  'NANA_TELEGRAM_BOT_TOKEN',
  'NANA_TELEGRAM_NOTIFIER_BOT_TOKEN',
  'NANA_TELEGRAM_CHAT_ID',
  'NANA_TELEGRAM_NOTIFIER_CHAT_ID',
  'NANA_TELEGRAM_NOTIFIER_UID',
  'NANA_SLACK_WEBHOOK_URL',
  'NANA_REPLY_ENABLED',
  'NANA_REPLY_DISCORD_USER_IDS',
  'NANA_REPLY_POLL_INTERVAL_MS',
  'NANA_REPLY_RATE_LIMIT',
];
...
describe('buildConfigFromEnv', () => {
  ...
  it('builds discord-bot config from env vars', () => {
    process.env.NANA_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.NANA_DISCORD_NOTIFIER_CHANNEL = '123456';
  ...
  it('includes validated mention in discord-bot config', () => {
    process.env.NANA_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.NANA_DISCORD_NOTIFIER_CHANNEL = '123456';
    process.env.NANA_DISCORD_MENTION = '<@12345678901234567>';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('rejects invalid mention in env var', () => {
    process.env.NANA_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.NANA_DISCORD_NOTIFIER_CHANNEL = '123456';
    process.env.NANA_DISCORD_MENTION = '@everyone';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds discord webhook config from env var', () => {
    process.env.NANA_DISCORD_WEBHOOK_URL = 'https://discord.com/api/webhooks/test';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds telegram config from env vars', () => {
    process.env.NANA_TELEGRAM_BOT_TOKEN = '123:abc';
    process.env.NANA_TELEGRAM_CHAT_ID = '999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds slack config from env var', () => {
    process.env.NANA_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('uses NANA_TELEGRAM_NOTIFIER_BOT_TOKEN as fallback', () => {
    process.env.NANA_TELEGRAM_NOTIFIER_BOT_TOKEN = '123:fallback';
    process.env.NANA_TELEGRAM_CHAT_ID = '999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('uses NANA_TELEGRAM_NOTIFIER_UID as fallback for chat ID', () => {
    process.env.NANA_TELEGRAM_BOT_TOKEN = '123:abc';
    process.env.NANA_TELEGRAM_NOTIFIER_UID = 'uid-999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds config with multiple platforms from env', () => {
    process.env.NANA_DISCORD_NOTIFIER_BOT_TOKEN = 'bot-token';
    process.env.NANA_DISCORD_NOTIFIER_CHANNEL = 'channel-123';
    process.env.NANA_TELEGRAM_BOT_TOKEN = '456:tg';
    process.env.NANA_TELEGRAM_CHAT_ID = 'chat-789';
    process.env.NANA_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
