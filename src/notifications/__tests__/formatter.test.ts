...
describe('parseTmuxTail', () => {
  ...
  it('keeps wrapped Korean continuation lines in the same block', () => {
    const raw = [
      ...
      'block 10: 2. 아예 ~/.codex/.nana-config.json에서 조절 가능하',
      '  게 로컬 패치하기',
    ].join('\n');

    const result = parseTmuxTail(raw);
    assert.ok(result.includes('block 10: 2. 아예 ~/.codex/.nana-config.json에서 조절 가능하'));
    assert.ok(result.includes('  게 로컬 패치하기'));
    assert.ok(
      result.indexOf('block 10: 2. 아예 ~/.codex/.nana-config.json에서 조절 가능하') <
  ...
  it('removes NANA HUD status lines', () => {
    const raw = [
      '[NANA#3] ultrawork active',
      '[NANA] idle',
      'Normal output line',
    ].join('\n');
    const result = parseTmuxTail(raw);
    assert.ok(!result.includes('[NANA'));
