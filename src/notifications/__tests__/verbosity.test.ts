...
describe('getVerbosity', () => {
  const origEnv = process.env.NANA_NOTIFY_VERBOSITY;

  afterEach(() => {
    if (origEnv === undefined) {
      delete process.env.NANA_NOTIFY_VERBOSITY;
    } else {
      process.env.NANA_NOTIFY_VERBOSITY = origEnv;
    }
  });
