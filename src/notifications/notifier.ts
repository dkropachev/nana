...
/**
 * Load notification config from .nana/notifications.json
 */
export async function loadNotificationConfig(projectRoot?: string): Promise<NotificationConfig | null> {
  const configPath = join(projectRoot || process.cwd(), '.nana', 'notifications.json');
...
export function _buildDesktopArgs(
  ...
  if (platform === 'darwin') {
    ...
    const ps =
      ...
      `[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('nana').Show($xml)`;
...
async function sendDiscordNotification(payload: NotificationPayload, webhookUrl: string): Promise<void> {
  ...
  const body = JSON.stringify({
    embeds: [{
      title: `[NANA] ${payload.title}`,
      description: payload.message,
      color: colorMap[payload.type],
      footer: { text: `nana | ${payload.mode || 'general'}` },
