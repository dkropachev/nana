 ...
 *
 * Config key : notifications.idleCooldownSeconds in ~/.codex/.nana-config.json
 * Env var    : NANA_IDLE_COOLDOWN_SECONDS  (overrides config)
 * State file : .nana/state/idle-notif-cooldown.json
 ...
 *
 * Resolution order:
 *   1. NANA_IDLE_COOLDOWN_SECONDS env var
 *   2. notifications.idleCooldownSeconds in ~/.codex/.nana-config.json
 *   3. Default: 60 seconds
 */
export function getIdleNotificationCooldownSeconds(): number {
  // 1. Environment variable override
  const envVal = process.env.NANA_IDLE_COOLDOWN_SECONDS;
  if (envVal !== undefined) {
