export const NANA_NOTIFY_TEMP_ENV = 'NANA_NOTIFY_TEMP';
export const NANA_NOTIFY_TEMP_CONTRACT_ENV = 'NANA_NOTIFY_TEMP_CONTRACT';
...
export function parseNotifyTempContractFromArgs(
  ...
  const envActivated = env[NANA_NOTIFY_TEMP_ENV] === '1';
...
export function isNotifyTempEnvActive(env: NodeJS.ProcessEnv = process.env): boolean {
  return env[NANA_NOTIFY_TEMP_ENV] === '1';
}

export function readNotifyTempContractFromEnv(
  env: NodeJS.ProcessEnv = process.env,
): NotifyTempContract | null {
  const raw = env[NANA_NOTIFY_TEMP_CONTRACT_ENV];
  if (!raw) return null;
  try {
