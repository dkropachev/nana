...
const AUTORESEARCH_WORKTREE_EXCLUDES = ['results.tsv', 'run.log', 'node_modules', '.nana/'];
...
function activeRunStateFile(projectRoot: string): string {
  return join(projectRoot, '.nana', 'state', 'autoresearch-state.json');
}
...
export function buildAutoresearchInstructions(
  ...
  return [
    '# NANA Autoresearch Supervisor Instructions',
    '',
...
export async function loadAutoresearchRunManifest(projectRoot: string, runId: string): Promise<AutoresearchRunManifest> {
  const manifestFile = join(projectRoot, '.nana', 'logs', 'autoresearch', runId, 'manifest.json');
...
export async function prepareAutoresearchRuntime(
  ...
  const runDir = join(projectRoot, '.nana', 'logs', 'autoresearch', runId);
