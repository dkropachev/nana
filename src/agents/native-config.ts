...
const DELIVERY_CLASS_OVERLAYS: Record<AgentDefinition["deliveryClass"], string> = {
  reviewer: [
    "<delivery_class_guidance>",
    "",
    "You are operating in the reviewer delivery class.",
    "- Default contract: inspect, diagnose, and recommend with grounded evidence; do not own code edits or test-writing by default.",
    "- In split reviewer/executor workflows, stop at findings, remediation guidance, and clear handoff notes for the executor lane.",
    "- If a separate executor lane exists, do not silently absorb its implementation work just because the fix seems obvious.",
    "- If you are explicitly wrapped by a merged reviewer+executor surface, keep reviewer rigor first and let the paired executor contract apply the resulting changes.",
    "",
    "</delivery_class_guidance>",
  ].join("\n"),
  executor: [
    "<delivery_class_guidance>",
    "",
    "You are operating in the executor delivery class.",
    "- Default contract: own the edits, tests, and verification required to finish the accepted scope.",
    "- In split reviewer/executor workflows, consume grounded reviewer findings as implementation requirements unless fresh repo evidence disproves them.",
    "- Do not pretend to be the reviewer of record when a separate reviewer lane exists; implement and verify instead.",
    "- If you are explicitly wrapped by a merged reviewer+executor surface, perform the review pass first, then execute and verify the resulting changes yourself.",
    "",
    "</delivery_class_guidance>",
  ].join("\n"),
};
...
interface RoleInstructionMetadata {
  name: string;
  deliveryClass: AgentDefinition["deliveryClass"];
...
export function composeRoleInstructions(
  ...
  if (metadata) {
    parts.push(
      "",
      DELIVERY_CLASS_OVERLAYS[metadata.deliveryClass],
      "",
  ...
  if (metadata) {
    metadataLines.push(
      "## NANA Agent Metadata",
      `- role: ${metadata.name}`,
      `- delivery_class: ${metadata.deliveryClass}`,
  ...
  if (resolvedModel) {
    if (metadataLines.length === 0) {
      metadataLines.push("## NANA Agent Metadata");
    }
...
export function composeRoleInstructionsForRole(
  ...
  return composeRoleInstructions(
    ...
    agent
      ? {
          name: agent.name,
          deliveryClass: agent.deliveryClass,
...
/**
 * Generate TOML content for a prompt-backed NANA role agent.
 */
