...
/** Default NANA-managed user Codex home directory (~/.nana/codex-home/) */
export function defaultUserCodexHome(baseHome?: string): string {
  return join(baseHome || homedir(), ".nana", "codex-home");
}

/** Legacy Codex user home directory (~/.codex/) used as migration/import source */
export function legacyDefaultUserCodexHome(baseHome?: string): string {
  return join(baseHome || homedir(), ".codex");
}

/** Codex CLI home directory */
export function codexHome(): string {
  return process.env.CODEX_HOME || defaultUserCodexHome();
}

/** Codex auth file path */
export function codexAuthPath(codexHomeDir?: string): string {
  return join(codexHomeDir || codexHome(), "auth.json");
}
...
/** User-level skills directory ($CODEX_HOME/skills) */
...
/** nana state directory (.nana/state/) */
export function nanaStateDir(projectRoot?: string): string {
  return join(projectRoot || process.cwd(), ".nana", "state");
}

/** nana project memory file (.nana/project-memory.json) */
export function nanaProjectMemoryPath(projectRoot?: string): string {
  return join(projectRoot || process.cwd(), ".nana", "project-memory.json");
}

/** nana notepad file (.nana/notepad.md) */
export function nanaNotepadPath(projectRoot?: string): string {
  return join(projectRoot || process.cwd(), ".nana", "notepad.md");
}

/** nana plans directory (.nana/plans/) */
export function nanaPlansDir(projectRoot?: string): string {
  return join(projectRoot || process.cwd(), ".nana", "plans");
}

/** nana logs directory (.nana/logs/) */
export function nanaLogsDir(projectRoot?: string): string {
  return join(projectRoot || process.cwd(), ".nana", "logs");
}
