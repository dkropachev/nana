...
export const NANA_MODELS_START_MARKER = '<!-- NANA:MODELS:START -->';
export const NANA_MODELS_END_MARKER = '<!-- NANA:MODELS:END -->';
...
export function buildAgentsModelTable(
  ...
  return [
    ...
    '',
    'Auto-generated by `nana setup` from the current `config.toml` plus NANA model overrides.',
    '',
...
export function upsertAgentsModelTable(
  ...
  const startIndex = content.indexOf(NANA_MODELS_START_MARKER);
  const endIndex = content.indexOf(NANA_MODELS_END_MARKER);

  if (startIndex >= 0 && endIndex > startIndex) {
    const replaceEnd = endIndex + NANA_MODELS_END_MARKER.length;
