...
describe("codexHome", () => {
  ...
  it("defaults to the NANA-managed user Codex home when CODEX_HOME is not set", () => {
    delete process.env.CODEX_HOME;
    assert.equal(codexHome(), join(homedir(), ".nana", "codex-home"));
    assert.equal(codexHome(), defaultUserCodexHome());
  });
});

describe("legacyDefaultUserCodexHome", () => {
  it("returns the legacy ~/.codex location", () => {
    assert.equal(legacyDefaultUserCodexHome("/tmp/home"), join("/tmp/home", ".codex"));
  });
});

describe("codexAuthPath", () => {
  it("returns auth.json under codex home", () => {
    assert.equal(codexAuthPath("/tmp/test-codex"), join("/tmp/test-codex", "auth.json"));
  });
});
...
describe("listInstalledSkillDirectories", () => {
  ...
  it("deduplicates by skill name and prefers project skills over user skills", async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), "nana-paths-project-"));
    const codexHomeRoot = await mkdtemp(join(tmpdir(), "nana-paths-codex-"));
  ...
  it("detects overlapping legacy and canonical user skill roots including content mismatches", async () => {
    const homeRoot = await mkdtemp(join(tmpdir(), "nana-paths-home-"));
  ...
  it("treats a legacy link to canonical skills as the same resolved target", async () => {
    const homeRoot = await mkdtemp(join(tmpdir(), "nana-paths-linked-home-"));
    const codexHomeRoot = join(homeRoot, ".codex");
...
describe("nanaStateDir", () => {
  it("uses provided projectRoot", () => {
    assert.equal(nanaStateDir("/my/project"), join("/my/project", ".nana", "state"));
  });

  it("defaults to cwd when no projectRoot given", () => {
    assert.equal(nanaStateDir(), join(process.cwd(), ".nana", "state"));
  });
});

describe("nanaProjectMemoryPath", () => {
  it("uses provided projectRoot", () => {
    assert.equal(
      nanaProjectMemoryPath("/my/project"),
      join("/my/project", ".nana", "project-memory.json"),
    );
  });
  ...
  it("defaults to cwd when no projectRoot given", () => {
    assert.equal(
      nanaProjectMemoryPath(),
      join(process.cwd(), ".nana", "project-memory.json"),
    );
  });
});

describe("nanaNotepadPath", () => {
  it("uses provided projectRoot", () => {
    assert.equal(nanaNotepadPath("/my/project"), join("/my/project", ".nana", "notepad.md"));
  });

  it("defaults to cwd when no projectRoot given", () => {
    assert.equal(nanaNotepadPath(), join(process.cwd(), ".nana", "notepad.md"));
  });
});

describe("nanaPlansDir", () => {
  it("uses provided projectRoot", () => {
    assert.equal(nanaPlansDir("/my/project"), join("/my/project", ".nana", "plans"));
  });

  it("defaults to cwd when no projectRoot given", () => {
    assert.equal(nanaPlansDir(), join(process.cwd(), ".nana", "plans"));
  });
});

describe("nanaLogsDir", () => {
  it("uses provided projectRoot", () => {
    assert.equal(nanaLogsDir("/my/project"), join("/my/project", ".nana", "logs"));
  });

  it("defaults to cwd when no projectRoot given", () => {
    assert.equal(nanaLogsDir(), join(process.cwd(), ".nana", "logs"));
  });
});
