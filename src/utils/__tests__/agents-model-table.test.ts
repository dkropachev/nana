...
import {
  ...
  NANA_MODELS_END_MARKER,
  NANA_MODELS_START_MARKER,
  resolveAgentsModelTableContext,
  upsertAgentsModelTable,
} from '../agents-model-table.js';

const originalFrontierEnv = process.env.NANA_DEFAULT_FRONTIER_MODEL;
const originalStandardEnv = process.env.NANA_DEFAULT_STANDARD_MODEL;
const originalSparkEnv = process.env.NANA_DEFAULT_SPARK_MODEL;
const originalLegacySparkEnv = process.env.NANA_SPARK_MODEL;
...
afterEach(() => {
  if (typeof originalFrontierEnv === 'string') {
    process.env.NANA_DEFAULT_FRONTIER_MODEL = originalFrontierEnv;
  } else {
    delete process.env.NANA_DEFAULT_FRONTIER_MODEL;
  }
  if (typeof originalStandardEnv === 'string') {
    process.env.NANA_DEFAULT_STANDARD_MODEL = originalStandardEnv;
  } else {
    delete process.env.NANA_DEFAULT_STANDARD_MODEL;
  }
  if (typeof originalSparkEnv === 'string') {
    process.env.NANA_DEFAULT_SPARK_MODEL = originalSparkEnv;
  } else {
    delete process.env.NANA_DEFAULT_SPARK_MODEL;
  }
  if (typeof originalLegacySparkEnv === 'string') {
    process.env.NANA_SPARK_MODEL = originalLegacySparkEnv;
  } else {
    delete process.env.NANA_SPARK_MODEL;
  }
});
...
describe('agents model table', () => {
  it('resolves frontier from config.toml, standard from environment, and spark from environment', () => {
    process.env.NANA_DEFAULT_FRONTIER_MODEL = 'frontier-env';
    process.env.NANA_DEFAULT_STANDARD_MODEL = 'standard-env';
    process.env.NANA_DEFAULT_SPARK_MODEL = 'spark-env';
  ...
  it('replaces existing marker-bounded content and inserts the block after team_model_resolution when missing', () => {
    ...
    assert.match(
      inserted,
      /<\/team_model_resolution>\n\n<!-- NANA:MODELS:START -->[\s\S]*<!-- NANA:MODELS:END -->\n\n---/,
    );
  });
});
