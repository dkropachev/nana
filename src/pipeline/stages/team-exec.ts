 ...
 * canonical NANA execution surface.
 */
 ...
 *
 * This stage delegates to the existing `nana team` infrastructure, which
...
/**
 * Build the `nana team` CLI instruction from a descriptor.
 */
export function buildTeamInstruction(descriptor: TeamExecDescriptor): string {
  const launchCommand = `nana team ${descriptor.workerCount}:${descriptor.agentType} ${JSON.stringify(descriptor.task)}`;
