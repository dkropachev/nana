...
async function setup(): Promise<string> {
  tempDir = await mkdtemp(join(tmpdir(), 'nana-pipeline-test-'));
  return tempDir;
}
...
describe('Pipeline Orchestrator', () => {
  ...
  describe('runPipeline', () => {
    ...
    it('persists pipeline state to mode state file', async () => {
      ...
      const statePath = join(tempDir, '.nana', 'state', 'autopilot-state.json');
  ...
  describe('canResumePipeline', () => {
    ...
    it('returns true when pipeline state is active and in-progress', async () => {
      ...
      const stateDir = join(tempDir, '.nana', 'state');
