/**
 * Config.toml generator/merger for nana
 * Merges NANA MCP server entries and feature flags into existing config.toml
 *
...
// ---------------------------------------------------------------------------
// Top-level NANA keys (must live before any [table] header)
// ---------------------------------------------------------------------------

/** Keys we own at the TOML root level. Used for upsert + strip. */
const NANA_TOP_LEVEL_KEYS = [
  "notify",
...
const SHARED_MCP_REGISTRY_MARKER = "nana (NANA) Shared MCP Registry Sync";
const SHARED_MCP_REGISTRY_END_MARKER =
  "# End nana shared MCP registry sync";
const NANA_AGENTS_MAX_THREADS = 6;
const NANA_AGENTS_MAX_DEPTH = 2;
const NANA_EXPLORE_ROUTING_DEFAULT = '1';
const NANA_EXPLORE_CMD_ENV = 'USE_NANA_EXPLORE_CMD';
const NANA_TUI_STATUS_LINE =
...
function stripRootLevelKeys(config: string, keys: readonly string[]): string {
  ...
  if (
    keys.some((key) =>
      NANA_TOP_LEVEL_KEYS.includes(key as (typeof NANA_TOP_LEVEL_KEYS)[number]),
    )
  ) {
    lines = lines.filter(
      (l) =>
...
/**
 * Remove any existing NANA-owned top-level keys so we can re-insert them
 * cleanly. Also removes the comment line that precedes them.
 */
export function stripNanaTopLevelKeys(config: string): string {
  return stripRootLevelKeys(config, NANA_TOP_LEVEL_KEYS);
}
...
function upsertEnvSettings(config: string): string {
  ...
  if (envStart < 0) {
    ...
    const envBlock = [
      "[env]",
      `${NANA_EXPLORE_CMD_ENV} = "${NANA_EXPLORE_ROUTING_DEFAULT}"`,
      "",
    ].join("\n");
  ...
  for (let i = envStart + 1; i < sectionEnd; i++) {
    if (new RegExp(`^\\s*${NANA_EXPLORE_CMD_ENV}\\s*=`).test(lines[i])) {
      exploreRoutingIdx = i;
      break;
    }
  }
...
function upsertAgentsSettings(config: string): string {
  ...
  if (agentsStart < 0) {
    ...
    const agentsBlock = [
      "[agents]",
      `max_threads = ${NANA_AGENTS_MAX_THREADS}`,
      `max_depth = ${NANA_AGENTS_MAX_DEPTH}`,
      "",
    ].join("\n");
  ...
  if (maxThreadsIdx < 0) {
    lines.splice(sectionEnd, 0, `max_threads = ${NANA_AGENTS_MAX_THREADS}`);
    sectionEnd += 1;
  }
  if (maxDepthIdx < 0) {
    lines.splice(sectionEnd, 0, `max_depth = ${NANA_AGENTS_MAX_DEPTH}`);
  }
...
/**
 * Remove NANA-owned feature flags from the [features] section.
 * If the section becomes empty after removal, remove the section header too.
 */
export function stripNanaFeatureFlags(config: string): string {
  const lines = config.split(/\r?\n/);
  const featuresStart = lines.findIndex((line) =>
    /^\s*\[features\]\s*$/.test(line),
  );
  ...
  const nanaFlags = ["multi_agent", "child_agents_md", "collab"];
  const filtered: string[] = [];
  for (let i = 0; i < lines.length; i++) {
    if (i > featuresStart && i < sectionEnd) {
      const isNanaFlag = nanaFlags.some((f) =>
        new RegExp(`^\\s*${f}\\s*=`).test(lines[i]),
      );
      if (isNanaFlag) continue;
    }
...
export function stripNanaEnvSettings(config: string): string {
  const lines = config.split(/\r?\n/);
  const envStart = lines.findIndex((line) => /^\s*\[env\]\s*$/.test(line));
  ...
  for (let i = 0; i < lines.length; i++) {
    if (i > envStart && i < sectionEnd) {
      const isNanaEnvKey = new RegExp(
        `^\\s*${NANA_EXPLORE_CMD_ENV}\\s*=`,
      ).test(lines[i]);
      if (isNanaEnvKey) continue;
    }
    filtered.push(lines[i]);
  }
...
// ---------------------------------------------------------------------------
// Orphaned NANA table sections (no marker block)
// ---------------------------------------------------------------------------

/**
 * Check whether a TOML table name belongs to a legacy NANA-managed agent entry.
 * Handles both `agents.name` and `agents."name"` forms.
 */
function isLegacyNanaAgentSection(tableName: string): boolean {
...
/**
 * Strip NANA-owned table sections that exist outside the marker block.
 ...
 *
 * Targets: [mcp_servers.nana_*], legacy [agents.<name>] entries, [tui]
 */
function stripOrphanedNanaSections(config: string): string {
  const lines = config.split(/\r?\n/);
  const result: string[] = [];
  ...
  while (i < lines.length) {
    ...
    if (tableMatch) {
      ...
      // The marker-based stripExistingNanaBlocks already handles [tui]
      // when it lives inside the NANA marker block.
      const isNanaSection =
        /^mcp_servers\.nana_/.test(tableName) ||
        isLegacyNanaAgentSection(tableName);

      if (isNanaSection) {
        // Remove preceding NANA comment lines and blank lines
        while (result.length > 0) {
          const last = result[result.length - 1];
          if (last.trim() === "" || /^#\s*(NANA|nana)/i.test(last)) {
            result.pop();
          } else {
            break;
          }
...
function upsertTuiStatusLine(config: string): {
  ...
  const mergedSection = ["[tui]", ...preservedKeyLines, NANA_TUI_STATUS_LINE];
...
// ---------------------------------------------------------------------------
// NANA [table] sections block (appended at end of file)
// ---------------------------------------------------------------------------

export function stripExistingNanaBlocks(config: string): {
  cleaned: string;
  removed: number;
} {
  const marker = "nana (NANA) Configuration";
  const endMarker = "# End nana";
  let cleaned = config;
  let removed = 0;
...
function getSharedMcpRegistryBlock(
  ...
  const lines = [
    ...
    "# Managed by nana setup - edit the registry file instead",
  ];
  if (sourcePath) {
...
/**
 * NANA table-section block (MCP servers, TUI).
 * Contains ONLY [table] sections — no bare keys.
 */
function getNanaTablesBlock(pkgRoot: string, includeTui = true): string {
  ...
  return [
    ...
    "startup_timeout_sec = 5",
    "",
    "# NANA Code Intelligence MCP Server (LSP diagnostics, AST search)",
    ...
    "",
    "# NANA Trace MCP Server (agent flow timeline & statistics)",
    ...
    "startup_timeout_sec = 5",
    "",
    "# NANA Team MCP Server (team job lifecycle: start, status, wait, cleanup)",
    "[mcp_servers.nana_team_run]",
    'command = "node"',
    `args = ["${teamServerPath}"]`,
    "enabled = true",
    "startup_timeout_sec = 5",
...
/**
 * Merge NANA config into existing config.toml
 * Preserves existing user settings, appends NANA block if not present.
 *
 * Layout:
 *   1. NANA top-level keys (notify, model_reasoning_effort, developer_instructions)
 *   2. [features] with multi_agent + child_agents_md
 *   3. [env] with defaulted explore-routing opt-in
 *   4. … user sections …
 *   5. NANA [table] sections (mcp_servers, tui)
 */
export function buildMergedConfig(
  ...
  if (existing.includes("nana (NANA) Configuration")) {
    const stripped = stripExistingNanaBlocks(existing);
  ...
  const topLines = getNanaTopLevelLines(
    pkgRoot,
    existing,
    options.modelOverride,
  );
  const tablesBlock = getNanaTablesBlock(
    pkgRoot,
 ...
 *
 * After an nana version upgrade the OLD setup code (still loaded in memory)
...
export async function mergeConfig(
  ...
  if (existing.includes("nana (NANA) Configuration")) {
    ...
    if (options.verbose && stripped.removed > 0) {
      console.log("  Updating existing NANA config block.");
    }
  }
