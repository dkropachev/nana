 ...
 * Verifies that repeated `nana setup` runs do not duplicate NANA sections.
 */
...
/** Assert the NANA block appears exactly once */
function assertSingleNanaBlock(toml: string): void {
  assert.equal(
    count(toml, /# nana \(NANA\) Configuration/g),
    1,
    "NANA marker should appear once",
  );
  assert.equal(
    count(toml, /# End nana/g),
    1,
    "End marker should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.nana_state\]$/gm),
    1,
    "[mcp_servers.nana_state] should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.nana_memory\]$/gm),
    1,
    "[mcp_servers.nana_memory] should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.nana_code_intel\]$/gm),
    1,
    "[mcp_servers.nana_code_intel] should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.nana_trace\]$/gm),
    1,
    "[mcp_servers.nana_trace] should appear once",
  );
  ...
  assert.equal(
    count(toml, /^USE_NANA_EXPLORE_CMD = "1"$/gm),
    1,
    "USE_NANA_EXPLORE_CMD should appear once",
  );
}

describe("config generator idempotency (#384)", () => {
  it("first run creates config with all NANA sections", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-idem-"));
    try {
      const configPath = join(wd, "config.toml");
      await mergeConfig(configPath, wd);
      const toml = await readFile(configPath, "utf-8");

      assertSingleNanaBlock(toml);
  ...
  it("second run updates without duplicating any section", async () => {
    ...
    try {
      ...
      assertSingleNanaBlock(first);

      // Second run
      await mergeConfig(configPath, wd);
      const second = await readFile(configPath, "utf-8");
      assertSingleNanaBlock(second);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it("cleans up legacy config without markers", async () => {
    ...
    try {
      const configPath = join(wd, "config.toml");
      // Simulate a legacy config written without NANA markers
  ...
  it("cleans up orphaned NANA sections outside marker block", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-idem-"));
    try {
      const configPath = join(wd, "config.toml");
  ...
  it("preserves user content between NANA re-runs", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-idem-"));
    try {
      const configPath = join(wd, "config.toml");
  ...
  it("handles config with only orphaned agents sections", async () => {
    ...
    try {
      ...
      const orphanedAgents = [
        ...
        "multi_agent = true",
        "",
        "# NANA Native Agent Roles (Codex multi-agent)",
        "",
        "[agents.executor]",
      ...
      assert.doesNotMatch(toml, /^\[agents\.executor\]$/m, "legacy NANA agent entry removed");
      assert.doesNotMatch(toml, /^\[agents\.explore\]$/m, "legacy NANA agent entry removed");
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

  it("preserves non-NANA agent sections", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-idem-"));
    try {
      const configPath = join(wd, "config.toml");
      const userAgents = [
  ...
  it("merges NANA status_line into an existing user [tui] section without duplicating the table", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-idem-"));
    try {
      const configPath = join(wd, "config.toml");
      const userTui = [
  ...
  it("skips emitting an NANA [tui] table when includeTui is disabled", () => {
    const toml = buildMergedConfig("", "/tmp/nana", {
      includeTui: false,
    });

    assert.doesNotMatch(toml, /^\[tui\]$/m);
    assert.match(toml, /^\[mcp_servers\.nana_state\]$/m);
    assert.match(toml, /^\[env\]$/m);
    assert.match(toml, /^USE_NANA_EXPLORE_CMD = "1"$/m);
  });

  it('seeds USE_NANA_EXPLORE_CMD=1 into generated config by default', () => {
    const toml = buildMergedConfig('', '/tmp/nana');
  ...
  it('preserves existing [env] keys and explicit explore routing opt-outs', () => {
    const toml = buildMergedConfig(
      ['[env]', 'FOO = "bar"', 'USE_NANA_EXPLORE_CMD = "0"', ''].join('\n'),
      '/tmp/nana',
    );

    assert.match(toml, /^\[env\]$/m);
    assert.match(toml, /^FOO = "bar"$/m);
    assert.match(toml, /^USE_NANA_EXPLORE_CMD = "0"$/m);
  });

  it("replaces an existing NANA notify entry without leaving orphan fragments behind", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-idem-"));
    try {
      const configPath = join(wd, "config.toml");
      const existing = [
  ...
  it("repairs config with duplicate [tui] sections from upgrade", async () => {
    ...
    try {
      const configPath = join(wd, "config.toml");
      // Simulate a broken config left by an older nana setup: an orphaned
      // [tui] outside the NANA block AND another [tui] inside the block.
      const broken = [
        '[mcp_servers.figma]',
  ...
  it("repairConfigIfNeeded fixes duplicate [tui] and is a no-op when clean", async () => {
    ...
    try {
      ...
      assertSingleNanaBlock(repaired);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it("syncs shared MCP registry entries in a dedicated managed block", async () => {
    ...
    try {
      const first = buildMergedConfig("", wd, {
        ...
        sharedMcpRegistrySource: "/tmp/.nana/mcp-registry.json",
      });
      ...
      assert.equal(
        count(second, /nana \(NANA\) Shared MCP Registry Sync/g),
        1,
      ...
      assert.match(second, /# Source: \/tmp\/\.nana\/mcp-registry\.json/);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it("skips shared MCP entries that already exist in user config", () => {
    ...
    const merged = buildMergedConfig(existing, "/tmp/nana", {
      sharedMcpServers: [
        {
          name: "gitnexus",
          command: "gitnexus",
