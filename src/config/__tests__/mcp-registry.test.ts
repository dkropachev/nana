...
describe("unified MCP registry loader", () => {
  it("prefers ~/.nana/mcp-registry.json over ~/.omc/mcp-registry.json", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-mcp-registry-"));
    try {
      const nanaPath = join(wd, ".nana", "mcp-registry.json");
      ...
      assert.equal(result.sourcePath, nanaPath);
  ...
  it("falls back to ~/.omc/mcp-registry.json when ~/.nana registry is absent", async () => {
    const wd = await mkdtemp(join(tmpdir(), "nana-mcp-registry-"));
    try {
      const omcPath = join(wd, ".omc", "mcp-registry.json");
      await mkdir(join(wd, ".omc"), { recursive: true });
  ...
  it("returns canonical home-based registry candidates", () => {
    ...
    assert.deepEqual(candidates, [
      "/tmp/home/.nana/mcp-registry.json",
