...
describe('config generator', () => {
  it('places top-level keys before [features]', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-config-gen-'));
    try {
  ...
  it('writes model_reasoning_effort and strengthened developer_instructions', async () => {
    ...
    try {
      ...
      assert.match(toml, /^developer_instructions = "You have nana installed/m);
  ...
  it('handles paths with spaces in notify array', async () => {
    const base = await mkdtemp(join(tmpdir(), 'nana config gen space-'));
  ...
  it('re-runs setup replacing NANA config cleanly', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-config-gen-'));
    try {
      const configPath = join(wd, 'config.toml');
      await mergeConfig(configPath, wd);
      ...
      // NANA block appears exactly once
      assert.equal(
        (rerun.match(/# nana \(NANA\) Configuration/g) ?? []).length,
        1
      );
      assert.equal((rerun.match(/# End nana/g) ?? []).length, 1);
  ...
  it('writes a global [agents] section with NANA defaults', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-config-gen-'));
    try {
      const configPath = join(wd, 'config.toml');
      await mergeConfig(configPath, wd);
  ...
  it('migrates a legacy NANA block and preserves user settings', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'nana-config-gen-'));
    try {
      const configPath = join(wd, 'config.toml');
      const legacy = [
      ...
      assert.equal(
        (toml.match(/nana \(NANA\) Configuration/g) ?? []).length,
        1
      );
  ...
  it('escapes Windows-style backslashes for MCP server args', async () => {
    ...
    try {
      ...
      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\nana\/dist\/mcp\/state-server\.js"\]/,
      );
      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\nana\/dist\/mcp\/memory-server\.js"\]/,
      );
      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\nana\/dist\/mcp\/code-intel-server\.js"\]/,
      );
      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\nana\/dist\/mcp\/trace-server\.js"\]/,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
