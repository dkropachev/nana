...
async function main() {
  ...
  const teamWorkerEnv = process.env.NANA_TEAM_WORKER; // e.g., "fix-ts/worker-1"
  ...
  if (!isTeamWorker) {
    const metricsPath = join(nanaDir, 'metrics.json');
    try {
      let metrics = {
        total_turns: 0,
        session_turns: 0,
  ...
  // 4. Write HUD state summary for `nana hud` (lead session only)
  if (!isTeamWorker) {
