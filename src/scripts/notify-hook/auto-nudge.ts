...
export async function loadAutoNudgeConfig() {
  const codexHomePath = process.env.CODEX_HOME || defaultUserCodexHome(process.env.HOME?.trim() || homedir());
  const configPath = join(codexHomePath, '.nana-config.json');
...
function resolveCodexPaneByCwdFallback(cwd) {
  ...
  try {
    ...
    for (const line of panes) {
      ...
      if (/\bnana\b.*\bhud\b.*--watch/i.test(normalizedStart)) continue;
...
async function resolveCodexPaneFromAnchor(anchorPane) {
  ...
  try {
    ...
    for (const line of panes) {
      ...
      if (/\bnana\b.*\bhud\b.*--watch/i.test(startCommand)) continue;
...
function buildExpectedManagedTmuxSessionName(cwd, sessionId) {
  ...
  const sessionToken = sanitizeTmuxToken(safeString(sessionId).replace(/^nana-/, ''));
...
async function isManagedNanaSessionForAutoNudge(cwd, payload) {
  if (safeString(process.env.NANA_TEAM_WORKER || '').trim() !== '') return true;
...
export async function maybeAutoNudge({ cwd, stateDir, logsDir, payload }) {
  ...
  const managedSession = await isManagedNanaSessionForAutoNudge(cwd, payload);
  if (!managedSession) {
