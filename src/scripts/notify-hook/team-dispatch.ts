 ...
 * owned path.
 * Disable entirely with NANA_RUNTIME_BRIDGE=0.
 */
function runtimeExec(command, stateDir) {
  if (process.env.NANA_RUNTIME_BRIDGE === '0') return;
  try {
...
const ISSUE_DISPATCH_COOLDOWN_ENV = 'NANA_TEAM_DISPATCH_ISSUE_COOLDOWN_MS';
const DEFAULT_DISPATCH_TRIGGER_COOLDOWN_MS = 30 * 1000;
const DISPATCH_TRIGGER_COOLDOWN_ENV = 'NANA_TEAM_DISPATCH_TRIGGER_COOLDOWN_MS';
...
export async function drainPendingTeamDispatch({
  ...
  logsDir = join(cwd, '.nana', 'logs'),
  maxPerTick = 5,
  injector = injectDispatchRequest,
} = {}) {
  if (safeString(process.env.NANA_TEAM_WORKER)) {
  ...
  for (const teamName of teams) {
    ...
    await withDispatchLock(teamDirPath, async () => {
      const legacyRequests = await readJson(requestsPath, null);
      const bridgeRequests = await readBridgeDispatchRequests(stateDir, teamName);
      const useLegacyOnly = Array.isArray(legacyRequests) && legacyRequests.length > 0;
      const requests = useLegacyOnly
        ? legacyRequests
        : (Array.isArray(bridgeRequests) ? bridgeRequests : (Array.isArray(legacyRequests) ? legacyRequests : []));
      const usingLegacyRequests = true;
