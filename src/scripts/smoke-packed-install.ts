...
function usage(): string {
  return [
    ...
    '',
    'Creates an npm tarball, installs it into an isolated prefix, and smoke tests the installed nana CLI.',
...
async function main(): Promise<void> {
  ...
  const tempRoot = mkdtempSync(join(tmpdir(), 'nana-packed-install-'));
  ...
  try {
    ...
    const nanaPath = join(prefixDir, process.platform === 'win32' ? '' : 'bin', npmBinName('nana'));
    for (const argv of PACKED_INSTALL_SMOKE_CORE_COMMANDS) {
      run(nanaPath, argv, { cwd: repoRoot });
    }
