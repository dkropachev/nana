...
async function main(): Promise<void> {
  ...
  for (const file of ['AGENTS.md', 'templates/AGENTS.md']) {
    ...
    text = replaceBetween(text, '<!-- NANA:GUIDANCE:OPERATING:START -->', '<!-- NANA:GUIDANCE:OPERATING:END -->', op);
    text = replaceBetween(text, '<!-- NANA:GUIDANCE:VERIFYSEQ:START -->', '<!-- NANA:GUIDANCE:VERIFYSEQ:END -->', vs);
    await writeFile(file, text);
  }

  let text = await read('prompts/executor.md');
  text = replaceBetween(text, '<!-- NANA:GUIDANCE:EXECUTOR:CONSTRAINTS:START -->', '<!-- NANA:GUIDANCE:EXECUTOR:CONSTRAINTS:END -->', exC);
  text = replaceBetween(text, '<!-- NANA:GUIDANCE:EXECUTOR:OUTPUT:START -->', '<!-- NANA:GUIDANCE:EXECUTOR:OUTPUT:END -->', exO);
  await writeFile('prompts/executor.md', text);

  text = await read('prompts/planner.md');
  text = replaceBetween(text, '<!-- NANA:GUIDANCE:PLANNER:CONSTRAINTS:START -->', '<!-- NANA:GUIDANCE:PLANNER:CONSTRAINTS:END -->', plC);
  text = replaceBetween(text, '<!-- NANA:GUIDANCE:PLANNER:INVESTIGATION:START -->', '<!-- NANA:GUIDANCE:PLANNER:INVESTIGATION:END -->', plI);
  text = replaceBetween(text, '<!-- NANA:GUIDANCE:PLANNER:OUTPUT:START -->', '<!-- NANA:GUIDANCE:PLANNER:OUTPUT:END -->', plO);
  await writeFile('prompts/planner.md', text);

  text = await read('prompts/verifier.md');
  text = replaceBetween(text, '<!-- NANA:GUIDANCE:VERIFIER:CONSTRAINTS:START -->', '<!-- NANA:GUIDANCE:VERIFIER:CONSTRAINTS:END -->', vfC);
  text = replaceBetween(text, '<!-- NANA:GUIDANCE:VERIFIER:INVESTIGATION:START -->', '<!-- NANA:GUIDANCE:VERIFIER:INVESTIGATION:END -->', vfI);
