...
#   NANA_TEAM_WORKER_CLI             Worker CLI mode (default: auto)
#   NANA_TEAM_WORKER_CLI_MAP         Comma-separated CLI assignments per worker
#   NANA_TEAM_WORKER_LAUNCH_ARGS     Arguments passed to worker CLIs
#
# Example:
...
if [[ -n "${NANA_TEAM_WORKER_CLI_MAP:-}" ]]; then
  IFS=',' read -ra CLI_MAP_ENTRIES <<< "$NANA_TEAM_WORKER_CLI_MAP"
  if (("${#CLI_MAP_ENTRIES[@]}" != WORKER_COUNT)); then
    echo "error: NANA_TEAM_WORKER_CLI_MAP has ${#CLI_MAP_ENTRIES[@]} entries but WORKER_COUNT is $WORKER_COUNT" >&2
    exit 1
  fi
fi
...
NANA_TEAM_WORKER_CLI_MAP="${NANA_TEAM_WORKER_CLI_MAP:-$(build_default_cli_map "$WORKER_COUNT")}"
NANA_TEAM_WORKER_LAUNCH_ARGS="${NANA_TEAM_WORKER_LAUNCH_ARGS:--c model_reasoning_effort=\"low\"}"
...
echo "NANA_TEAM_WORKER_LAUNCH_ARGS=$NANA_TEAM_WORKER_LAUNCH_ARGS"
...
nana team api mailbox-mark-notified --input "$MARK_INPUT" --json >/dev/null
nana team api mailbox-mark-delivered --input "$MARK_INPUT" --json >/dev/null
