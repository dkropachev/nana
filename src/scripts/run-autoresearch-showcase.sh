...
Available showcases:
  nana-self       -> missions/in-action-cat-shellout-demo
  ...
  all            -> nana-self ml-tabular bayesopt latent sorting

Examples:
  scripts/run-autoresearch-showcase.sh --list
  scripts/run-autoresearch-showcase.sh bayesopt
  scripts/run-autoresearch-showcase.sh nana-self ml-tabular
USAGE
}

mission_for() {
  case "$1" in
    nana-self)   printf '%s' 'missions/in-action-cat-shellout-demo' ;;
...
args=("$@")
if [[ " ${args[*]} " == *" all "* ]]; then
  args=(nana-self ml-tabular bayesopt latent sorting)
fi
