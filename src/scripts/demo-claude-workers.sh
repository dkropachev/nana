#!/usr/bin/env bash
#
# NANA Tmux Claude Workers Demo Script
#
...
#   NANA_TEAM_WORKER_LAUNCH_MODE     Worker launch mode (default: interactive)
#   NANA_TEAM_WORKER_LAUNCH_ARGS     Arguments passed to Claude CLI
#
# Example:
...
NANA_TEAM_WORKER_LAUNCH_MODE="${NANA_TEAM_WORKER_LAUNCH_MODE:-interactive}"
...
NANA_TEAM_WORKER_CLI="${NANA_TEAM_WORKER_CLI:-auto}"
NANA_TEAM_WORKER_CLI_MAP="${NANA_TEAM_WORKER_CLI_MAP:-$(build_claude_cli_map "$WORKER_COUNT")}"

TEAM_STARTED=0
cleanup() {
  if ((TEAM_STARTED == 1)); then
    echo "[cleanup] shutting down team: $TEAM_NAME"
    nana team shutdown "$TEAM_NAME" >/dev/null 2>&1 || true
    echo "[cleanup] cleaning state for team: $TEAM_NAME"
    nana team api cleanup --input "{\"team_name\":\"$TEAM_NAME\"}" --json >/dev/null 2>&1 || true
  fi
}
trap cleanup EXIT

echo "== NANA Tmux Claude Workers Demo v${SCRIPT_VERSION} =="
echo "TEAM_TASK=$TEAM_TASK"
echo "TEAM_NAME=$TEAM_NAME"
echo "WORKER_COUNT=$WORKER_COUNT"
echo "NANA_TEAM_WORKER_CLI=$NANA_TEAM_WORKER_CLI"
echo "NANA_TEAM_WORKER_CLI_MAP=$NANA_TEAM_WORKER_CLI_MAP"
echo ""
echo "This demo showcases Claude Code CLI workers in tmux panes"
echo "coordinated through the NANA team orchestration system."
echo ""

echo "[1/10] Starting team with ${WORKER_COUNT} Claude workers..."
nana team "${WORKER_COUNT}:executor" "$TEAM_TASK"
TEAM_STARTED=1
echo ""
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  CREATE_JSON="$(nana team api create-task --input "$CREATE_INPUT" --json)"
...
nana team api list-tasks --input "$LIST_INPUT" --json | jq -r '.data.tasks[] | "  Task \(.id): \(.subject) [\(.status)]"'
echo ""
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  TASK_ID="$(nana team api list-tasks --input "$(jq -nc --arg team "$TEAM_NAME" '{team_name:$team}')" --json | \
  ...
  if [[ -n "$TASK_ID" ]]; then
    ...
    CLAIM_JSON="$(nana team api claim-task --input "$CLAIM_INPUT" --json)"
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  TASK_INFO="$(nana team api list-tasks --input "$(jq -nc --arg team "$TEAM_NAME" '{team_name:$team}')" --json | \
  ...
  if [[ -n "$TASK_INFO" ]]; then
    ...
    nana team api transition-task-status --input "$TRANSITION_INPUT" --json >/dev/null
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  nana team api send-message --input "$SEND_INPUT" --json >/dev/null
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  MAILBOX_JSON="$(nana team api mailbox-list --input "$MAILBOX_INPUT" --json)"
...
BROADCAST_RESULT="$(nana team api broadcast --input "$BROADCAST_INPUT" --json)"
...
SUMMARY_JSON="$(nana team api get-summary --input "$SUMMARY_INPUT" --json)"
...
nana team api cleanup --input "{\"team_name\":\"$TEAM_NAME\"}" --json >/dev/null
TEAM_STARTED=0
echo ""
...
echo ""
echo "The Claude workers were coordinated through the NANA"
