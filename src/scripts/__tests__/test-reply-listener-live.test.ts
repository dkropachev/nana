...
test('resolveReplyListenerLiveEnv stays disabled until explicitly opted in', () => {
  const result = resolveReplyListenerLiveEnv({
    NANA_DISCORD_NOTIFIER_BOT_TOKEN: 'discord-token',
    NANA_DISCORD_NOTIFIER_CHANNEL: 'channel-1',
    NANA_TELEGRAM_BOT_TOKEN: 'telegram-token',
    NANA_TELEGRAM_CHAT_ID: 'chat-1',
  });
