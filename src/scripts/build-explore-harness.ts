...
const binaryName = process.platform === 'win32' ? 'nana-explore-harness.exe' : 'nana-explore-harness';
const sourcePath = join(root, 'target', 'release', binaryName);
const outputPath = join(binDir, binaryName);
const metadataPath = join(binDir, 'nana-explore-harness.meta.json');

const build = spawnSync('cargo', ['build', '--release', '-p', 'nana-explore-harness'], {
  cwd: root,
  encoding: 'utf-8',
