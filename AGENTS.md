...
## Lazy Runtime Skills
Load docs only when invoked. When a listed keyword matches, invoke that `$skill` by reading its RUNTIME.md. Use `nana route --explain "<prompt>"` to preview routing (`nana route --explain <prompt>`). Explicit `$skill` invocations run left-to-right before implicit keyword matches; keyword matches are case-insensitive; `/prompts:<name>` disables implicit keyword activation unless explicit `$skill` tokens are present.
