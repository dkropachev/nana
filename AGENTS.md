...
- final-report checklist: changed files, verification evidence, simplifications made, remaining risks; verify before completion.
...
- `.nana/logs/context-telemetry.ndjson` (1MiB + `.1`): skill_doc_load, skill_reference_load, shell_output_compaction; no args/out.
...
<!-- NANA:MODELS:END -->
- Run `nana setup` for prompts/skills/hooks and write generated `AGENTS.md` to the selected AGENTS target (user scope=Codex home, `./AGENTS.md` for project scope); verify with `nana doctor`.
