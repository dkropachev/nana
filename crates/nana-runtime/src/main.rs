use nana_mux::{canonical_contract_summary, MuxAdapter, MuxOperation, MuxTarget, TmuxAdapter};
use nana_runtime_core::{runtime_contract_summary, RuntimeCommand, RuntimeEngine};
use std::env;
use std::process;

fn main() {
    if let Err(error) = run() {
        eprintln!("nana-runtime: {error}");
        process::exit(1);
    }
}
        ...
        Some("schema") => {
            if second == Some("--json") {
                let summary = serde_json::json!({
                    "schema_version": nana_runtime_core::RUNTIME_SCHEMA_VERSION,
                    "commands": nana_runtime_core::RUNTIME_COMMAND_NAMES,
                    "events": nana_runtime_core::RUNTIME_EVENT_NAMES,
                    "transport": "tmux",
                });
                println!(
                    "{}",
