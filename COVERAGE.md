...
| Category | OMC Features | NANA Implemented | Coverage |
|----------|-------------|-----------------|----------|
...
| OMC Skill | NANA Status | Mechanism |
|-----------|-----------|-----------|
| autopilot | DONE | ~/.codex/skills/autopilot/SKILL.md |
| verify-loop | DONE (runtime mode) | `.nana/state/{scope}/verify-loop-state.json` |
...
| deepinit | DONE (lightweight CLI successor) | `nana agents-init [path]` (`nana deepinit [path]` alias) |
...
| nana-setup | DONE | ~/.codex/skills/nana-setup/SKILL.md |
...
| SessionEnd | nana postLaunch lifecycle phase | PARTIAL (post-exit cleanup) |

`*` FULL via terminal automation workaround (default-enabled in `v0.2.3` generated `.nana/tmux-hook.json`), not native hook context injection.

### Infrastructure

| Component | OMC | NANA Status |
|-----------|-----|-----------|
| CLI (setup) | DONE | DONE |
...
2. **Native context injection from hooks** - Not available in Codex hooks API. Workaround: tmux prompt injection (`nana tmux-hook`) plus state files + AGENTS.md instructions (default-enabled in `v0.2.3` generated config).
...
4. **Session end** - No real-time event. Workaround: `nana` wrapper detects Codex exit via blocking execSync and runs postLaunch cleanup (overlay strip, session archive, mode cancellation).
