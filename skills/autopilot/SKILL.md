...
0. **Pre-context Intake (required before Phase 0 starts)**:
   ...
   - Load the latest relevant snapshot from `.nana/context/{slug}-*.md` when available.
   - If no snapshot exists, create `.nana/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) with:
     - Task statement
     - Desired outcome
...
1. **Phase 0 - Expansion**: Turn the user's idea into a detailed spec
   - If `.nana/specs/deep-interview-*.md` exists for this task: reuse it and skip redundant expansion work
   ...
   - Output: `.nana/plans/autopilot-spec.md`
...
2. **Phase 1 - Planning**: Create an implementation plan from the spec
   ...
   - Output: `.nana/plans/autopilot-impl.md`

3. **Phase 2 - Execution**: Implement the plan using verify-loop + ultrawork
...
Use `nana_state` MCP tools for autopilot lifecycle state.
...
```
RALPLAN (consensus planning) -> team-exec (Codex CLI workers) -> verify-loop (architect verification)
```
...
maxVerifyLoopIterations = 10    # Verify-loop iteration ceiling
