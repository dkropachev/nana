---
name: help
description: Guide on using nana plugin
---

# How NANA Works

**You don't need to learn any commands!** NANA enhances Codex CLI with intelligent behaviors that activate automatically.
...
If you haven't configured NANA yet:
...
Analyze your nana usage and get tailored recommendations to improve your workflow.

> Note: This replaces the former `/learn-about-omc` skill.

### What It Does

1. Reads token tracking from `~/.nana/state/token-tracking.jsonl`
2. Reads session history from `.nana/state/session-history.json`
...
TOKEN_FILE="$HOME/.nana/state/token-tracking.jsonl"
SESSION_FILE=".nana/state/session-history.json"
CONFIG_FILE="$HOME/.codex/.nana-config.json"

echo "Analyzing NANA Usage..."
echo ""
...
- "Set defaultExecutionMode in /nana-setup for consistent behavior"
...
1. Ensure ~/.nana/state/ directory exists
2. Run any NANA command to start tracking

Tip: Run /nana-setup to configure NANA properly.
```

## Need More Help?

- **README**: https://github.com/Yeachan-Heo/nana
- **Issues**: https://github.com/Yeachan-Heo/nana/issues
