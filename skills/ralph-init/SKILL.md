...
2. **Create PRD** at `.nana/plans/prd-{slug}.md` with:
   - Problem statement
   - Goals and non-goals
...
4. **Initialize/ensure canonical progress ledger** at `.nana/state/{scope}/ralph-progress.json` (session scope if active session exists)

### Canonical source contract

- Canonical PRD source of truth is `.nana/plans/prd-{slug}.md`.
- Ralph progress source of truth is `.nana/state/{scope}/ralph-progress.json` (session scope when available).
- Legacy `.nana/prd.json` / `.nana/progress.txt` inputs are compatibility-only and migrate one-way into canonical artifacts.

## Output

A structured PRD file saved to `.nana/plans/` that serves as the definition of done for Ralph execution.
