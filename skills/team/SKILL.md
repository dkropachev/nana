...
`$team` is the tmux-based parallel execution mode for NANA. It starts real worker Codex and/or Claude CLI sessions in split panes and coordinates them through `.nana/state/team/...` files plus CLI team interop (`nana team api ...`) and state files.
...
- Use **`nana team`** when you need durable tmux workers, shared task state, mailbox/dispatch coordination, worktrees, explicit lifecycle control, or long-running parallel execution that must survive beyond one local reasoning burst.
...
1. Invoke NANA runtime directly with `nana team ...`
...
If `nana team` is unavailable, stop with a hard error.

## Invocation Contract

```bash
nana team [N:agent-type] "<task description>"
```

Examples:

```bash
nana team 3:executor "analyze feature X and report flaws"
nana team "debug flaky integration tests"
nana team "ship end-to-end fix with verification"
```

### Team-first launch contract

`nana team ...` is now the canonical launch path for coordinated execution.
...
- **Canonical launch:** use plain `nana team ...` / `$team ...` for coordinated workers.
- **Verification ownership:** keep one lane focused on tests, regression coverage, and evidence before shutdown.
- **Escalation:** start a separate `nana ralph ...` / `$ralph ...` only when a later manual follow-up still needs a persistent single-owner fix/verification loop.
- **Deprecation:** `nana team ralph ...` has been removed. Use plain `nana team ...` for team execution or run `nana ralph ...` separately when you explicitly want a later Ralph loop.
...
NANA_TEAM_WORKER_CLI=claude nana team 2:executor "update docs and report"

# Mixed team (worker 1 = Codex, worker 2 = Claude)
NANA_TEAM_WORKER_CLI_MAP=codex,claude nana team 2:executor "split doc/code tasks"

# Auto mode: Claude is selected when worker launch args/model contains 'claude'
NANA_TEAM_WORKER_CLI=auto NANA_TEAM_WORKER_LAUNCH_ARGS="--model claude-..." nana team 2:executor "run mixed validation"
```
...
3. `nana` command resolves to the intended install/build
4. If running repo-local `node bin/nana.js ...`, run `npm run build` after `src` changes
...
If duplicates exist, remove extras before `nana team` to prevent HUD ending up in worker stack.

## Pre-context Intake Gate

Before launching `nana team`, require a grounded context snapshot:

1. Derive a task slug from the request.
2. Reuse the latest relevant snapshot in `.nana/context/{slug}-*.md` when available.
3. If none exists, create `.nana/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) with:
   - task statement
   - desired outcome
...
For simple read-only brownfield lookups during intake, follow active session guidance: when `USE_NANA_EXPLORE_CMD` is enabled, prefer `nana explore` with narrow, concrete prompts; otherwise use the richer normal explore path and fall back normally if `nana explore` is unavailable.
...
- include an explicit launch hint (`nana team N "<task>"` / `$team N "<task>"`) for the coordinated team run; mention a later separate Ralph follow-up only when genuinely needed
...
`nana team` currently performs:

1. Parse args (`N`, `agent-type`, task)
2. Sanitize team name from task text
3. Initialize team state:
   - `.nana/state/team/<team>/config.json`
   - `.nana/state/team/<team>/manifest.v2.json`
   - `.nana/state/team/<team>/tasks/task-<id>.json`
4. Compose team-scoped worker instructions file at:
   - `.nana/state/team/<team>/worker-agents.md`
   - Uses project `AGENTS.md` content (if present) + worker overlay, without mutating project `AGENTS.md`
5. Resolve canonical shared state root from leader cwd (`<leader-cwd>/.nana/state`)
6. Split current tmux window into worker panes
7. Launch workers with:
   - `NANA_TEAM_WORKER=<team>/worker-<n>`
   - `NANA_TEAM_STATE_ROOT=<leader-cwd>/.nana/state`
   - `NANA_TEAM_LEADER_CWD=<leader-cwd>`
   - worker CLI selected by `NANA_TEAM_WORKER_CLI` / `NANA_TEAM_WORKER_CLI_MAP` (`codex` or `claude`)
...
- Workers may run in separate git worktrees (`nana team --worktree[=<name>]`) while sharing one team state root
...
- Submit routing uses this CLI resolution order per worker trigger:
  ...
  2) `NANA_TEAM_WORKER_CLI_MAP` entry for that worker index,
  3) fallback `NANA_TEAM_WORKER_CLI` / auto detection.
...
1. Explicit worker model in `NANA_TEAM_WORKER_LAUNCH_ARGS`
2. Inherited leader `--model` flag
3. Low-complexity default from `NANA_DEFAULT_SPARK_MODEL` (legacy alias: `NANA_SPARK_MODEL`) when 1+2 are absent and team `agentType` is low-complexity

Default-model rule:
- Do **not** assume a frontier or spark model from recency or model-family heuristics.
- Use `NANA_DEFAULT_FRONTIER_MODEL` for frontier-default guidance.
- Use `NANA_DEFAULT_SPARK_MODEL` for spark/low-complexity worker-default guidance.
...
- When the leader assigns teammate roles/tasks, NANA allocates **per-worker reasoning effort dynamically** from the resolved worker role (`low`, `medium`, `high`).
- Explicit launch args still win: if `NANA_TEAM_WORKER_LAUNCH_ARGS` already includes `-c model_reasoning_effort=...`, that explicit value overrides dynamic allocation for every worker.
...
2. Monitor task and worker progress with runtime/state tools first (`nana team status <team>`, `nana team resume <team>`, mailbox/state files)
3. Wait for terminal task state before shutdown:
   - `pending=0`
   - `in_progress=0`
   - `failed=0` (or explicitly acknowledged failure path)
4. Only then run `nana team shutdown <team>`
...
```bash
sleep 30 && nana team status <team-name>
```

Repeat that check while the team stays active, or use `nana team await <team-name> --timeout-ms 30000 --json` when event-driven waiting is a better fit.

If the leader gets a stale/team-stalled nudge, immediately run `nana team status <team-name>` before taking any manual intervention.
...
1. Use `nana team ...` runtime lifecycle commands for orchestration.
2. Use `nana team api ... --json` for mailbox/task mutations.
3. Verify delivery via mailbox/state evidence (`mailbox/*.json`, task status, `nana team status`).
...
- `shutdown`: graceful shutdown request, then cleanup (deletes `.nana/state/team/<team>`)

## Data Plane and Control Plane

### Control Plane

- tmux panes/processes (`NANA_TEAM_WORKER` per worker)
- leader notifications via `tmux display-message`

### Data Plane

- `.nana/state/team/<team>/...` files
- Team mailbox files:
- `.nana/state/team/<team>/mailbox/leader-fixed.json`
- `.nana/state/team/<team>/mailbox/worker-<n>.json`
- `.nana/state/team/<team>/dispatch/requests.json` (durable dispatch queue; hook-preferred, fallback-aware)
...
- `.nana/state/team/<team>/manifest.v2.json`
- `.nana/state/team/<team>/tasks/task-<id>.json`
- `.nana/state/team/<team>/workers/worker-<n>/identity.json`
- `.nana/state/team/<team>/workers/worker-<n>/inbox.md`
- `.nana/state/team/<team>/workers/worker-<n>/heartbeat.json`
- `.nana/state/team/<team>/workers/worker-<n>/status.json`
- `.nana/state/team-leader-nudge.json`


## Team Mutation Interop (CLI-first)

Use `nana team api` for machine-readable mutation/reads instead of legacy `team_*` MCP tools.

```bash
nana team api <operation> --input '{"team_name":"my-team",...}' --json
```

Examples:

```bash
nana team api send-message --input '{"team_name":"my-team","from_worker":"worker-1","to_worker":"leader-fixed","body":"ACK"}' --json
nana team api claim-task --input '{"team_name":"my-team","task_id":"1","worker":"worker-1"}' --json
nana team api transition-task-status --input '{"team_name":"my-team","task_id":"1","from":"in_progress","to":"completed","claim_token":"<token>"}' --json
```
...
- Send ACK to `leader-fixed` mailbox via `nana team api send-message --json`
- Claim/transition/release task lifecycle via `nana team api <operation> --json`
...
Use only after checking `nana team status <team>` and mailbox/state evidence:

1. Capture pane tail to confirm current worker state:
   - `tmux capture-pane -t %<worker-pane> -p -S -120`
   - If a larger-tail read or bounded summary would help, prefer explicit opt-in inspection via `nana sparkshell --tmux-pane %<worker-pane> --tail-lines 400` before improvising extra tmux commands.
...
2. `.nana/state/team/<team>/mailbox/leader-fixed.json` exists
3. Worker skill loaded and `nana team api send-message --json` called
4. Task-id mismatch not blocking worker flow

### Worker logs `nana team api ... ENOENT` (or legacy `team_send_message ENOENT` / `team_update_task ENOENT`)

Meaning:
- Team state path no longer exists while worker is still running.
- Typical cause: leader/manual flow ran `nana team shutdown <team>` (or removed `.nana/state/team/<team>`) before worker finished.

Checks:

1. `nana team status <team>` and confirm whether tasks were still `in_progress` when shutdown occurred
2. Verify whether `.nana/state/team/<team>/` exists
3. Inspect worker pane tail for post-shutdown writes
4. Confirm no external cleanup (`rm -rf .nana/state/team/<team>`) happened during execution
...
rm -rf .nana/state/team/<team-name>

# 4) Retry
nana team 1:executor "fresh retry"
```

Guidelines:

- Do not kill leader pane
- Do not kill HUD pane (`nana hud --watch`) unless intentionally restarting HUD
...
Use `nana sparkshell --tmux-pane ...` as an explicit opt-in operator aid for pane inspection and summaries; keep raw `tmux capture-pane` evidence available for manual intervention and proof.

## MCP Job Lifecycle Tools

For programmatic or agent-driven team spawning (as opposed to interactive CLI use), NANA exposes four MCP tools via the `team-server`:

| Tool | Description |
|------|-------------|
| `nana_run_team_start` | Spawn tmux CLI workers in the background; returns a `jobId` immediately |
| `nana_run_team_status` | Non-blocking status check for a running job |
| `nana_run_team_wait` | Block until the job completes, with automatic idle-pane nudging |
| `nana_run_team_cleanup` | Kill worker tmux panes for a job (early stop only) |

### CLI vs MCP Tools

- **`nana team ...` CLI** — Primary method for interactive team orchestration. Use this when you are operating inside a live tmux session and want direct pane visibility.
- **`nana_run_team_*` MCP tools** — For programmatic or agent-driven team spawning (analogous to OMC's `omc_run_team_*` tools). Use these when an agent needs to launch workers, poll status, and collect results without manual intervention.

### Naming Distinction

Two cleanup tools exist and must not be confused:

- `team_cleanup` (**state-server**): Deletes team state **files** on disk (`.nana/state/team/<team>/`). Use after a team run is fully complete.
- `nana_run_team_cleanup` (**team-server**): Kills tmux worker **panes** for a job. Use only when stopping workers early; otherwise `nana_run_team_wait` handles natural termination.

### Basic Usage Example

```
1. nana_run_team_start({
     teamName: "fix-bugs",
     agentTypes: ["codex"],
     tasks: [{ subject: "Fix bug", description: "..." }],
     cwd: "/path/to/project"
   })
   → Returns { jobId: "nana-abc123" }

2. nana_run_team_wait({ job_id: "nana-abc123", timeout_ms: 300000 })
   → Blocks until done, auto-nudges idle panes

3. nana_run_team_cleanup({ job_id: "nana-abc123" })
   → Only needed if stopping workers early
```

`nana_run_team_status` can be called between steps 1 and 2 for a non-blocking poll if you need to interleave other work while workers run.
