...
- **team-exec**: Coordinated execution via Codex CLI workers.
- **verify-loop**: Verification loop with configurable iteration count.
...
const config = createAutopilotPipelineConfig('build feature X', {
  stages: [
    ...
    createVerifyLoopStage({ maxIterations: 15 }),
  ],
});
