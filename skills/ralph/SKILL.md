...
0. **Pre-context intake (required before planning/execution loop starts)**:
   - Assemble or load a context snapshot at `.nana/context/{task-slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`).
   ...
   - If request ambiguity is high, gather brownfield facts first. When session guidance enables `USE_NANA_EXPLORE_CMD`, prefer `nana explore` for simple read-only repository lookups with narrow, concrete prompts; otherwise use the richer normal explore path. Then run `$deep-interview --quick <task>` to close critical gaps.
...
5. **Visual task gate (when screenshot/reference images are present)**:
   ...
   - Persist verdict to `.nana/state/{scope}/ralph-progress.json` including numeric + qualitative feedback.
...
7.5 **Mandatory Deslop Pass**:
   - After Step 7 passes, run `nana:ai-slop-cleaner` on **all files changed during the Ralph session**.
...
Use the `nana_state` MCP server tools (`state_write`, `state_read`, `state_clear`) for Ralph lifecycle state.
...
1. Run deep-interview in quick mode before creating PRD artifacts:
   ...
   - Persist interview output to `.nana/interviews/{slug}-{timestamp}.md`
2. Create canonical PRD/progress artifacts:
   - PRD: `.nana/plans/prd-{slug}.md`
   - Progress ledger: `.nana/state/{scope}/ralph-progress.json` (session scope when available, else root scope)
...
5. Initialize canonical progress ledger at `.nana/state/{scope}/ralph-progress.json`
...
Workflow: Detect flag, extract task, create `.nana/plans/prd-{slug}.md`, create `.nana/state/{scope}/ralph-progress.json`, begin ralph loop.

### Legacy compatibility
- If `.nana/prd.json` exists and canonical PRD is absent, migrate one-way into `.nana/plans/prd-{slug}.md`.
- If `.nana/progress.txt` exists and canonical progress ledger is absent, import one-way into `.nana/state/{scope}/ralph-progress.json`.
