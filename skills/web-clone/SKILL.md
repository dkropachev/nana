...
- If running within verify-loop, use `state_write` / `state_read` for web-clone state persistence between iterations.
...
- **After Pass 1 completes**: Write extraction summary to `.nana/state/{scope}/web-clone-extraction.json` containing:
...
- **After each Pass 4 verification**: Append the composite verdict to `.nana/state/{scope}/web-clone-verdicts.json`.
- **When running within verify-loop**: Also persist the `visual` portion of the composite verdict to `.nana/state/{scope}/verify-loop-progress.json`, mapping `visual.score` → top-level `score` and `visual.verdict` → top-level `verdict`.
...
- [ ] When in verify-loop: visual verdict persisted to `verify-loop-progress.json`
