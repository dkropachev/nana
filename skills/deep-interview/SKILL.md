...
<Purpose>
Deep Interview is an intent-first Socratic clarification loop before planning or implementation. It turns vague ideas into execution-ready specifications by asking targeted questions about why the user wants a change, how far it should go, what should stay out of scope, and what NANA may decide without confirmation.
</Purpose>
...
- When session guidance enables `USE_NANA_EXPLORE_CMD`, prefer `nana explore` for simple read-only brownfield fact gathering; keep prompts narrow and concrete, and keep ambiguous or non-shell-only investigation on the richer normal path and fall back normally if `nana explore` is unavailable.
...
2. Attempt to load the latest relevant context snapshot from `.nana/context/{slug}-*.md`.
...
4. Save snapshot to `.nana/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) and reference it in mode state.
...
```json
{
  ...
  "state": {
    ...
    "context_snapshot_path": ".nana/context/<slug>-<timestamp>.md"
  }
}
```
...
1. Write interview transcript summary to:
   - `.nana/interviews/{slug}-{timestamp}.md`  
     (kept for verify-loop PRD alignment)
2. Write execution-ready spec to:
   - `.nana/specs/deep-interview-{slug}.md`
...
- Decision Boundaries (what NANA may decide without confirmation)
- Constraints
...
- **Canonical artifact path:** `.nana/specs/deep-interview-autoresearch-{slug}.md`
- **Launch artifact bundle:** `.nana/specs/autoresearch-{slug}/mission.md`, `.nana/specs/autoresearch-{slug}/sandbox.md`, and `.nana/specs/autoresearch-{slug}/result.json`
- **Launch artifact directory:** `.nana/specs/autoresearch-{slug}/`
- **Required artifact sections:**
  ...
  - `Seed Inputs`
  - `Confirmation Bridge`
- **Required launch artifacts under `.nana/specs/autoresearch-{slug}/`:**
  - `mission.md`
  - `sandbox.md`
  - `result.json`
...
- **Input Artifact:** `.nana/specs/deep-interview-{slug}.md` (optionally accompanied by the transcript/context snapshot for traceability)
...
- Read/write context snapshots under `.nana/context/`
- Save transcript/spec artifacts under `.nana/interviews/` and `.nana/specs/`
</Tool_Usage>
...
<Final_Checklist>
- [ ] Preflight context snapshot exists under `.nana/context/{slug}-{timestamp}.md`
