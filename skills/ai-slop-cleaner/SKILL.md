...
## Scoped File Lists and Verify-Loop Workflow

- This skill can accept a **file list scope** instead of a whole feature area.
- When the caller provides a changed-files list (for example, verify-loop session-owned edits), keep the cleanup strictly bounded to those files.
- In the **verify-loop workflow**, the mandatory deslop pass should run this skill on those changed files only, in standard mode unless the caller explicitly requests otherwise.
