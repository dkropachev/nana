...
- When session guidance enables `USE_NANA_EXPLORE_CMD`, prefer `nana explore` for simple read-only repository lookups during planning; keep prompts narrow and concrete, and keep prompt-heavy or ambiguous planning work on the richer normal path and fall back normally if `nana explore` is unavailable.
...
1. Read plan file from `.nana/plans/`
...
- If ToolSearch finds no MCP tools or Codex is unavailable, fall back to equivalent NANA prompt agents -- never block on external tools
