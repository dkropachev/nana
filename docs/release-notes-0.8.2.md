...
- `nana setup` now skips the deprecated **`[tui]`** config section for Codex CLI `>= 0.107.0` (`#572`, fixes `#564`).
...
NANA team mode now supports **Gemini** as a worker CLI provider in addition to Codex and Claude.
...
- #573 — feat(team): add Gemini CLI worker support to NANA team mode
