# NANA ↔ mux canonical operation space

This document defines the mux boundary owned by NANA core semantics.
...
Target handles use the format `session_name:window_index.pane_index` (e.g. `"nana:0.1"`). `MuxTarget::Detached` is rejected for all operations that require a real pane.
