...
Ralph runtime state is stored at `.nana/state/{scope}/ralph-state.json` and MUST use this schema:
...
explicit follow-up, but there is no built-in `nana team ralph ...` linked launch
path anymore.
...
- Canonical progress ledger: `.nana/state/{scope}/ralph-progress.json`
- Legacy compatibility migration:
  - `.nana/prd.json` migrates one-way to canonical PRD markdown when no canonical PRD exists.
  - `.nana/progress.txt` migrates one-way to canonical `ralph-progress.json` when no canonical ledger exists.
