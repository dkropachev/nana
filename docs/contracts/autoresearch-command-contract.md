# `nana autoresearch` parity contract

`nana autoresearch` is a thin supervisor that drives one Codex experiment session per iteration while NANA owns the durable keep/discard/reset loop.

## CLI

```bash
nana autoresearch <mission-dir> [codex-args...]
nana autoresearch --resume <run-id> [codex-args...]
nana autoresearch --help
```

- Fresh launch always creates a new run-tagged lane.
- `--resume <run-id>` loads `.nana/logs/autoresearch/<run-id>/manifest.json`.
- A second launch is rejected while repo-root `.nana/state/autoresearch-state.json` points at an active run.
...
- worktree `<repo>.nana-worktrees/autoresearch-<mission-slug>-<run-tag>`
- repo-root run artifacts under `.nana/logs/autoresearch/<run-id>/`

Repo-root state responsibilities:
- `.nana/state/autoresearch-state.json` = active-run pointer/lock only
- `.nana/logs/autoresearch/<run-id>/manifest.json` = authoritative per-run state
- `.nana/logs/autoresearch/<run-id>/candidate.json` = candidate handoff from the just-finished Codex session
- `.nana/logs/autoresearch/<run-id>/iteration-ledger.json` = durable iteration history
- `.nana/logs/autoresearch/<run-id>/latest-evaluator-result.json` = latest evaluator output
