# `nana autoresearch` Full-Parity Review Notes
...
- `.nana/plans/prd-autoresearch-full-parity.md`
- `.nana/plans/test-spec-autoresearch-full-parity.md`
...
- `nana autoresearch` exposes both fresh launch and `--resume <run-id>` flows.
...
**Pass.**
- repo-root active-run lock lives at `.nana/state/autoresearch-state.json`.
- per-run manifest, candidate, ledger, and latest evaluator files live under `.nana/logs/autoresearch/<run-id>/`.
