# PR Draft: Rebase `feat/nana-sparkshell` onto `experimental/dev`

## Target branch
`experimental/dev`

## Summary
This PR rebases the `nana sparkshell` feature line onto `experimental/dev` and preserves the follow-on team-status inspection work built on top of it.

It adds the native Rust-backed `nana sparkshell <command> [args...]` flow, plus a long sequence of `nana team status` improvements that surface pane-aware inspection metadata and ready-to-run sparkshell commands for leader triage.

## What’s included
### `nana sparkshell`
- adds CLI dispatch/help for `nana sparkshell`
- adds `src/cli/sparkshell.ts` for native binary discovery and launch
- adds the Rust crate under `native/nana-sparkshell/`
- adds packaging/build helpers:
  - `scripts/build-sparkshell.mjs`
  - `scripts/test-sparkshell.mjs`
- stages the packaged native binary under `bin/native/linux-x64/nana-sparkshell`
- adds focused CLI + packaging tests

### `nana team status` inspection improvements
...
- excluded `native/nana-sparkshell` from the root Cargo workspace so manifest-path cargo commands work again
...
- `npm run build:full`
- `cargo test --manifest-path native/nana-sparkshell/Cargo.toml`
- `node scripts/build-sparkshell.mjs`
- `node scripts/test-sparkshell.mjs`
- `node bin/nana.js sparkshell cargo --version`
- `node bin/nana.js sparkshell npm --version`
- `node bin/nana.js sparkshell git log --oneline -3`
- `npm test`
...
- Native Rust build artifacts under `native/nana-sparkshell/target/` were removed after verification.
