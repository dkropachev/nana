...
### Spark Initiative: `nana explore` and `nana sparkshell`

NANA now has a stronger native fast path for repository discovery and shell-native inspection.

This release:
- introduces `nana explore` as the default read-only exploration entrypoint
- adds the Rust-backed explore harness plus packaging and source-fallback flow
- introduces `nana sparkshell <command> [args...]` as an explicit operator-facing native sidecar
- allows qualifying read-only shell-native `nana explore` tasks to route through `nana sparkshell`
- keeps the explore path intentionally constrained: shell-only, read-only, and allowlisted

Representative changes:
- `fb07c3c` — feat: add nana explore harness and packaging flow
- `71858c3` — feat: add nana sparkshell and team inspection metadata
...
- users can install NANA normally with `npm install -g nana`
...
`0.9.0` also upgrades NANA's release shape so the new native surfaces are publishable and consumable across platforms.

This release:
- unifies cross-platform native publishing for `nana-explore-harness` and `nana-sparkshell`
...
This release:
- exposes `nana sparkshell --tmux-pane <pane-id> --tail-lines <100-1000>` for explicit pane summarization
...
- If you use project-scoped NANA installs, rerun:

```bash
nana setup --force --scope project
```

- Expect `nana explore` and `nana sparkshell` packaged installs to rely on release-asset hydration when no explicit binary override or repo-local artifact is present.
...
- [#782](https://github.com/Yeachan-Heo/nana/pull/782) — explore routes qualifying read-only shell tasks via sparkshell
- [#784](https://github.com/Yeachan-Heo/nana/pull/784) — cross-platform native publishing and release-pipeline follow-through
- [#785](https://github.com/Yeachan-Heo/nana/pull/785) — team runtime lifecycle and cleanup hardening
- [#786](https://github.com/Yeachan-Heo/nana/pull/786) — nested help routing cleanup
- [#787](https://github.com/Yeachan-Heo/nana/pull/787) — centralized NANA default model resolution
- [#788](https://github.com/Yeachan-Heo/nana/pull/788) — HUD branch/config loading hardening
- [#789](https://github.com/Yeachan-Heo/nana/pull/789) — distribute generated aspect tasks across workers
- [#793](https://github.com/Yeachan-Heo/nana/pull/793) — Windows Codex command shim probing fix
- [#794](https://github.com/Yeachan-Heo/nana/pull/794) — merge `experimental/dev` into `dev`
- [#805](https://github.com/Yeachan-Heo/nana/pull/805) — keep workers running after mailbox replies

## Related issues highlighted in this release

- [#781](https://github.com/Yeachan-Heo/nana/pull/781) — force low reasoning for sparkshell summaries
- [#744](https://github.com/Yeachan-Heo/nana/issues/744) — persist lifecycle profiles for linked Ralph vs default team runs
- [#745](https://github.com/Yeachan-Heo/nana/issues/745) — team cleanup policy hardening
- [#746](https://github.com/Yeachan-Heo/nana/issues/746) — team policy/governance split follow-through
- [#741](https://github.com/Yeachan-Heo/nana/issues/741) — release-readiness / linked team-Ralph runtime follow-up
- [#732](https://github.com/Yeachan-Heo/nana/issues/732) — related team stall/lifecycle cleanup follow-up
