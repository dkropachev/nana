...
If you use `$team`/`nana team` heavily, this release makes the runtime more consistent by standardizing around **CLI-first interop**.
...
- If you previously relied on legacy `team_*` MCP workflows, migrate to:
  - `nana team api <operation> ...`
- For notification setup, prefer:
  - `nana configure-notifications` (or skill equivalent)
...
Thanks for using **nana**. If anything regresses, please open an issue with repro steps and logs.
