...
Benchmark runs showed spawned child agents being labeled with legacy model aliases such as `sonnet`, even when NANA was launched under Codex with GPT-5.x reasoning settings.
...
After rebuilding and rerunning `nana setup --force`, spawned-agent metadata should stop surfacing `haiku` / `sonnet` / `opus` as if they were active runtime models for Codex-based NANA runs.

## Re-test steps

```bash
npm run build   # TypeScript build
node bin/nana.js setup --scope project --force
```
