...
After `nana setup`, native agent configs in `~/.nana/agents/` include new sections:
`## NANA Posture Overlay`, `## Model-Class Guidance`, and `## NANA Agent Metadata`.
...
PRs: [#588](https://github.com/Yeachan-Heo/nana/pull/588), [#592](https://github.com/Yeachan-Heo/nana/pull/592) ([@HaD0Yun](https://github.com/HaD0Yun))

## Bug fixes

### Windows ESM import crash

`bin/nana.js` failed on Windows with `ERR_UNSUPPORTED_ESM_URL_SCHEME` because `import()` received a bare absolute path (`C:\...`) instead of a `file://` URL.

Fix: convert the resolved path to a `file://` URL via `url.pathToFileURL()` before dynamic import.

PR: [#589](https://github.com/Yeachan-Heo/nana/pull/589) ([@sjals93](https://github.com/sjals93))
Fixes: [#557](https://github.com/Yeachan-Heo/nana/issues/557)
...
PR: [#593](https://github.com/Yeachan-Heo/nana/pull/593)
Fixes: [#591](https://github.com/Yeachan-Heo/nana/issues/591)
...
Part of PR: [#592](https://github.com/Yeachan-Heo/nana/pull/592) ([@HaD0Yun](https://github.com/HaD0Yun))
...
2f4862a docs(nana): remove remaining legacy model alias references
0d2115c fix(nana): remove legacy model aliases from prompts and runtime metadata
8fb3aa0 fix(bin): use file:// URL for dynamic import on Windows
f448108 feat(nana): add posture-aware agent routing metadata
```
