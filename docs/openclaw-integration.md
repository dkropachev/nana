...
3. NANA emits a warning for clarity
    ...
    "openclaw": {
      ...
      "hooks": {
        "session-end": {
          "enabled": true,
          "gateway": "local",
          "instruction": "NANA task completed for {{projectPath}}"
        },
        "ask-user-question": {
          "enabled": true,
          "gateway": "local",
          "instruction": "NANA needs input: {{question}}"
        }
      }
    }
  }
...
```json
{
  "notifications": {
    ...
    "custom_webhook_command": {
      ...
      "instruction": "NANA event {{event}} for {{projectPath}}"
    },
    "custom_cli_command": {
      "enabled": true,
...
These aliases are normalized by NANA into internal OpenClaw gateway mappings.

## Option C: Clawdbot agent-command workflow (recommended for dev)

Use this when you want NANA hook events to trigger **agent turns** (not plain
...
>
> Command gateway timeout precedence: `gateways.<name>.timeout` > `NANA_OPENCLAW_COMMAND_TIMEOUT_MS` > `5000`.
> For `clawdbot agent` workflows, use `120000` (2 minutes) to avoid premature timeout.
>
> **Production best practices:**
> - Use `|| true` at the end of the command to prevent NANA hook failures from blocking sessions
  ...
  "notifications": {
    ...
    "openclaw": {
      ...
      "gateways": {
        "local": {
          "type": "command",
          "command": "(clawdbot agent --session-id nana-hooks --message {{instruction}} --thinking minimal --deliver --reply-channel discord --reply-to 'channel:1468539002985644084' --timeout 120 --json >>/tmp/nana-openclaw-agent.jsonl 2>&1 || true)",
          "timeout": 120000
        }
      },
      "hooks": {
...
```text
NANA 훅={{event}} 프로젝트={{projectName}} 세션={{sessionId}}.
반드시 한국어로 응답하세요.
NANA tmux 세션: {{tmuxSession}}.
SOUL.md 및 #omc-dev 맥락을 참고해 필요한 후속 액션이 있으면 즉시 안내하세요.
```

### 2) Track which NANA tmux session emitted the hook
...
clawdbot agent --session-id nana-hooks \
  --message "NANA hook retry 점검: session={{sessionId}} tmux={{tmuxSession}}" \
...
curl -sS -X POST http://127.0.0.1:18789/hooks/wake \
  ...
  -d '{"text":"NANA wake smoke test","mode":"now"}'
```
...
```bash
curl -sS -o /tmp/nana-openclaw-agent-check.json -w "HTTP %{http_code}\n" \
  -X POST http://127.0.0.1:18789/hooks/agent \
  -H "Authorization: Bearer ${HOOKS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{"message":"NANA delivery verification","instruction":"NANA delivery verification","event":"session-end","sessionId":"manual-check"}'
```
...
# gate checks
test "$NANA_OPENCLAW" = "1" && echo "NANA_OPENCLAW=1" || echo "missing NANA_OPENCLAW=1"
test "$NANA_OPENCLAW_COMMAND" = "1" && echo "NANA_OPENCLAW_COMMAND=1" || echo "missing NANA_OPENCLAW_COMMAND=1"
```
...
- **Command gateway disabled**: set both `NANA_OPENCLAW=1` and `NANA_OPENCLAW_COMMAND=1`.
- **Command killed by `SIGTERM`**: increase `gateways.<name>.timeout` (recommend `120000` for clawdbot agent) or set `NANA_OPENCLAW_COMMAND_TIMEOUT_MS`.
- **Hook failures blocking sessions**: ensure command ends with `|| true` to prevent NANA from waiting on clawdbot failures.
