# Heavy / Manual Stress Lane: `nana-explore` and `nana-sparkshell`
...
| Command | Result |
|---|---|
| `node bin/nana.js explore --help` | PASS |
| `node bin/nana.js sparkshell --help` | PASS |
| `node bin/nana.js sparkshell git --version` | PASS |
| `node bin/nana.js version` | PASS |
| `for i in $(seq 1 20); do node bin/nana.js sparkshell git --version; done` | PASS (bounded sample verified locally) |
| `seq 1 5 | xargs -I{} -P5 node bin/nana.js sparkshell git --version` | PASS (bounded sample verified locally) |
...
Final deterministic recheck (required for the later verify-loop verification sweep; not re-run in this doc-only pass):
...
**Goal**
- Confirm `nana-sparkshell --tmux-pane` preserves operator-critical facts when pane tails are large and noisy.
...
```bash
node bin/nana.js sparkshell git --version
```

Suggested repeated-run shape:

```bash
for i in $(seq 1 20); do node bin/nana.js sparkshell git --version; done
```

Suggested concurrent-run shape:

```bash
seq 1 5 | xargs -I{} -P5 node bin/nana.js sparkshell git --version
```
...
**Goal**
- Stress `nana-explore` and `nana-sparkshell` summarization with outputs that bury signal under distractors.
...
```bash
node bin/nana.js sparkshell --help
node bin/nana.js explore --help
```
...
## Residual-risk prompts for the final verify-loop sweep
