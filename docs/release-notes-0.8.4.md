...
This is a **setup-flow patch release** focused on making `nana setup` refresh behavior safer, more predictable, and easier to rerun.

---

## TL;DR

- `nana setup` now refreshes managed NANA artifacts by default instead of leaving stale generated content behind.
...
### 1) Managed NANA artifacts refresh by default

Setup now treats managed NANA artifacts as refreshable outputs rather than one-time drops. Re-running `nana setup` updates shipped artifacts more consistently, helping existing installations stay aligned with current templates and generated assets.
...
**Why this matters:**
- lowers risk when refreshing existing local NANA-managed files
...
- `fed035b` — feat(setup): refresh managed NANA artifacts by default with backups
...
Thanks for using **nana**. If anything regresses, please open an issue with reproduction steps, logs, and your CLI/runtime details.
