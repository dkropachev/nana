...
4. **Lifecycle/process handling**
   - `bin/nana.js` now awaits `main(...)` and exits explicitly.
...
- Set `NANA_TEAM_MOUSE=0`, restart session, verify mouse mode is not forcibly enabled.
