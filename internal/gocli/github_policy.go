...
type githubWorkActionPolicy struct {
...
type githubWorkPolicy struct {
	Version              int                     `json:"version"`
	Experimental         *bool                   `json:"experimental,omitempty"`
	AllowedActions       *githubWorkActionPolicy `json:"allowed_actions,omitempty"`
...
type githubResolvedWorkPolicy struct {
	Version              int                    `json:"version"`
	Experimental         bool                   `json:"experimental"`
	AllowedActions       githubWorkActionPolicy `json:"allowed_actions"`
...
func defaultGithubResolvedWorkPolicy() githubResolvedWorkPolicy {
	return githubResolvedWorkPolicy{
		Version:      1,
		Experimental: false,
		AllowedActions: githubWorkActionPolicy{
			Commit:        true,
			Push:          true,
			OpenDraftPR:   true,
			RequestReview: false,
...
func githubGlobalWorkPolicyPath() string {
	return filepath.Join(workHomeRoot(), "policy.json")
}

func githubRepoProfilePath(repoSlug string) string {
	return githubManagedPaths(repoSlug).RepoProfilePath
}

func readGithubWorkPolicy(path string) (*githubWorkPolicy, error) {
	var policy githubWorkPolicy
...
func resolveGithubWorkPolicy(repoPath string) (githubResolvedWorkPolicy, error) {
	resolved := defaultGithubResolvedWorkPolicy()
	apply := func(path string, source string) error {
		policy, err := readGithubWorkPolicy(path)
		if err != nil {
		...
		mergeGithubWorkPolicy(&resolved, *policy, source)
		return nil
	}
	if err := apply(githubGlobalWorkPolicyPath(), "global"); err != nil {
		return githubResolvedWorkPolicy{}, err
	}
...
func mergeGithubWorkPolicy(target *githubResolvedWorkPolicy, policy githubWorkPolicy, source string) {
...
func githubRepoNativeEnabled(policy *githubResolvedWorkPolicy) bool {
...
func detectGithubReviewRulesSummary(repoSlug string) *githubReviewRulesSummary {
	...
	path := githubManagedPaths(repoSlug).ReviewRulesPath
...
func buildGithubControlPlaneReviewers(manifest githubWorkManifest, reviewerOverride string, apiBaseURL string, token string) ([]string, error) {
...
func fetchGithubAssignedControlPlaneReviewers(manifest githubWorkManifest, apiBaseURL string, token string) ([]string, error) {
	actors := []string{}
...
func determineGithubHumanGateState(policy *githubResolvedWorkPolicy, createPR bool) (bool, string, string) {
	if policy == nil {
...
func buildGithubRuntimeContextLines(manifest githubWorkManifest) []string {
	lines := []string{}
	...
	if strings.TrimSpace(manifest.PublicationState) != "" {
		lines = append(lines, fmt.Sprintf("Publication state: %s.", manifest.PublicationState))
		if strings.TrimSpace(manifest.PublicationDetail) != "" {
			lines = append(lines, fmt.Sprintf("Publication detail: %s.", manifest.PublicationDetail))
		}
		if strings.TrimSpace(manifest.PublicationError) != "" {
			lines = append(lines, fmt.Sprintf("Publication error: %s.", manifest.PublicationError))
		}
	}
...
func hydrateGithubWorkManifestDefaults(manifest *githubWorkManifest) {
	if manifest == nil {
		return
	}
	if manifest.Policy == nil {
...
func githubEffectiveMergeMethod(policy *githubResolvedWorkPolicy) string {
	if policy == nil {
		return "squash"
	}
