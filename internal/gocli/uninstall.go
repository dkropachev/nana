...
func stripNanaConfigContent(content string) string {
	...
	for _, line := range lines {
		...
		if regexp.MustCompile(`^\s*(notify|model_reasoning_effort|developer_instructions)\s*=`).MatchString(line) {
			continue
		}
		...
		if inFeatures {
			if strings.HasPrefix(trimmed, "[") && strings.HasSuffix(trimmed, "]") {
				inFeatures = false
			} else if regexp.MustCompile(`^\s*(multi_agent|child_agents_md)\s*=`).MatchString(line) {
				continue
			} else if trimmed == "" {
				continue
			}
