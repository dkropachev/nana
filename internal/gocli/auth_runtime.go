...
type managedAuthUsageSample struct {
...
func (m *managedAuthManager) collectUsageSamples() []managedAuthUsageSample {
	if m == nil {
		return nil
	}
	m.mu.Lock()
	settings := m.settings
	accounts := m.registry.orderedAccounts()
	m.mu.Unlock()

	samples := make([]managedAuthUsageSample, 0, len(accounts))
	for _, account := range accounts {
		samples = append(samples, managedAuthUsageSample{
			account: account,
...
func (m *managedAuthManager) evaluateUsageLocked(now time.Time, queueOnly bool, samples []managedAuthUsageSample) error {
