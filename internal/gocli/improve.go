...
func runScoutPrompt(runtime scoutExecutionRuntime, task string, codexArgs []string, alias string) ([]byte, error) {
	...
	args := append([]string{"exec", "-C", runtime.RepoPath}, normalizedCodexArgs...)
	args = append(args, "--", task)

	if strings.TrimSpace(runtime.CodexHome) != "" {
		sessionID := fmt.Sprintf("%s-%d", sanitizePathToken(alias), time.Now().UnixNano())
		sessionInstructionsPath, err := writeSessionModelInstructions(runtime.RepoPath, sessionID, runtime.CodexHome)
		if err != nil {
			return nil, err
		}
		defer removeSessionInstructionsFile(runtime.RepoPath, sessionID)
	...
	if strings.TrimSpace(runtime.CodexHome) != "" {
		cmd.Env = append(buildCodexEnv(NotifyTempContract{}, runtime.CodexHome), "NANA_PROJECT_AGENTS_ROOT="+runtime.RepoPath)
	}
