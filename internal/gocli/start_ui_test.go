...
func TestStartUIWorkItemPausedStateIsPendingAndBlocked(t *testing.T) {
	item := workItem{
		ID:          "wi-paused",
		Status:      workItemStatusPaused,
		PauseReason: "rate limited",
		PauseUntil:  "2026-04-20T00:00:00Z",
	}
	view := startUIWorkItemFromItem(item)
	if !view.Pending || view.AttentionState != "blocked" {
		t.Fatalf("expected paused work item to be pending+blocked, got %+v", view)
	}
	if view.PauseUntil != item.PauseUntil || view.PauseReason != item.PauseReason {
		t.Fatalf("expected pause fields to round-trip, got %+v", view)
	}
}

func TestStartUIAttentionTreatsPausedRunsAndInvestigationsAsBlocked(t *testing.T) {
	if got := startUIWorkRunAttentionState("paused", "rate limited", false); got != "blocked" {
		t.Fatalf("expected paused work run to be blocked, got %q", got)
	}
	if got := startUIInvestigationAttentionState("paused", "rate limited"); got != "blocked" {
		t.Fatalf("expected paused investigation to be blocked, got %q", got)
	}
}

func TestStartUIScoutItemCarriesPauseFields(t *testing.T) {
	...
	item := startWorkScoutJobFromItem(job)
	if item.PauseReason != job.PauseReason || item.PauseUntil != job.PauseUntil {
		t.Fatalf("expected scout pause fields to round-trip, got %+v", item)
	}
}

func TestStartUIFeedbackQueueItemCarriesPauseFields(t *testing.T) {
	item := workItem{
		ID:          "wi-feedback-paused",
		Source:      "github",
		SourceKind:  "thread_comment",
		Subject:     "Reply later",
		Status:      workItemStatusPaused,
	...
	view := startUIFeedbackQueueItemFromItem(item, "reply")
	if view.PauseReason != item.PauseReason || view.PauseUntil != item.PauseUntil {
		t.Fatalf("expected feedback pause fields to round-trip, got %+v", view)
	}
}

func TestLoadStartUIApprovalsSkipsPausedRuns(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	manifest := localWorkManifest{
		...
		RepoRoot:        repoRoot,
		RepoName:        "repo",
		RepoID:          "repo-paused-approval-skip",
		SourceBranch:    "main",
		BaselineSHA:     "abc123",
		SandboxPath:     filepath.Join(localWorkSandboxesDir(), "repo-paused-approval-skip", "lw-paused-approval-skip"),
		SandboxRepoPath: filepath.Join(localWorkSandboxesDir(), "repo-paused-approval-skip", "lw-paused-approval-skip", "repo"),
		PauseReason:     "rate limited",
		PauseUntil:      "2026-04-20T00:00:00Z",
	}
	store, err := openLocalWorkDB()
	...
	for _, item := range items {
		if item.RunID == manifest.RunID {
			t.Fatalf("expected paused run to stay out of approvals, got %+v", item)
		}
	}
}

func TestLoadStartUIApprovalsIncludesRepoAutomationPreflightFailures(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	repoSlug := "acme/widget"
	...
	state.ServiceTasks[startRepoPreflightTaskID()] = startWorkServiceTask{
		ID:            startRepoPreflightTaskID(),
		Kind:          startTaskKindPreflight,
		Queue:         startTaskQueueService,
		Status:        startWorkServiceTaskFailed,
		LastError:     "GitHub auth required. Install `gh` and run `gh auth login`.",
	...
	found := false
	for _, item := range items {
		if item.Kind == "repo_service" && item.RepoSlug == repoSlug {
			found = true
			if !strings.Contains(item.Reason, "gh auth login") {
				t.Fatalf("unexpected repo preflight approval: %+v", item)
			}
			break
		}
	}
	if !found {
		t.Fatalf("expected repo automation preflight approval, got %+v", items)
	}
}
...
func TestLoadStartUIWorkRunsSkipsSupersededBlockedLocalRuns(t *testing.T) {
	...
	repoRoot := t.TempDir()
	blocked := localWorkManifest{
		Version:             4,
		RunID:               "lw-blocked-superseded",
		CreatedAt:           "2026-04-18T00:00:00Z",
		UpdatedAt:           "2026-04-18T00:00:00Z",
		Status:              "blocked",
		...
		RepoName:            "repo",
		RepoSlug:            "acme/widget",
		RepoID:              "repo-superseded",
		SourceBranch:        "main",
		BaselineSHA:         "abc123",
		SandboxPath:         filepath.Join(localWorkSandboxesDir(), "repo-superseded", "lw-blocked-superseded"),
		SandboxRepoPath:     filepath.Join(localWorkSandboxesDir(), "repo-superseded", "lw-blocked-superseded", "repo"),
	...
	completed := localWorkManifest{
		Version:             4,
		RunID:               "lw-completed-superseding",
		...
		SourceBranch:        "main",
		BaselineSHA:         "def456",
		SandboxPath:         filepath.Join(localWorkSandboxesDir(), "repo-superseded", "lw-completed-superseding"),
		SandboxRepoPath:     filepath.Join(localWorkSandboxesDir(), "repo-superseded", "lw-completed-superseding", "repo"),
		FinalApplyStatus:    "pushed",
		FinalApplyCommitSHA: "deadbeef",
	}
	store, err := openLocalWorkDB()
	if err != nil {
		t.Fatalf("openLocalWorkDB: %v", err)
	}
	if err := store.writeManifest(blocked); err != nil {
		_ = store.Close()
		t.Fatalf("write blocked manifest: %v", err)
	}
	if err := store.writeManifest(completed); err != nil {
		_ = store.Close()
		t.Fatalf("write completed manifest: %v", err)
	}
	if err := store.Close(); err != nil {
		t.Fatalf("close store: %v", err)
	}
	if err := markSupersededLocalWorkRuns(completed); err != nil {
	...
	for _, item := range approvals {
		if item.RunID == blocked.RunID {
			t.Fatalf("expected superseded blocked run to stay out of approvals, got %+v", item)
		}
	}
}
...
func TestStartUIAPILocalWorkRunResolve(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	if err := os.WriteFile(inputPath, []byte("resolve via UI\n"), 0o644); err != nil {
		t.Fatalf("write input plan: %v", err)
	}
	sandboxPath := filepath.Join(home, "sandbox-ui-resolve")
	sandboxRepoPath := filepath.Join(sandboxPath, "repo")
	...
	manifest := localWorkManifest{
		...
		MaxIterations:         8,
		FinalApplyStatus:      "blocked-before-apply",
		FinalApplyError:       "blocked for UI test",
		LastError:             "blocked for UI test",
	}
	if err := writeLocalWorkManifest(manifest); err != nil {
		t.Fatalf("write blocked manifest: %v", err)
	}
	...
	defer detailResponse.Body.Close()
	var detail startUIWorkRunDetail
	if err := json.NewDecoder(detailResponse.Body).Decode(&detail); err != nil {
		t.Fatalf("decode work run detail: %v", err)
	}
	if !detail.ResolveAllowed || detail.SyncAllowed {
		t.Fatalf("expected blocked local run detail to allow resolve only, got %+v", detail)
	}
	...
	defer approvalsResponse.Body.Close()
	var approvals startUIApprovalQueueResponse
	if err := json.NewDecoder(approvalsResponse.Body).Decode(&approvals); err != nil {
		t.Fatalf("decode approvals: %v", err)
	}
	foundResolve := false
	for _, item := range approvals.Items {
		if item.Kind == "work_run" && item.RunID == runID {
			foundResolve = item.ActionKind == "resolve_run"
			break
		}
	}
	if !foundResolve {
		t.Fatalf("expected blocked local run approval item to use resolve_run, got %+v", approvals.Items)
	}

	resolveRequest, err := http.NewRequest(http.MethodPost, server.URL+"/api/v1/work/runs/"+runID+"/resolve", nil)
	if err != nil {
		t.Fatalf("new resolve request: %v", err)
	}
	resolveResponse, err := http.DefaultClient.Do(resolveRequest)
	if err != nil {
		t.Fatalf("POST work run resolve: %v", err)
	}
	defer resolveResponse.Body.Close()
	if resolveResponse.StatusCode != http.StatusOK {
		t.Fatalf("unexpected work run resolve status: %d", resolveResponse.StatusCode)
	}
	...
	if updated.Status != "completed" || updated.FinalApplyStatus != "committed" || strings.TrimSpace(updated.FinalApplyCommitSHA) == "" {
		t.Fatalf("expected resolved run manifest, got %#v", updated)
	}
}
...
func TestLoadStartUIRepoSummaryUsesPersistedScoutJobCounts(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
...
func TestListStartUIScoutItemsDoesNotRequeueFailedScoutJobFromPickupState(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	if len(items) != 1 || items[0].Status != startScoutJobFailed || items[0].RunID != "lw-failed" {
		t.Fatalf("expected failed scout item to remain failed, got %+v", items)
	}
	workState, err := readStartWorkState(repoSlug)
	if err != nil {
		t.Fatalf("read start state: %v", err)
	}
	if workState.ScoutJobs[proposalID].Status != startScoutJobFailed || workState.ScoutJobs[proposalID].RunID != "lw-failed" {
		t.Fatalf("expected failed scout job to remain failed, got %+v", workState.ScoutJobs[proposalID])
	}
}
...
func TestLoadStartUIApprovalsSkipsStaleScoutCleanupWhenManifestMissing(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	proposalID := localScoutProposalID(improvementScoutRole, proposal)
	runID := "lw-missing-stale-scout-approval"
	now := time.Now().UTC().Format(time.RFC3339)
	artifactPath := filepath.ToSlash(filepath.Join(".nana", "improvements", "improve-test"))
	if err := writeStartWorkState(startWorkState{
		Version:    startWorkStateVersion,
	...
	if summary.ScoutJobCounts[startScoutJobFailed] != 0 || summary.ScoutJobCounts[startScoutJobQueued] != 1 {
		t.Fatalf("expected missing-manifest stale scout job to count as queued not failed, got %+v", summary.ScoutJobCounts)
	}
	...
	for _, item := range approvals {
		if item.ScoutJobID == proposalID {
			t.Fatalf("expected missing-manifest stale scout cleanup to stay out of approvals, got %+v", item)
		}
	}
	...
	job := workState.ScoutJobs[proposalID]
	if job.Status != startScoutJobQueued || job.RunID != "" {
		t.Fatalf("expected missing-manifest stale scout job to be requeued, got %+v", job)
	}
	if job.RecoveryCount != 1 || job.LastRecoveryReason != localWorkStaleCleanupError || job.LastRecoveredRunID != runID || strings.TrimSpace(job.LastRecoveryAt) == "" {
		t.Fatalf("expected persisted recovery metadata for missing-manifest stale scout job, got %+v", job)
	}
}
