...
type startWorkIssueState struct {
	...
	ManualPriority          int      `json:"manual_priority,omitempty"`
	ManualPriorityUpdatedAt string   `json:"manual_priority_updated_at,omitempty"`
...
type startWorkServiceTaskStatusCounts struct {
	Kind      string `json:"kind"`
...
type startWorkWaitingServiceTaskSummary struct {
	Kind   string `json:"kind"`
	Reason string `json:"reason"`
	Count  int    `json:"count"`
}

type startWorkStatusSummary struct {
	BlockedReasons      []startWorkReasonCount               `json:"blocked_reasons,omitempty"`
	WaitingServiceTasks []startWorkWaitingServiceTaskSummary `json:"waiting_service_tasks,omitempty"`
	ServiceTaskDetail   []startWorkServiceTaskStatusCounts   `json:"service_task_detail,omitempty"`
}
...
func mirrorStartWorkIssues(state *startWorkState, forkRepo string, forkIssuesMode string, apiBaseURL string, token string) error {
	...
	for _, issue := range issues {
		...
		updated := startWorkIssueState{
			...
			LastRunUpdatedAt:        existing.LastRunUpdatedAt,
			LastRunError:            existing.LastRunError,
			PublishedPRNumber:       existing.PublishedPRNumber,
			PublishedPRURL:          existing.PublishedPRURL,
			PublicationState:        existing.PublicationState,
			BlockedReason:           existing.BlockedReason,
			ManualPriority:          existing.ManualPriority,
			ManualPriorityUpdatedAt: existing.ManualPriorityUpdatedAt,
			ScheduleAt:              existing.ScheduleAt,
...
func startWorkStatus(options startWorkOptions) error {
	...
	summary := buildStartWorkStatusSummary(*state)
	if options.JSON {
		return writeStartWorkStatusJSON(os.Stdout, *state, summary)
	}
	...
	if len(state.PlannedItems) > 0 {
		fmt.Fprintf(os.Stdout, "[start] Planned items: queued=%d launching=%d launched=%d failed=%d total=%d\n", plannedCounts[startPlannedItemQueued], plannedCounts[startPlannedItemLaunching], plannedCounts[startPlannedItemLaunched], plannedCounts[startPlannedItemFailed], len(state.PlannedItems))
	}
	if len(summary.ServiceTaskDetail) > 0 {
		parts := make([]string, 0, len(summary.ServiceTaskDetail))
		for _, item := range summary.ServiceTaskDetail {
			parts = append(parts, fmt.Sprintf("%s(q=%d r=%d c=%d f=%d)", item.Kind, item.Queued, item.Running, item.Completed, item.Failed))
		}
		fmt.Fprintf(os.Stdout, "[start] Service task detail: %s\n", strings.Join(parts, " "))
	}
	if len(summary.WaitingServiceTasks) > 0 {
		parts := make([]string, 0, len(summary.WaitingServiceTasks))
		for _, item := range summary.WaitingServiceTasks {
			parts = append(parts, fmt.Sprintf("%s: %s (%d)", item.Kind, item.Reason, item.Count))
		}
		fmt.Fprintf(os.Stdout, "[start] Waiting service tasks: %s\n", strings.Join(parts, "; "))
	}
	if len(summary.BlockedReasons) > 0 {
		reasons := make([]string, 0, len(summary.BlockedReasons))
		for _, item := range summary.BlockedReasons {
...
func buildStartWorkStatusSummary(state startWorkState) startWorkStatusSummary {
	...
	waitingServiceTasks := map[string]map[string]int{}
	for _, issue := range state.Issues {
	...
	for _, task := range state.ServiceTasks {
		...
		if isWaitingStartServiceTask(task) {
			reason := defaultString(strings.TrimSpace(task.LastError), "waiting")
			if waitingServiceTasks[task.Kind] == nil {
				waitingServiceTasks[task.Kind] = map[string]int{}
			}
			waitingServiceTasks[task.Kind][reason]++
		}
	}
	summary := startWorkStatusSummary{}
	for reason, count := range blockedReasons {
		summary.BlockedReasons = append(summary.BlockedReasons, startWorkReasonCount{Reason: reason, Count: count})
	}
	for kind, counts := range waitingServiceTasks {
		for reason, count := range counts {
			summary.WaitingServiceTasks = append(summary.WaitingServiceTasks, startWorkWaitingServiceTaskSummary{
				Kind:   kind,
				Reason: reason,
				Count:  count,
			})
	...
	for kind, countByStatus := range serviceTaskByKind {
		summary.ServiceTaskDetail = append(summary.ServiceTaskDetail, startWorkServiceTaskStatusCounts{
			Kind:      kind,
			Queued:    countByStatus[startWorkServiceTaskQueued],
			Running:   countByStatus[startWorkServiceTaskRunning],
			Completed: countByStatus[startWorkServiceTaskCompleted],
			Failed:    countByStatus[startWorkServiceTaskFailed],
		})
	}
	slices.SortFunc(summary.BlockedReasons, func(a, b startWorkReasonCount) int {
		return strings.Compare(a.Reason, b.Reason)
	})
	slices.SortFunc(summary.WaitingServiceTasks, func(a, b startWorkWaitingServiceTaskSummary) int {
		if cmp := strings.Compare(a.Kind, b.Kind); cmp != 0 {
			return cmp
		}
		return strings.Compare(a.Reason, b.Reason)
	})
	slices.SortFunc(summary.ServiceTaskDetail, func(a, b startWorkServiceTaskStatusCounts) int {
		return strings.Compare(a.Kind, b.Kind)
	})
	return summary
}

func writeStartWorkStatusJSON(target io.Writer, state startWorkState, summary startWorkStatusSummary) error {
	...
	if tasks, ok := document["service_tasks"].(map[string]any); ok {
		for _, raw := range tasks {
			task, ok := raw.(map[string]any)
			if !ok {
				continue
			}
			delete(task, "wait_cycle")
		}
	}
	if summary.BlockedReasons != nil {
		document["blocked_reasons"] = summary.BlockedReasons
	}
	if summary.WaitingServiceTasks != nil {
		document["waiting_service_tasks"] = summary.WaitingServiceTasks
	}
	if summary.ServiceTaskDetail != nil {
		document["service_task_detail"] = summary.ServiceTaskDetail
	}
...
func startWorkResolvePriority(existing startWorkIssueState, issue startWorkIssuePayload, labels []string, sourceFingerprint string) (int, string, string, string, string, string, string) {
	now := time.Now().UTC().Format(time.RFC3339)
	if strings.TrimSpace(existing.ManualPriorityUpdatedAt) != "" && existing.ManualPriority >= 0 && existing.ManualPriority <= 5 {
		return existing.ManualPriority, "manual_override", startWorkTriageCompleted, "manual override " + startWorkPriorityLabel(existing.ManualPriority), sourceFingerprint, now, ""
	}
...
func startWorkIssueHasFreshPriority(issue startWorkIssueState) bool {
	if strings.TrimSpace(issue.PrioritySource) == "manual_label" || strings.TrimSpace(issue.PrioritySource) == "manual_override" {
		return true
	}
...
func startWorkEffectivePriority(issue startWorkIssueState) int {
	if strings.TrimSpace(issue.ManualPriorityUpdatedAt) != "" && issue.ManualPriority >= 0 && issue.ManualPriority <= 5 {
		return issue.ManualPriority
	}
...
func startWorkScheduleParsed(value string) (time.Time, bool) {
...
func startWorkIssueScheduledForFuture(issue startWorkIssueState, now time.Time) bool {
	scheduled, ok := startWorkScheduleParsed(issue.ScheduleAt)
	if !ok {
		return false
	}
...
func startWorkBuildImplementationQueue(state *startWorkState, options startWorkOptions, openForkPRs int) ([]startWorkIssueState, string) {
	...
	now := time.Now().UTC()
	for _, issue := range state.Issues {
		if startWorkIssueReadyForImplementation(issue, options) && !startWorkIssueScheduledForFuture(issue, now) {
	...
	slices.SortFunc(queue, func(a, b startWorkIssueState) int {
		leftPriority := startWorkEffectivePriority(a)
		rightPriority := startWorkEffectivePriority(b)
...
func refreshStartWorkPreferences(state *startWorkState) {
	...
	if state.SourceRepo != "" {
		...
		artifacts["source_review_rules"] = githubManagedPaths(state.SourceRepo).ReviewRulesPath
	...
	if state.ForkRepo != "" {
		...
		artifacts["fork_review_rules"] = githubManagedPaths(state.ForkRepo).ReviewRulesPath
...
func startWorkHasAutomationLabel(labels []string, action string) bool {
	for _, label := range labels {
		...
		if normalized == "nana" || scoutRoleListIncludes(supportedScoutRoleOrder, normalized) {
			return true
		}
	}
	return false
...
func startWorkPlannedItemFingerprint(item startWorkPlannedItem) string {
	return sha256Hex(strings.Join([]string{
		item.ID,
		item.RepoSlug,
		item.Title,
...
func startWorkPlannedItemDue(item startWorkPlannedItem, now time.Time) bool {
...
func startWorkStatePath(repoSlug string) string {
	return githubManagedPaths(repoSlug).StartStatePath
}
