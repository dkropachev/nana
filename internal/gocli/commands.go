...
func Cancel(cwd string) error {
	...
	hadActiveVerifyLoop := false
	if hasVerifyLoop {
		if active, _ := verifyLoop.state["active"].(bool); active {
			hadActiveVerifyLoop = true
			cancelMode("verify-loop")
			if linked, _ := verifyLoop.state["linked_ultrawork"].(bool); linked {
				cancelMode("ultrawork")
			} else if linkedMode, _ := verifyLoop.state["linked_mode"].(string); linkedMode == "ultrawork" {
				cancelMode("ultrawork")
			}
		}
	}
