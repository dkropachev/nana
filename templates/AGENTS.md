...
- Prefer repo/tool evidence over assumption; keep using tools when correctness depends on them.
...
- Delegate only for quality/speed/safety. Leader scopes and verifies; workers stay scoped. Max 6 children.
...
- `$backend-performance-scout` (`~/.codex/skills/backend-performance-scout/RUNTIME.md`): `backend performance scout`, `api hot paths`, `cpu hotspots`
...
- Run independent work in parallel and dependent checks sequentially. Use background jobs for long checks.
...
- NANA state lives in `.nana/`: `.nana/state/`, `.nana/notepad.md`, `.nana/project-memory.json`, `.nana/plans/`, and `.nana/logs/`.
- Telemetry lives in `.nana/logs/context-telemetry.ndjson`; no raw args/out; `nana telemetry summary --run-id <id>` audits 3/5/8 budgets.
- Keep overlay markers stable: `<!-- NANA:RUNTIME:START --> ... <!-- NANA:RUNTIME:END -->` and `<!-- NANA:TEAM:WORKER:START --> ... <!-- NANA:TEAM:WORKER:END -->`.
<!-- NANA:MODELS:START -->
